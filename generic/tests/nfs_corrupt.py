...
class NFSCorruptConfig:
    ...
    def setup(self, force_start=False):
        ...
        if free < required:
            msg = f"Space available: {utils_numeric.format_size_human_readable(free)}, space needed: {self.required_size}"
            raise NFSCorruptError(msg)
        ...
        process.run(
            f"exportfs {self.nfs_ip}:{self.nfs_dir} -o rw,no_root_squash",
            shell=True,
        )
        process.run(
            f"mount {self.nfs_ip}:{self.nfs_dir} {self.mnt_dir} -o rw,soft,timeo=30,retrans=1,vers=3",
            shell=True,
        )
    ...
    def cleanup(self, force_stop=False):
        ...
        process.run(f"umount -l -f {self.mnt_dir}", shell=True)
        process.run(f"exportfs -u {self.nfs_ip}:{self.nfs_dir}", shell=True)
        if force_stop:
            self.stop()
...
def run(test, params, env):
    ...
    write_disk_cmd = f"dd if=/dev/zero of={nfs_devname} oflag=direct"
