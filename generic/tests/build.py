...
def run(test, params, env):
    ...
    for name in params.get("installers", "").split():
        ...
        if installer_obj.minor_failure is True:
            minor_failure = True
            reason = f"{installer_obj.name}_{installer_obj.mode}: {installer_obj.minor_failure_reason}"
            minor_failure_reasons.append(reason)

    if minor_failure:
        test.error(
            "Minor (worked around) failures during build " "test: {}".format(
                ", ".join(minor_failure_reasons)
            )
        )
