...
def run(test, params, env):
    ...
    def add_vlan(test, session, v_id, iface="eth0", cmd_type="ip"):
        ...
        """
        vlan_if = f"{iface}.{v_id}"
        txt = f"Create vlan interface '{vlan_if}' on {iface}"
        ...
        if cmd_type == "vconfig":
            ...
            v_name = f"{iface}.{v_id}"
            cmd = f"ip link add link {iface} {v_name} type vlan id {v_id} "
        else:
            err_msg = f"Unexpected vlan operation command: {cmd_type}, "
    ...
    def set_ip_vlan(session, v_id, vlan_ip, iface="eth0"):
        ...
        """
        iface = f"{iface}.{v_id}"
        txt = f"Assign IP '{vlan_ip}' to vlan interface '{iface}'"
        error_context.context(txt, test.log.info)
        session.cmd(f"ifconfig {iface} {vlan_ip}")
    ...
    def rem_vlan(test, session, v_id, iface="eth0", cmd_type="ip"):
        ...
        rem_vlan_cmd = ""
        if cmd_type == "vconfig":
            rem_vlan_cmd = f"vconfig rem {v_iface}"
        elif cmd_type == "ip":
            rem_vlan_cmd = f"ip link delete {v_iface}"
        else:
            err_msg = f"Unexpected vlan operation command: {cmd_type}, "
            err_msg += "only support 'ip' and 'vconfig' now"
            test.error(err_msg)
        error_context.context(f"Remove vlan interface '{v_iface}'.", test.log.info)
    ...
    def nc_transfer(test, src, dst):
        ...
        try:
            ...
            test.fail("Fail to receive file" f" from vm{src + 1} to vm{dst + 1}")
    ...
    def flood_ping(src, dst):
        ...
        """
        txt = (
            f"Flood ping from {vms[src].name} interface {ifname[src]} to {vlan_ip[dst]}"
        )
        error_context.context(txt, test.log.info)
    ...
    def get_netkvmco_path(session):
        ...
        middle_path = f"{guest_name}\\{guest_arch}"
    ...
    for vm_index, vm in enumerate(vms):
        ...
        error_context.base_context(f"Prepare test env on {vm.name}")
        session = vm.wait_for_login(timeout=login_timeout)
        if not session:
        ...
        for vlan_i in range(1, vlan_num + 1):
            ...
            v_ip = f"{subnet}.{vlan_i}.{ip_unit[vm_index]}"
    ...
    if params["os_type"] == "windows":
        for vm_index, vm in enumerate(vms):
            ...
            # window get loss=0 when ping fail sometimes, need further check
                ...
                test.fail(f"Guests ping test hit unexpected loss, error info: {output}")
    ...
    try:
        for vlan in range(1, vlan_num + 1):
            error_context.base_context(f"Test for vlan {vlan}", test.log.info)
            ...
            for vm_index, vm in enumerate(vms):
                for vlan2 in range(1, vlan_num + 1):
                    ...
                    if (vlan == vlan2) ^ (status == 0):
                        err_msg = f"{interface} ping {dest} unexpected, "
                        err_msg += f"error info: {output}"
                        test.fail(err_msg)
    ...
    if params.get("do_maximal_test", "no") == "yes":
        ...
        error_context.context(f"Create vlan with ID {bound} in guest", test.log.info)
        try:
            add_vlan(test, sessions[0], bound, ifname[0], cmd_type)
            test.fail(f"Maximal ID allow to vlan is {maximal}")
