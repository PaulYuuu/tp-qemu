...
def run(test, params, env):
    ...
    pool_name = f"{client_names[0]}_pool"
    submission_name = "{}_{}".format(client_names[0], params.get("submission_name"))
    cmd = "{} {} {} {} {} {}".format(
        ...
        if m != 0:
            ...
            if errors:
                ...
                test.error(
                    ...
                    f"could not find '{prompt}' prompt"
                )
        ...
        if "logs" in r:
            try:
                ...
                try:
                    ...
                    # readable)
                    link_name = "logs_{}".format(r["report"].split("\\")[-1])
    ...
    errors = []
    if failed_jobs:
        errors += [f"Jobs failed: {failed_jobs}."]
    if running_jobs:
        for vm in vms:
            vm.destroy()
        errors += [f"Jobs did not complete on time: {running_jobs}."]
    if errors:
