...
def start_netserver_win(session, start_cmd, test):
    ...
    if not check_reg.findall(session.cmd_output("tasklist")):
        ...
        if not utils_misc.wait_for(
            ...
            msg = f"Can not start netserver with command {start_cmd}"
            test.fail(msg)
...
def run(test, params, env):
    ...
    def mtu_set(mtu):
                ...
                for nic in vm.virtnet:
                    if nic.netdst in ovs_br_all:
                        ...
                        vf_pci = netperf_base.ssh_cmd(
                            host,
                            f"vdpa dev show |grep {nic.netdst} | grep -o 'pci/[^[:space:]]*' | awk -F/ '{{print $2}}'",
                        )
                        pf_pci = netperf_base.ssh_cmd(
                            host,
                            f"grep PCI_SLOT_NAME /sys/bus/pci/devices/{vf_pci}/physfn/uevent | cut -d'=' -f2",
                        )
                        port = netperf_base.ssh_cmd(
                            host, f"ls /sys/bus/pci/devices/{pf_pci}/net/ | head -n 1"
                        )
                        ovs_br_vdpa = netperf_base.ssh_cmd(
                            host, f"ovs-vsctl port-to-br {port}"
                        )
                        cmd = f"ovs-ofctl add-flow {ovs_br_vdpa} 'in_port=1,idle_timeout=0 actions=output:2'"
                        cmd += f"&&  ovs-ofctl add-flow {ovs_br_vdpa} 'in_port=2,idle_timeout=0 actions=output:1'"
                        cmd += f"&&  ovs-ofctl dump-flows {ovs_br_vdpa}"
                        netperf_base.ssh_cmd(host, cmd)
                        ovs_br.append(ovs_br_vdpa)
                for br in ovs_br:
                    ovs_list = f"ovs-vsctl list-ports {br}"
        ...
        error_context.context(f"Change all Bridge NICs MTU to {mtu}", test.log.info)
            ...
            if cmd:
                ...
                if "querysettings" in cmd:
                    if ".sys" in o:
                        ...
                        if ".sys" in output:
                            msg = f"{verifier_clear_cmd} does not work correctly"
                            test.error(msg)
                elif s != 0:
                    msg = f"Config command {cmd} failed. Output: {o}"
                    test.error(msg)
            session.close()
    ...
    if failover_exist:
        if params.get("os_type") == "linux":
            ...
            for i in ifname:
                cmd = f"ethtool -i {i} |grep driver| awk -F': ' '{{print $2}}'"
                driver = session.cmd_output(cmd).strip()
                if driver == "net_failover":
                    session.cmd_output(f"dhclient -r && dhclient {i}")
                    break
            ...
            for driver_name in driver_name.split():
                ...
                if driver_name == "netkvm":
                    ...
                    error_context.context(
                        f"Uninstall {driver_name} driver", test.log.info
                    )
                    ...
                    for hwid in device_hwid.split():
                        install_driver_cmd = f"{devcon_path} install {inf_path} {hwid}"
                ...
                if driver_name == "VIOPROT":
                    ...
                    if status:
                        test.error(f"Install inf file failed, output={output}")
    ...
    if queues > 1:
        if params.get("os_type") == "linux":
            session.cmd_status_output(f"ethtool -L {ethname} combined {queues}")
        else:
    ...
    if params.get("server_private_ip") and params.get("os_type") == "linux":
        ...
        cmd += f" && ifconfig {ethname} {server_ip} up"
    ...
    for i in range(2):
        if client in params.get("vms"):
            ...
            cmd = "ifconfig {} {} up".format(
                params.get("client_physical_nic"), client_ip
            )
    ...
    try:
        ...
        if params.get("log_hostinfo_script"):
            ...
            process.system_output(f"bash {src} {test.resultsdir} &> {path}", shell=True)
        ...
        if params.get("log_guestinfo_script") and params.get("log_guestinfo_exec"):
            ...
            output = server_ctl.cmd_output(f"{logexec} {destpath}")
            logfile = open(path, "a+")
        ...
        if mtu != 1500:
            mtu_default = 1500
            error_context.context(
                f"Change back server, client and host's mtu to {mtu_default}"
            )
            mtu_set(mtu_default)
        if (
            ...
            cmd = "ifconfig {} 0.0.0.0".format(params.get("client_physical_nic"))
...
def start_test(
    ...
    fd = open(f"{resultsdir}/netperf-result.{time.time()}.RHS", "w")
    ...
    for i in range(int(params.get("queues", 0))):
        record_list.append(f"rx_intr_{i}")
    record_list.append("rx_intr_sum")
    for i in range(int(params.get("queues", 0))):
        record_list.append(f"tx_intr_{i}")
    ...
    for protocol in protocols.split():
        error_context.context(f"Testing {protocol} protocol", test.log.info)
        protocol_log = ""
        ...
        for i in sizes_test:
            for j in sessions_test:
                if protocol in ("TCP_RR", "TCP_CRR"):
                    nf_args = f"-t {protocol} -v 1 -- -r {i},{i}"
                elif protocol == "TCP_MAERTS":
                    nf_args = f"-C -c -t {protocol} -- -m ,{i}"
                else:
                    nf_args = f"-C -c -t {protocol} -- -m {i}"
                ...
                if ret:
                    ...
                    prefix = f"{protocol}--{i}--{j}"
                    for key in key_list:
                        test.write_test_keyval({f"{prefix}--{key}": ret[key]})
                    ...
                    process.system_output(
                        "rm -f /tmp/netperf.{}.nf".format(ret["pid"]),
                        verbose=False,
                        ignore_status=True,
                        shell=True,
                    )
...
def launch_client(
    ...
    client_path = f"/tmp/netperf-{netperf_version}/src/netperf"
    server_path = f"/tmp/netperf-{netperf_version}/src/netserver"
        ...
        if params.get("os_type") == "windows":
            ...
            if params.get("use_cygwin") == "yes":
                ...
                if "netserver" not in server_ctl.cmd_output("tasklist"):
                    netperf_pack = "netperf-{}".format(params.get("netperf_version"))
                    s_check_cmd = f"dir {netserver_path}"
                    p_check_cmd = f"dir {cygwin_root}"
                    if not (
                        ...
                        cmd = f"xcopy {netperf_src} {cygwin_root} /S /I /Y"
                        server_ctl.cmd(cmd)
            ...
            netperf_base.ssh_cmd(server_ctl, f"pidof netserver || {server_path}")
    ...
    def count_interrupt(name):
        ...
        sum = 0
        intr = []
        stat = netperf_base.ssh_cmd(server_ctl, f"grep {name} /proc/interrupts")
    ...
    def get_state():
        ...
        path = f"find /sys/devices|grep net/{ifname}/statistics"
        cmd = (
            f"{path}/rx_packets|xargs cat;{path}/tx_packets|xargs cat;"
            f"{path}/rx_bytes|xargs cat;{path}/tx_bytes|xargs cat"
        )
        ...
        try:
            ...
            for i in range(len(nrx_intr)):
                state_list.append(f"rx_intr_{i}")
            ...
            for i in range(len(ntx_intr)):
                state_list.append(f"tx_intr_{i}")
    ...
    def all_clients_up():
        try:
            content = netperf_base.ssh_cmd(clients[-1], f"cat {fname}")
        except:
            content = ""
            return False
    ...
    while tries > 0:
        ...
        pid = str(os.getpid())
        fname = f"/tmp/netperf.{pid}.nf"
        netperf_base.ssh_cmd(clients[-1], f"rm -f {fname}")
        ...
        if utils_misc.wait_for(
            ...
            finished_result = netperf_base.ssh_cmd(clients[-1], f"cat {fname}")
