...
def run(test, params, env):
    ...
    def get_mem_info(mem_check_list):
        ...
        for key in mem_check_list:
            ...
            mem_info[key] = int(float(normalize_data_size(f"{value} kB")))
        return mem_info
    ...
    try:
        ...
        try:
            ...
            process.run(
                f"mount -t tmpfs  -o size={tmpfs_size}M none {mem_path}",
                shell=True,
            )
            ...
            if s != 0:
                test.error(f"Disable swap in guest failed as {o}")

            error_context.context("making guest to swap memory", test.log.debug)
            free = mem_info["MemFree"]
            count = free // hugepage_size
            cmd = (
                f"dd if=/dev/zero of={mem_path}/zero bs={hugepage_size}M count={count}"
            )
            ...
            if session is not None:
                process.run(f"umount {mem_path}", shell=True)
