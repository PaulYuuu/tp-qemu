    ...
    def fuzz(test, session, inst_list):
        ...
        for wr_op, operand in inst_list:
            if wr_op == "read":
                ...
                test.error(f"Unknown command {wr_op}")
            ...
            if not session.is_responsive():
                ...
                try:
                    ...
                    test.fail(f"Qemu crash, error info: {err}")
                except virt_vm.VMDeadKernelCrashError as err:
                    test.fail(f"Guest kernel crash, info: {err}")
                else:
                ...
                if vm.process.is_alive():
                    ...
                    test.fail("VM has quit abnormally during " f"{wr_op}: {operand}")
