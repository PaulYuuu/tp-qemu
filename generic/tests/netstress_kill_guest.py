...
def run(test, params, env):
    ...
    def netperf_stress(test, params, vm):
        ...
        try:
            ...
            t_option = f"{test_option} -t {test_protocols}"
            n_client.bg_start(
                utils_net.get_host_ip_address(params),
                t_option,
    ...
    def netdriver_kill_problem(test, session_serial):
        ...
        msg = f"Repeatedly load/unload network driver(s) for {times} times."
        error_context.context(msg, test.log.info)
        for i in range(times):
            for module in modules:
                error_context.context(f"Unload driver {module}. Repeat: {i}/{times}")
                session_serial.cmd_output_safe(f"rmmod {module}")
            for module in modules:
                error_context.context(f"Load driver {module}. Repeat: {i}/{times}")
                session_serial.cmd_output_safe(f"modprobe {module}")
