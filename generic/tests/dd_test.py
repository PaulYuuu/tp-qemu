...
def run(test, params, env):
    ...
    def _get_file(filename, select, test=test):
        ...
        if filename == "NULL":
            ...
            drive_id = params[f"blk_extra_params_{filename}"].split("=")[1]
            drive_path = utils_misc.get_linux_drive_path(session, drive_id)
            if drive_path:
                return drive_path
            test.error(f"Failed to get '{filename}' drive path")
        else:
            # get all matching filenames
            try:
                disks = sorted(session.cmd(f"ls -1d {filename}").split("\n"))
            ...
            try:
                ...
                err = (
                    f"Incorrect cfg: dd_select out of the range (disks={disks},"
                    f" select={select})"
                )
                test.log.error(err)
                test.error(err)
    ...
    def _check_disk_partitions_number():
        ...
        partitions.extend(
            re.findall(
                rf"{dev_id}\d+", " ".join(utils_disk.get_linux_disks(session, True))
            )
        )
    ...
    for arg in ["dd_if", "dd_of"]:
        ...
        path = _get_file(filename, int(params.get(f"{arg}_select", "-1")))
        if bs_count > 1 and filename in params.objects("images"):
            ...
            for i in range(bs_count):
                utils_disk.create_partition_linux(
                    session, dev_id, f"{psize:f}M", f"{start:f}M"
                )
                start += psize
    ...
    try:
        ...
        if stat != dd_stat:
            err = (
                f"Return code doesn't match (expected={dd_stat}, actual={stat})\n"
                f"Output:\n{out}"
            )
            test.fail(err)
        if dd_output not in out:
            err = f"Output doesn't match:\nExpected:\n{dd_output}\nActual:\n{out}"
            test.fail(err)
