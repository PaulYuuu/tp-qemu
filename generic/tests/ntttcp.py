...
def _verify_vm_driver(vm, test, driver_name, timeout=360):
    ...
    error_context.context(
        "Check if driver is installed" f" and verified for vm: {vm.name}",
        test.log.info,
    )
...
def run(test, params, env):
    ...
    results_path = os.path.join(test.resultsdir, f"raw_output_{test.iteration}")
    platform = "x86"
    ...
    try:
        ...
        if not receiver_addr:
            test.error(f"Can't get receiver({vm_receiver.name}) ip address")
        if params.get("numa_node"):
    ...
    try:
        ...
        for i in glob.glob(f"{results_path}.receiver"):
            ...
            desc = f"""#desc# The tests are sessions of "NTttcp", send buf"
" number is {buf_num}. 'throughput' was taken from ntttcp's report.
