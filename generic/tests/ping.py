...
def _ping_with_params(
    ...
    if status != 0:
        test.fail(f"Ping failed, status: {status}," f" output: {output}")
    if params.get("strict_check", "no") == "yes":
        ratio = utils_test.get_loss_ratio(output)
        if ratio != 0:
            test.fail(f"Loss ratio is {ratio}")
...
def run(test, params, env):
    ...
    for ip, interface, session in zip(dest_ips, interfaces, sessions):
        error_context.context(f"Ping test with dest: {ip}", test.log.info)
        ...
        if not ping_ext_host or params.get("os_type") == "linux":
            ...
            _ping_with_params(test, params, ip, interface, session=session, flood=True)
