...
class RtcTest(TimeClientTest):
    def __init__(self, test, params, env, test_name, rtc_path):
        super().__init__(test, params, env, test_name)
        self.def_rtc = rtc_path
        self.maxfreq = 64

    def _test(self):
        if self.session.cmd_status(f"ls {self.def_rtc}"):
            self.test.cancel(f"RTC device {self.def_rtc} does not exist")
        (exit_status, output) = self.session.cmd_status_output(
            f"cd {self.src_dir} && ./rtctest {self.def_rtc} {self.maxfreq}",
            timeout=240,
        )
        if exit_status != 0:
            self.test.fail(f"Test fail on RTC device, output: {output}")
