...
def run_whql_hck_client_install(test, params, env):
    ...
    if params.get("join_domain") == "yes":
        ...
        cmd = (
            f"netdom join {client_name} /domain:{server_domname} /UserD:{client_username} "
            f"/PasswordD:{client_password}"
        )
        session.cmd(cmd, timeout=600)

    error_context.context(
        (f"Setting up auto logon for user '{client_username}'"), LOG_JOB.info
    )
    cmd = (
        "reg add "
