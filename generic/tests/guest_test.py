...
def run(test, params, env):
    ...
    try:
        ...
        if params.get("download") == "yes":
            ...
            rm_cmd = (
                f"cd {dst_rsc_dir} && (rmdir /s /q {rsc_dir} || del /s /q {rsc_dir})"
            )
        ...
        try:
            ...
            if s != 0:
                test.fail(f"Run script '{cmd}' failed, script output is: {o}")
        finally:
