...
def run(test, params, env):
    ...
    def get_ovs_ports(ovs):
        ...
        """
        cmd = f"ovs-vsctl list-ports {ovs}"
    ...
    if not isinstance(host_bridges, utils_net.Bridge):
        ...
        if tmp_ports:
            for p in tmp_ports:
                process.system_output(f"ovs-vsctl del-port {netdst} {p}")
    ...
    try:
        ...
        if os_type == "linux":
            ...
            guest_mtu_cmd = f"ifconfig {ethname} mtu {mtu}"
        else:
            ...
            if os_variant == "winxp":
                ...
                copy_cmd = rf"xcopy {cd_num}:\devcon\wxp_x86\devcon.exe c:\ "
        ...
        arp_add_cmd = f"arp -s {guest_ip} {mac} -i {ifname}"
        ...
        def large_frame_ping(count=100):
            ...
            if ratio != 0:
                test.fail(f"Loss ratio of large frame ping is {ratio}")
            ...
            for size in range(0, max_icmp_pkt_size + 1, step):
                ...
                if status != 0:
                    ...
                    if utils_test.get_loss_ratio(output) > fail_ratio:
                        test.fail(
                            "Ping loss ratio is greater "
                            "than 50{: f}or size {}".format(*size)
                        )
        ...
        if not utils_misc.wait_for(is_mtu_ok, wait_mtu_ok, 0, 1):
            ...
            test.error(f"MTU is not as expected even after {wait_mtu_ok} " "seconds")
        ...
        if params.get("emulate_vf") == "yes":
            ...
            for i in range(1, len(ifnames)):
                ...
                if status != 0:
                    test.log.info(f"Setup vf device's mtu failed with: {output}")
            ifname = ifnames[1]
            ...
            session.cmd_output_safe(f"dhclient {ifname}")
        ...
        if params.get("emulate_vf") == "yes":
            ifname = vm.get_ifname(0)
            guest_ip = vm.get_address(0)
        grep_cmd = f"grep '{guest_ip}.*{ifname}' /proc/net/arp"
        if process.system(grep_cmd, shell=True) == "0":
            process.run(f"arp -d {guest_ip} -i {ifname}", shell=True)
