...
def run(test, params, env):
    ...
    def check_gpt_labletype(disk_index):
        ...
        pattern = rf"Disk {disk_index}.+?B.{{8}}\*"
    ...
    analysisdir = os.path.join(test.resultsdir, f"analysis_{test.iteration}")
    ...
    error_context.context(
        f"Running IOzone command on guest, timeout {iozone_timeout}s", test.log.info
    )
    ...
    error_context.context(f"Write results to {results_path}", test.log.info)
    if status != 0:
        test.fail(f"iozone test failed: {results}")
