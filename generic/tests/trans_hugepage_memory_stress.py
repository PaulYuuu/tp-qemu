...
def run(test, params, env):
    ...
    if not os.path.ismount(debugfs_path):
        ...
        try:
            process.system(f"mount -t debugfs none {debugfs_path}", shell=True)
        except Exception:
            debugfs_flag = 0
    ...
    try:
        ...
        stress_args = f"--cpu 4 --io 4 --vm 2 --vm-bytes {int(test_mem / 2)}M"
        ...
        if nr_ah[1] <= nr_ah[0]:
            test.log.warn("VM don't use transparent hugepage while memory stress")

        if debugfs_flag == 1:
            if int(open(hugetlbfs_path).read()) <= 0:
