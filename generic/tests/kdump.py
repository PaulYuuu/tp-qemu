...
def preprocess_kdump(test, vm, timeout):
    ...
    backup_key_cmd = f"/bin/cp -f {auth_key_path} {auth_key_path}-bk"
    cp_kdumpcf_cmd = f"/bin/cp -f {kdump_cfg_path} {kdump_cfg_path}-bk"
    cp_kdumpcf_cmd = vm.params.get("cp_kdumpcf_cmd", cp_kdumpcf_cmd)

    session = vm.wait_for_login(timeout=timeout)
    if auth_key_path:
        create_key_cmd = f"/bin/touch {auth_key_path}"
...
def postprocess_kdump(test, vm, timeout):
    ...
    kdump_cfg_path = vm.params.get("kdump_cfg_path", "/etc/kdump.conf")
    auth_key_path = vm.params.get("auth_key_path")
    restore_kdumpcf_cmd = f"/bin/cp -f {kdump_cfg_path}-bk {kdump_cfg_path}"
    restore_kdumpcf_cmd = vm.params.get("restore_kdumpcf_cmd", restore_kdumpcf_cmd)

    session = vm.wait_for_login(timeout=timeout)
    if auth_key_path:
        restore_key_cmd = f"/bin/cp -f {auth_key_path}-bk {auth_key_path}"
...
def kdump_enable(
    ...
    error_context.context(f"Try to log into guest '{vm_name}'.", LOG_JOB.info)
    session = vm.wait_for_login(timeout=timeout)

    error_context.context(
        f"Checking the existence of crash kernel in {vm_name}", LOG_JOB.info
    )
    try:
    ...
    if kdump_config:
        ...
        session.cmd(f"cat /dev/null > {kdump_cfg_path}")
        session.cmd(
            f"echo 'core_collector makedumpfile -F -c -d 31' > {kdump_cfg_path}"
        )
...
def crash_test(test, vm, vcpu, crash_cmd, timeout):
    ...
    if kdump_method == "ssh":
        output = session.cmd("cat {}".format(vm.params["kdump_rsa_path"]))
        process.run(vmcore_rm_cmd, shell=True)
        process.run(
            "cat /dev/null > {}".format(vm.params["auth_key_path"]),
            shell=True,
            sudo=True,
        )
...
def run(test, params, env):
    ...
    try:
        for vm_name in vms:
            ...
            vm.copy_files_from(
                kdump_cfg_path, os.path.join(test.debugdir, f"kdump.conf-{vm_name}")
            )
        ...
        for vm in vm_list:
            ...
            vm.copy_files_from(
                kdump_cfg_path,
                os.path.join(test.debugdir, f"kdump.conf-{vm.name}-test"),
            )
            if crash_cmd == "nmi":
