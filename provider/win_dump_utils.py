...
def set_vm_for_dump(test, params):
    ...
    image_size_stg = int(
        float(utils_misc.normalize_data_size("{}M".format(params["mem"]), "G")) * 1.4
    )
...
def generate_mem_dump(test, params, vm):
    ...
    dump_name_zip = f"{dump_name}.zip"
    process.system(f"cd {tmp_dir} && zip {dump_name_zip} {dump_name}", shell=True)
...
def dump_windbg_check(test, params, session):
    ...
    if status:
        test.fail(f"Failed to check dump file by windbg,command out is {output}")
    if not utils_misc.wait_for(
    ...
    if utils_misc.wait_for(
        ...
        output = session.cmd_output(f"type {log_file}")
        test.fail(f"Check dump file failed, output as {output}")
