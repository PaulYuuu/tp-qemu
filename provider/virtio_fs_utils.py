...
def get_virtiofs_driver_letter(test, fs_target, session):
    ...
    error_context.context(
        "Get driver letter of virtio fs target, " f"the driver label is {fs_target}.",
        LOG_JOB.info,
    )
...
def create_viofs_service(test, params, session, service="VirtioFsSvc"):
    ...
    if service == "VirtioFsSvc":
        ...
        if "not exist as an installed service" in output:
            ...
            if sc_create_s != 0:
                test.fail(
                    "Failed to create virtiofs service, " f"output is {sc_create_o}"
                )
    if service == "WinFSP.Launcher":
        ...
        if "completed successfully" not in output.lower():
            test.fail(
                "MultiFS: Config WinFsp.Launcher failed, " f"the output is {output}."
            )
...
def operate_debug_log(test, params, session, vm, operation):
    ...
    :return: session
    """
    error_context.context(f"{operation} virtiofs debug log in guest.", test.log.info)
