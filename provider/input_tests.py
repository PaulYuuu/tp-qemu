...
def key_tap_test(test, params, console, listener, wait_time):
    ...
    for key in keys_dict.keys():
        error_context.context(f"Send {key} key tap event", LOG_JOB.info)
        console.key_tap(key)
...
def mouse_btn_test(test, params, console, listener, wait_time):
    ...
    for btn in btns:
        error_context.context(f"Click mouse {btn} button", LOG_JOB.info)
...
def mouse_scroll_test(test, params, console, listener, wait_time, count=1):
    ...
    for scroll in scrolls:
        error_context.context(f"Scroll mouse {scroll}", LOG_JOB.info)
        if "up" in scroll:
        ...
        if num != count:
            test.fail(
                f"Received scroll number {num} don't match expected"
                f"scroll count {count}"
            )
        if counter:
            test.fail(
                "Received scroll events don't match expected events"
                f"Received scroll events as: {counter}\n Expected events as: {exp_event}"
            )
...
def mouse_move_test(test, params, console, listener, wait_time, end_pos, absolute):
    ...
    error_context.context(f"Moving pointer from {start_pos} to {end_pos}", LOG_JOB.info)
    ...
    error_context.context(
        "Compare if pointer move to destination pos ({}, {})"
        "the missed value should in tolerance scope.".format(*end_pos),
        LOG_JOB.info,
    )
        ...
        if not vertical:
            if abs((k * x + b) - y) > tolerance:  # pylint: disable=E0606
                test.fail(
                    ...
                    f"when move from {start_pos} to {end_pos}. Received pos is ({x}, {y}),"
                ...
                if i > 0:
                    ...
                    if xn - x0 > 0 and dx <= 0:
                        test.fail(
                            ...
                            f"move from {start_pos} to {end_pos}."
                        )
...
def mouse_test(test, params, vm, wait_time, count=1):
    ...
    error_context.context(f"Check if {mice_name} device is working", LOG_JOB.info)
    if not mice_info["current"]:
        test.fail(f"{mice_name} does not worked currently")
