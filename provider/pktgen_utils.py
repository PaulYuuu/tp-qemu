...
class PktgenConfig:
    ...
    def vp_vdpa_bind(self, session_serial):
        try:
            ...
            # Add vDPA device
            vdpa_cmd = f"vdpa dev add name vdpa0 mgmtdev pci/{pci_id}"
    ...
    def configure_pktgen(
        ...
        if pkt_cate == "tx":
            ...
            process.run(f"cp -r {local_path} {remote_path}")
    ...
    def generate_pktgen_cmd(
        ...
        script_path = f"/tmp/pktgen_perf/{script}.sh"
        if script in "pktgen_perf":
            dsc_option = "-m" if pkt_cate == "tx" else "-d"
            cmd = f"{script_path} -i {interface} {dsc_option} {dsc} -t {threads} -s {size}"
        else:
            cmd = f"{script_path} -i {interface} -m {dsc} -n 0 -t {threads} -s {size} -b {burst} -c 0"
...
class PktgenRunner:
    def run_test(self, script, cmd, runner, interface, timeout):
        ...
        packets = f"cat /sys/class/net/{interface}/statistics/tx_packets"
        ...
        try:
            ...
            # need to be killed.
            kill_cmd = (
                f"kill -9 `ps -ef | grep {script} --color | grep -v grep | "
                "awk '{print $2}'`"
            )
            runner(kill_cmd)
        ...
        mpps_results = f"{mpps_results:.2f}"
        return mpps_results
    ...
    def install_package(self, ver, vm=None, session_serial=None):
        ...
        kernel_ver = f"kernel-modules-internal-{ver}"
        cmd_download = f"cd /tmp && brew download-build {kernel_ver} --rpm"
        cmd_install = f"cd /tmp && rpm -ivh  {kernel_ver}.rpm --force --nodeps"
        output_cmd(cmd_download)
        cmd_clean = f"rm -rf /tmp/{kernel_ver}.rpm"
        if session_serial:
            output_cmd = session_serial.cmd_output
            local_path = f"/tmp/{kernel_ver}.rpm"
            remote_path = "/tmp/"
            vm.copy_files_to(local_path, remote_path)
...
def run_tests_for_category(
    ...
    for record in record_list.split():
        record_line += f"{format_result(record, base, fbase)}|"
    ...
    for script in pktgen_script.split():
        for pkt_cate in category.split():
            result_file.write(f"Script:{script} ")
