...
def get_qsd_name_by_image(img_name, params):
    ...
    if params.get(f"drive_format_{img_name}") != "vhost-user-blk-pci":
        return
    qsd_name = ""
...
def create_vubp_devices(vm, img_name, params, bus=None):
    ...
    qsd_basedir = qsd_params.get(
        "qsd_basedir", data_dir.get_data_dir() + f"/qsd/{qsd_name}"
    )

    qid = f"qsd_{qsd_name}"
    img_sock = f"{qsd_basedir}/{qsd_name}_vhost_user_{img_name}.sock"
    devices = []
    ...
    char_params["id"] = f"char_qsd_{qsd_name}"
    ...
    dev_params = {"id": f"vubp_{img_name}", "chardev": char.get_qid()}
    vubp_driver_props = json.loads(params.get(f"image_vubp_props_{img_name}", "{}"))
...
class QsdDaemonDev(QDaemonDev):
    ...
    def __init__(self, name, params):
        ...
        basedir = qsd_params.get(
            "qsd_basedir", data_dir.get_data_dir() + f"/qsd/{name}"
        )
        ...
        sock_path = qsd_params.get("qsd_sock_path", f"{basedir}/{name}_monitor.sock")
        qsd_monitor_id = f"qsd_monitor_{name}"
        qid = f"qsd_{name}"
        super().__init__("QSD", aobject=name, child_bus=QUnixSocketBus(qid, qid))
        self.name = name
        ...
        self.qsd_version = process.run(
            f"{binary} -V", verbose=False, ignore_status=True, shell=True
        ).stdout_text.split()[2]
        self.__qsd_help = process.run(
            f"{binary} -h", verbose=False, ignore_status=True, shell=True
        ).stdout_text
        ...
        if img["export"]["type"] == "nbd":
            ...
            if addr["type"] == "unix":
                if not img[addr["type"]]["path"]:
                    img[addr["type"]]["path"] = (
                        f"{self.basedir}/{self.name}_nbd_{name}.sock"
                    )
            img["nbd-server"]["addr"].update(img[addr["type"]])

        elif img["export"]["type"] == "vhost-user-blk":
            img["unix"]["path"] = f"{self.basedir}/{self.name}_vhost_user_{name}.sock"
            img["export"]["addr"] = img["unix"]
        else:
            raise QsdError("Unknown export type {} ".format(img["export"]["type"]))
        ...
        if image_created:
            ...
            if params.get("qsd_create_image", "yes") == "yes":
                ...
                if result.exit_status != 0:
                    raise QsdError(f"Failed create image {name} ")
    ...
    def has_option(self, option):
        ...
        """
        return bool(re.search(rf"{option}", self.__qsd_help, re.MULTILINE))
    ...
    def start_daemon(self):
        ...
        # check exist QSD
        get_pid_cmd = f"ps -e ww|grep qemu-storage-d|grep {self.sock_path}|"
        ...
        if pid:
            if params.get("qsd_force_create", "yes") == "yes":
                ...
                raise QsdError(f"Find running QSD:{pid}")

        # QSD monitor
        qsd_cmd = f"{self.binary} --chardev socket,server=on,wait=off,path={self.sock_path},id={self.qsd_monitor_id}"
        qsd_cmd += f" --monitor chardev={self.qsd_monitor_id},mode=control "
        ...
        for img in qsd_imgs:
            ...
            qsd_cmd += " --blockdev '{}'".format(json.dumps(img_info["protocol"]))
            qsd_cmd += " --blockdev '{}'".format(json.dumps(img_info["format"]))
            if img_info["filter"]["driver"]:
                qsd_cmd += " --blockdev '{}'".format(json.dumps(img_info["filter"]))
            if img_info["nbd-server"]["addr"]:
                qsd_cmd += " --nbd-server '{}'".format(
                    json.dumps(img_info["nbd-server"])
                )
            qsd_cmd += " --export '{}'".format(json.dumps(img_info["export"]))
        ...
        if params.get("qsd_enable_pidfile", "yes") == "yes":
            if self.check_capability(Flags.PIDFILE):
                self.pidfile = f"{self.basedir}/{self.name}.pid"
                qsd_cmd += f" --pidfile {self.pidfile}"
            else:
        ...
        if self.daemonize:
            ...
            if not self.is_daemon_alive():
                ...
                raise QsdError(f"Failed to run QSD daemon: {output}")
            LOG_JOB.info(
        ...
        if self.pidfile:
            file_pid = process.system_output(f"cat {self.pidfile}", shell=True).decode()
            if file_pid != pid:
                raise QsdError(f"Find mismatch pid: {pid} {file_pid}")
