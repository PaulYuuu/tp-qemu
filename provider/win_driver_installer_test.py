...
def install_gagent(session, test, qemu_ga_pkg, gagent_install_cmd, gagent_pkg_info_cmd):
    ...
    qemu_ga_pkg_path = r"{}:\{}\{}".format(vol_virtio, "guest-agent", qemu_ga_pkg)
    gagent_install_cmd = gagent_install_cmd % qemu_ga_pkg_path
    s_inst, o_inst = session.cmd_status_output(gagent_install_cmd)
    if s_inst != 0:
        test.fail("qemu-guest-agent install failed," f" the detailed info:\n{o_inst}.")
...
def run_installer_with_interaction(
    ...
    error_context.context(
        f"Run virtio-win-guest-tools.exe by {run_installer_cmd}.", LOG_JOB.info
    )
...
def driver_check(session, test, params):
    ...
    for driver_name, device_name in zip(driver_name_list, device_name_list):
        error_context.context(f"{driver_name} Driver Check", LOG_JOB.info)
        inf_path = win_driver_utils.get_driver_inf_path(
            session, test, media_type, driver_name
        )
        expected_ver = session.cmd(
            f"type {inf_path} | findstr /i /r DriverVer.*=", timeout=360
        )
    ...
    if wrong_ver_driver:
        test.fail(f"{wrong_ver_driver} not the expected driver version")
    if not_signed_driver:
        test.fail(f"{not_signed_driver} not digitally signed!")
...
def check_gagent_version(session, test, gagent_pkg_info_cmd, expected_gagent_version):
    ...
    if actual_gagent_version != expected_gagent_version:
        test.fail(
            f"gagent version is not right, expected is {expected_gagent_version} but got {actual_gagent_version}"
        )
...
def get_drive_letter(test, vm, img_size):
    ...
    if not disk_index:
        test.error(f"Failed to get the disk index of size {img_size}")
    if not utils_disk.update_windows_disk_attributes(session, disk_index):
        test.error(f"Failed to enable data disk {disk_index}")
...
def rng_test(test, params, vm):
    ...
    if len(re.findall(r"0x\w", output, re.M)) < 2:
        test.fail("Unable to read random numbers " f"from guest: {output}")
...
def iozone_test(test, params, vm, images):
    ...
    for img in images.split():
        drive_letter = get_drive_letter(test, vm, params[f"image_size_{img}"])
        try:
...
def balloon_test(test, params, vm, balloon_test_win):
    ...
    error_context.context(f"Running {tag} test", test.log.info)
...
def pvpanic_test(test, params, vm):
    ...
    if status:
        test.error(
            f"Command '{set_panic_cmd}' failed, status: {status}, output: {output}"
        )
...
def vioser_test(test, params, vm):
    ...
    if result is not True:
        test.fail(f"Test failed. {result[1]}")
...
def fwcfg_test(test, params, vm):
    ...
    output = vm.monitor.human_monitor_cmd(f"dump-guest-memory -w {dump_file}")
    if output and "warning" not in output:
        test.fail(f"Save dump file failed as: {output}")
    else:
        cmd = f"ls -l {dump_file} | awk '{{print $5}}'"
        dump_size = int(process.getoutput(cmd))
        process.system(f"rm -rf {dump_file}", shell=True)
        if dump_size == 0:
