        ...
        def wrapper(self, *args, **kwargs):
            ...
            if self._dev_type != CDROM:
                ...
                if not utils_misc.wait_for(
                    ...
                    _session.close()
                    raise TestError(
                        f"{action}--> Actual: {len(self._all_disks ^ orig_disks)} disks. Expected: {len(self._imgs)} disks."
                    )
...
class BlockDevicesPlug:
    ...
    def __iter__(self):
        ...
        yield from self._plugged_disks
    ...
    def _check_qmp_outputs(self, action):
        ...
        for dev_id in list(_QMP_OUTPUT.keys()):
            ...
            if output[1] is False:
                err = f"Failed to {action} device {dev_id}. "
                if not output[0] and action == "unplug":
                    err += f"No deleted event generated and {dev_id} still in qtree"
                else:
                    err += output[0]
                raise TestError(err)
    ...
    def _wait_events_deleted(self, timeout=300):
        ...
        if not utils_misc.wait_for(self._get_events_deleted, timeout):
            raise TestError(
                'No "DEVICE DELETED" event generated after unplug "{}".'.format(
    ...
    def _create_devices(self, images, pci_bus={"aobject": "pci.0"}):
        ...
        for img in images:
            ...
            devices_created = getattr(
                self.vm.devices, "{}_define_by_params".format(self._dev_type["name"])
            ...
            for dev in reversed(devices_created):
                ...
                if (
                    ...
                    and qid.startswith(f"{img}_")
                ):
                    self._hotplugged_devs[img].insert(0, dev)
                elif qid.endswith(f"_{img}") or qid == img:
                    self._hotplugged_devs[img].insert(0, dev)
        ...
        try:
            ...
            if not isinstance(qdev_out, list) or len(qdev_out) != 1:
                raise NotImplementedError(
                    f"This device {device} require to hotplug multiple devices {out}, "
            ...
            raise DeviceHotplugError(
                device, f"According to qemu_device: {exc}", self, ver_out
            )
        return out, ver_out
        ...
        for img, devices in devices_dict.items():
            for device in devices:
                ...
                if (
                    ...
                    args += (bus,)
                with _LOCK:
                    _QMP_OUTPUT[device.get_qid()] = getattr(self, f"_{action}_atomic")(
                        *args
                    )
                time.sleep(interval)
    ...
    def _unplug_devs(self, images, monitor, interval=0):
        ...
        for img in images:
            ...
            for dev in devs:
                qid = dev.get_qid()
                if qid == img or qid.startswith(f"{img}_"):
                    self._unplugged_devs[img].insert(0, dev)
                    if qid == img:
                        break
            else:
                raise TestError(f"No such device '{img}' in VM's devices.")
