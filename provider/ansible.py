...
class SyntaxCheckError(Exception):
    ...
    def __str__(self):
        return (
            f'The ansible-playbook command "{self.cmd}" cannot pass syntax check: '
            f"{self.output}"
        )
...
class PlaybookExecutor(Expect):
    def __init__(
        ...
        super().__init__(self._generate_cmd(extra_vars, addl_opts))
    ...
    def _generate_cmd(self, extra_vars=None, addl_opts=None):
        ...
        playbook_cmd_options.extend(
            [self.program, self.site_yml, f"-i {self.inventory}"]
        )
        not self.remote_user or playbook_cmd_options.append(f"-u {self.remote_user}")
        not extra_vars or playbook_cmd_options.append(f"-e '{extra_vars}'")
...
def check_ansible_playbook(params):
    ...
    def _pip_binary():
        ...
        for binary in ["pip", "pip3", "pip2"]:
            if process.system(f"which {binary}", ignore_status=True) == 0:
                return binary
    ...
    def python_install():
        ...
        """
        install_cmd = f"{pip_bin} install ansible"  # pylint: disable=E0606
