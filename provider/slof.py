...
def wait_for_loaded(
    ...
    if not utils_misc.wait_for(
        ...
        test.error(f"No found serial log in {file_timeout} sec.")
    ...
    test.fail(f"No found corresponding SLOF info in serial log during {timeout} sec.")
...
def verify_boot_device(
    ...
    if position in devices:
        ...
        info = (
            f"Check whether the device({parent_bus_type}@{child_bus_type}@{child_addr}) is the {position} bootable "
            "device."
        )
        if sub_child_addr:
            info = (
                f"Check whether the device({parent_bus_type}@{child_bus_type}@{child_addr}@{sub_child_addr}) is the {position} "
                "bootable device."
            )
        LOG_JOB.info(info)
