...
class StorageBenchmark:
    ...
    def __wait_procs_done(self, session, timeout=1800):
        ...
        if not utils_misc.wait_for(
            ...
            raise TestError(f"Not all {proc_name} processes done in {timeout} sec.")
    ...
    def _install_linux(self, src, dst, timeout):
        ...
        """
        self.session.cmd(
            f"cd {src} && ./configure --prefix={dst} && make && make install",
            timeout=timeout,
        )
...
class IozoneLinuxCfg:
    def __init__(self, params, session):
        ...
        self.cmd = (
            f"cd {self.iozone_inst}/src/current && make clean && make {self.arch}"
        )
        self.iozone_path = f"{self.iozone_inst}/src/current/iozone"
...
class Iozone(StorageBenchmark):
    ...
    def __init__(self, params, vm):
        ...
        super().__init__(self.os_type, vm, "iozone")
...
class FioLinuxCfg:
    def __init__(self, params, session):
        ...
        if fio_resource == "distro":
            ...
            self.fio_path = f"{self.fio_inst}/bin/fio"
            scp_benckmark = attrgetter("scp_benckmark")
            unpack_file = attrgetter("unpack_file")
...
class FioWinCfg:
    def __init__(self, params, session):
        ...
        self.fio_msi = {
            "x86_64": rf"{utils_letter}:\{fio_ver}-x64.msi",
            "i686": rf"{utils_letter}:\{fio_ver}-x86.msi",
        }
        self.fio_path = rf'"{self.fio_inst[arch]}\fio\fio.exe"'
        install = attrgetter("install")
...
class Fio(StorageBenchmark):
    ...
    def __init__(self, params, vm):
        self.os_type = params["os_type"]
        super().__init__(self.os_type, vm, "fio")
