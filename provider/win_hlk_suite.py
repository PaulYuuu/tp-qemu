...
class HLKServer:
    def __init__(self, test, vm_server):
        ...
        self._session.set_prompt(rf"toolsHLK@{hostname}")
    ...
    def create_pool(self, name):
        ...
        self._session.cmd_output(f"createpool {name}")
    ...
    def move_machine_from_default_pool(self, machine_name, dst_pool_name):
        ...
        cmd = f"movemachinefromdefaultpool {machine_name} {dst_pool_name}"
    ...
    def set_machine_state(self, machine_name, pool_name, state, timeout=360):
        ...
        cmd = f"setmachinestate {machine_name} {pool_name} {state}"
    ...
    def list_machine_targets(self, machine_name, pool_name, timeout=60):
        ...
        :type: list
        """
        cmd = f"listmachinetargets {machine_name} {pool_name}"
    ...
    def create_project(self, name):
        ...
        self._session.cmd_output(f"createproject {name}")
    ...
    def create_project_target(
        ...
        cmd = f"createprojecttarget {target_key} {project_name} {machine_name} {pool_name}"
        self._session.cmd_output(cmd, timeout)
    ...
    def list_tests(self, target_key, project_name, machine_name, pool_name, timeout=60):
        ...
        :rtype: list
        """
        cmd = f"listtests {target_key} {project_name} {machine_name} {pool_name}"
    ...
    def queue_test(self, test_id, target_key, project_name, machine_name, pool_name):
        ...
        cmd = f"queuetest {test_id} {target_key} {project_name} {machine_name} {pool_name}"
        self._session.cmd_output(cmd)
    ...
    def zip_test_result_logs(
        ...
        :rtype: str
        """
        cmd = f"ziptestresultlogs {result_index} {test_id} {target_key} {project_name} {machine_name} {pool_name}"
        LOG_JOB.info(
    ...
    def list_test_results(
        ...
        :rtype: str
        """
        cmd = f"listtestresults {test_id} {target_key} {project_name} {machine_name} {pool_name}"
    ...
    def run_tests(
        ...
        if err_msg:
            raise HLKRunError(f"Found task error messages:{err_msg}")
...
def install_hlk_client(vm_client, vm_server, timeout=1200):
    ...
    inst_cmd = rf"\\{server_ip}\HLKInstall\Client\Setup.cmd /qn ICFAGREE=Yes"
...
def download_hlk_server_image(params, src_img_uri, timeout=1800):
    ...
    if re.search(r'(^http:)|(^https:)|(^ftp")|(^ftps:")', src_img_uri):
        ...
        if not os.path.exists(dst_img_path):
            ...
            curl_check_cmd = f"curl -I -L -k -m 120 {src_img_uri}"
            output = process.run(curl_check_cmd).stdout_text
            if "File not found" in output:
                raise HLKError(f"Invalid URI {src_img_uri}.")

            LOG_JOB.info(
                "Downloading HLK Server from %s to %s/:", src_img_uri, dst_img_dir
            )
            curl_download_cmd = f"curl -o {dst_img_path} {src_img_uri}"
        ...
        info_cmd = f"{qemu_binary} info {dst_img_path} --output=json"
        ...
        raise HLKError(f"No supported URI: {src_img_uri}.")
