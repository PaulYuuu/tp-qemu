...
def uninstall_driver(session, test, devcon_path, driver_name, device_name, device_hwid):
    ...
    uninst_store_cmd = f"pnputil /f /d {inf_list[0]}"
...
def get_driver_inf_path(session, test, media_type, driver_name):
    ...
    try:
        ...
        test.error(f"Not supported virtio win media type '{media_type}'")
...
def install_driver_by_virtio_media(
    ...
    for hwid in device_hwid.split():
        ...
        if status > 1:
            test.fail(
                f"Failed to install driver '{driver_name}', " f"details:\n{output}"
            )
        installed_any |= True
...
def run_installer(vm, session, test, params, run_installer_cmd):
    ...
    installer_restart_version = params.get("installer_restart_version", "[1.9.37.0,)")
...
def copy_file_to_samepath(session, test, params):
    ...
    installer_path = r"{}:\{}".format(vol_virtio, "virtio-win-guest-tools.exe")
    ...
    if params.get("msi_name"):
        msi_path = r"{}:\{}".format(vol_virtio, params["msi_name"])
    ...
    for src_file in src_files:
        copy_cmd = f"xcopy {src_file} {dst_path} /Y"
        status, output = session.cmd_status_output(copy_cmd)
        if status != 0:
            test.error("Copy file error," f" the detailed info:\n{output}.")
...
def enable_driver(session, test, cmd):
    ...
    if status != 0:
        test.fail(f"failed to enable driver, {output}")
...
def disable_driver(session, vm, test, cmd):
    ...
    if status != 0:
        if "reboot" in output:
            ...
            test.fail(f"failed to disable driver, {output}")
    return session
...
def get_device_id(session, test, driver_name):
    ...
    for hwid in device_hwid.split():
        ...
        if hwid in device_id:
            find_devices = True
    if not find_devices:
        test.fail(f"Didn't find driver info from guest {output}")
    return device_id
...
def load_driver(session, test, params, load_method="enable"):
    ...
    if load_method != "enable":
        ...
        cmd = f'{devcon_path} enable "@{device_id}"'
...
def unload_driver(session, vm, test, params, load_method="enable"):
    ...
    if load_method != "enable":
        ...
        device_id = get_device_id(session, test, driver_name)
        cmd = f'{devcon_path} disable "@{device_id}"'
