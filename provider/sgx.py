...
class SGXHostCapability:
    ...
    def validate_sgx_cap(self):
        ...
        try:
            ...
            self._test.cancel(f"Host sgx capability check fail {e}")
        else:
    ...
    def validate_numa_node_count(self):
        ...
        if len(node_list) < monitor_expect_nodes:
            self._test.cancel(
                f"host numa nodes {node_list} isn't enough for " "testing."
            )
...
class SGXChecker:
    ...
    def verify_sgx_flags(self, qmp_command, flags):
        ...
        for ele in flags:
            if qmp_command[ele] is not True:
                self._test.fail(f"{ele} is not enabled, qmp check failed.")
    ...
    def verify_qmp_host_sgx_cap(self, host_epc_size):
        ...
        if host_epc_size != host_qmp_section_size:
            self._test.fail(
                f"Host epc size {host_epc_size} is not equal to query sgx"
                f"capabilities section size {host_qmp_section_size}"
            )
    ...
    def verify_qmp_guest_sgx_cap(self):
        ...
        if config_epc_size != sgx_section_size:
            self._test.fail(
                f"Guest epc size {config_epc_size} is not equal to query_sgx"
                f" section size {sgx_section_size}"
            )
    ...
    def verify_guest_epc_size(self, cmd_output):
        ...
        if guest_msg_epc_size != int(guest_total_epc_size):
            self._test.fail(
                f"Guest epc size {guest_msg_epc_size} is not equal to qemu set "
                f"section size {guest_total_epc_size}"
            )
