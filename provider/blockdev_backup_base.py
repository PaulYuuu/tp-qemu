...
class BlockdevBackupBaseTest:
    ...
    def blockdev_backup(self):
        ...
        source_lst = list(map(lambda x: f"drive_{x}", self.source_disks))
        target_lst = list(map(lambda x: f"drive_{x}", self.target_disks))
        bitmap_lst = list(map(lambda x: f"bitmap_{x}", range(len(self.source_disks))))
        try:
            if len(source_lst) > 1:
                error_context.context(
                    f"backup {source_lst} to {target_lst}, options: {self.backup_options}"
                )
                ...
                error_context.context(
                    f"backup {source_lst[0]} to {target_lst[0]}, options: {self.backup_options}"
                )
