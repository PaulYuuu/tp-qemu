...
class ThrottleTester:
    ...
    def build_default_option(self):
        ...
        def _count_normal_iops(variables, iops_type):
            iops_val = variables[f"iops_{iops_type}"]
            bps_val = variables[f"bps_{iops_type}"]
            normal_iops = 0
        ...
        def _count_burst_iops(variables, iops_type):
            iops_max = variables[f"iops_{iops_type}_max"]
            iops_length = variables[f"iops_{iops_type}_max_length"]
            bps_max = variables[f"bps_{iops_type}_max"]
            bps_length = variables[f"bps_{iops_type}_max_length"]
            normal_iops = variables[f"normal_{iops_type}_iops"]
            burst_iops = 0
    ...
    def build_image_fio_option(self, image):
        ...
        image_data["fio_option"] = self._fio_option + f" --filename={name}"
        return image_data
