...
def run(test, params, env):
    ...
    class TestMultihostMigrationLongWait(base_class):
        ...
        def firewall_block_port(self, port):
            utils.run(
                f"iptables -A INPUT -p tcp --dport {port}" " -j REJECT",
                ignore_status=True,
            )
        ...
        def migrate_vms_src(self, mig_data):
            super().migrate_vms_src(mig_data)
            self._hosts_barrier(
            ...
            if params.get("mig_cancel", "no") == "yes":
                ...
                for _ in range(self.mig_fir_timeout):
                    ...
                    raise error.TestWarn(
                        ...
                        f" is too short: {self.mig_fir_timeout}. For completing"
    ...
    if sub_type == "long_wait":
        ...
        raise error.TestNAError(f"Unsupported sub_type = '{sub_type}'.")
    mig.run()
