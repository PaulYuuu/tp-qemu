    ...
    class TestMultihostMigration(base_class):
        ...
        def migration_scenario(self):
            ...
            if self.hostid == self.master_id():
                try:
                    utils.run(
                        f"dd if=/dev/zero of={host_path} bs=1M" f" count={file_size}"
                    )
                    ...
                    if wrong_check_sum:
                        raise error.TestFail(
                            f"Returned file hash ({returned_hash}) differs"
                            f"from original one ({orig_hash})"
                        )
                    else:
                        # clean temp
                        utils.run(f"rm -rf {host_path}")
                        utils.run(f"rm -rf {host_path_returned}")
