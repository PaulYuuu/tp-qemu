    ...
    class TestMultihostMigration(base_class):
        ...
        def check_diff(self, mig_data):
                ...
                if self.is_src:
                    ...
                    if not s:
                        raise error.TestFail(
                            f"Guest didn't use '{clocksource}' " "clocksource"
                        )
            ...
            if time_drifted:
                ...
                for vm in mig_data.vms:
                    difs += (
                        f"\n            VM={vm.name}  HOST={self.diff_ht[vm.name]}s  GUEST={self.diff_gt[vm.name]}s"
                        f" DIFF={self.diff_ht[vm.name] - self.diff_gt[vm.name]}"
                    )
                raise error.TestError(
                    "Time DIFFERENCE for VM is greater than"
                    f" LIMIT:{self.diff_limit}s.{difs}\n"
                )
