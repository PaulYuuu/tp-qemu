    ...
    class TestMultihostMigration(mig_type, migration.MigrationBase):
                ...
                for i in range(len(mig_total_time_list)):
                    if min(mig_total_time_list) != mig_total_time_list[-1]:
                        raise error.TestFail(
                            ...
                            f"error, {mig_total_time_list[-1]} should be minimum, "
                            f"but actual is {min(mig_total_time_list)}"
                        )
                    else:
                    ...
                    if min(mig_downtime_list) != mig_downtime_list[-1]:
                        raise error.TestFail(
                            ...
                            f"error, {mig_downtime_list[-1]} should be minimum, "
                            f"but actual is {min(mig_downtime_list)}"
                        )
                    else:
                    ...
                    if min(transferred_ram_list) != transferred_ram_list[-1]:
                        raise error.TestFail(
                            ...
                            f"{transferred_ram_list[-1]} should be minimum, but actual is {min(transferred_ram_list)}"
                        )
                    else:
        ...
        def post_migration_capability_with_xbzrle_off_on(
            ...
            try:
                ...
                raise error.TestFail(
                    "Migration failed with setting cache " f"size to {cache_size}."
                )
