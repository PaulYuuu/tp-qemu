...
def run(test, params, env):
    ...
    class TestMultihostMigration(mig_type, migration.MigrationBase):
        ...
        def __init__(self, test, params, env):
            ...
            super().__setup__(test, params, env, self.srchost, self.dsthost)
            ...
            if self.check_running_cmd:
                if not utils_misc.wait_for(check_running, timeout=360):
                    raise error.TestFail(
                        f"Failed to start '{self.bg_stress_test}' in guest."
                    )
        ...
        def get_sar_output(self):
            ...
            while vm.is_alive():
                ...
                if s != 0:
                    raise error.TestFail(
                        "Failed to get sar output in guest." f"The detail is: {o}"
                    )
            session.close()
        ...
        def check_mig_cpu_throttling_percentage(self):
            ...
            if (self.parameters_value[0] not in cpu_throttling_percentage_list) or (
                ...
                raise error.TestFail(
                    ...
                    f"should include: {self.parameters_value[0]} {sum(self.parameters_value)}"
                )
            if min(cpu_throttling_percentage_list) != self.parameters_value[0]:
                raise error.TestFail(
                    f"The expected cpu-throttle-initial is {self.parameters_value[0]},"
                    f" but the actual value is {min(cpu_throttling_percentage_list)}"
                )
            if max(cpu_throttling_percentage_list) > 99:
                raise error.TestFail(
                    "The expected max cpu-throttling percentage"
                    f"is {99}, but the actual value is {max(cpu_throttling_percentage_list)}"
                )
        ...
        def post_migration_capability_load_host_io(
            ...
            try:
                ...
                raise error.TestFail(
                    "Migration failed with set auto-converge" f": {set_auto_converge}"
                )
            finally:
                if self.session:
                    self.session.close()
        ...
        def migration_scenario(self):
            error.context(
                f"Migration from {self.srchost} to {self.dsthost} over protocol {mig_protocol}.",
                logging.info,
            )
