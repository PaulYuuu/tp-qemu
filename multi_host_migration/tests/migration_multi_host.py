...
def run(test, params, env):
    ...
    class TestMultihostMigration(mig_type):
        def __init__(self, test, params, env):
            super().__init__(test, params, env, preprocess_env)
        ...
        def run_pre_sub_test(self):
            ...
            if self.is_src:
                if self.pre_sub_test:
                    ...
                    error.context(
                        f"Run sub test '{self.pre_sub_test}' before migration on src",
                        logging.info,
                    )
        ...
        def run_post_sub_test(self):
            ...
            if not self.is_src:
                if self.post_sub_test:
                    error.context(
                        f"Run sub test '{self.post_sub_test}' after migration on dst",
                        logging.info,
                    )
            ...
            def start_worker(mig_data):
                ...
                if s != 0:
                    raise error.TestError(
                        "Failed to run bg cmd in guest," f" Output is '{o}'."
                    )
                time.sleep(5)
                ...
                if not self.is_src:
                    ...
                    if s:
                        raise error.TestFail(
                            "Background command not found," f" Output is '{o}'."
                        )
