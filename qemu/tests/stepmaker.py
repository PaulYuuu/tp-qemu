...
from virttest import ppm_utils, qemu_monitor, step_editor, utils_misc
...
class StepMaker(step_editor.StepMakerWindow):
    ...
    def __init__(self, vm, steps_filename, tempdir, params):
        ...
        self.steps_file.write(f"# Generated on {time.asctime()}\n")
        self.steps_file.write(
            "# uname -a: {}\n".format(process.system_output("uname -a", verbose=False))
        )
    ...
    def redirect_timer(self, delay=0, func=None):
        ...
        if func is not None:
            self.timer_id = gobject.timeout_add(
                delay, func, priority=gobject.PRIORITY_LOW
            )
    ...
    def event_break_clicked(self, widget):
        ...
        self.entry_time.set_text(f"{self.run_time:.2f}")
        ...
        self.window.set_title(
            "Step Maker -- step %d at time %.2f" % (self.step_num, self.run_time)
        )
        ...
        for line in lines.splitlines():
            ...
            if words and words[0] == "var":
                ...
                if varname in self.vars.keys():
                    ...
                    val = self.inputdialog(f"${varname} =", "Variable")
                    if val is None:
                        return
                    var_dict[varname] = val
        for varname in var_dict.keys():
            self.vars_file.write(f"{varname}={var_dict[varname]}\n")
    ...
    def event_capture_clicked(self, widget):
        ...
        gtk.gdk.display_get_default().warp_pointer(
            gtk.gdk.screen_get_default(), self.prev_x, self.prev_y
        )
    ...
    def update_capture(self):
        ...
        if (x, y) != (self.prev_x, self.prev_y):
            self.vm.monitor.mouse_move(-8000, -8000)
            time.sleep(delay)
            self.vm.monitor.mouse_move(
                self.mouse_click_coords[0], self.mouse_click_coords[1]
            )
            time.sleep(delay)
        ...
        self.redirect_timer(int(self.spin_latency.get_value()), self.update_capture)
        return True
    ...
    def event_capture_button_release(self, widget, event):
        ...
        self.set_image(
            self.image_width_backup, self.image_height_backup, self.image_data_backup
        )
    ...
    def event_capture_scroll(self, widget, event):
        if event.direction == gtk.gdk.SCROLL_UP:
            ...
            direction = -1
        self.spin_sensitivity.set_value(self.spin_sensitivity.get_value() + direction)
        pass
