...
def cmd_runner_monitor(test, vm, monitor_cmd, test_cmd, guest_path, timeout=300):
    """
    ...
    def thread_kill(cmd, p_file):
        ...
        o = process.system_output(
            "pstree -p {}".format(fd["pid"]), verbose=False, ignore_status=True
        )
        tmp = re.split(r"\s+", cmd)[0]
        pid = re.findall(rf"{tmp}.(\d+)", o)[0]
        cmd_result = process.run(f"kill -9 {pid}", verbose=False, ignore_status=True)
        fd.close()
    ...
    tag = vm.instance
    pid_file = f"/tmp/monitor_pid_{tag}"
    result_file = f"/tmp/host_monitor_result_{tag}"

    monitor = threading.Thread(
        target=monitor_thread, args=(monitor_cmd, pid_file, result_file)
    )
    test_runner = threading.Thread(
        target=test_thread,
        args=(session, monitor_cmd, test_cmd, pid_file, kill_thread_flag, timeout),
    )
    monitor.start()
    test_runner.start()
    ...
    guest_result_file = f"/tmp/guest_result_{tag}"
    guest_monitor_result_file = f"/tmp/guest_monitor_result_{tag}"
    vm.copy_files_from(guest_path, guest_result_file)
    vm.copy_files_from(f"{guest_path}_monitor", guest_monitor_result_file)
    return tag
...
def run(test, params, env):
    ...
    test.outputdir.split(".performance.")[0]
    ...
    session.cmd("tar -xf /tmp/{} -C {}".format(tar_name, "/tmp/src_tmp"))
    ...
    session.cmd(f"mv /tmp/src_tmp/{new_file} /tmp/src")  # pylint: disable=E0606

    if test_patch:
        test_patch_path = os.path.join(
            data_dir.get_deps_dir(), "performance", test_patch
        )
        vm.copy_files_to(test_patch_path, "/tmp/src")
        session.cmd(f"cd /tmp/src && patch -p1 < /tmp/src/{test_patch}")

    compile_cmd = params.get("compile_cmd")
    if compile_cmd:
        session.cmd(f"cd /tmp/src && {compile_cmd}")
    ...
    cmd = f'cd /tmp/src && python /tmp/cmd_runner.py "{monitor_cmd} &> '
    cmd += f'/tmp/guest_result_monitor"  "/tmp/src/{test_cmd}'
    cmd += ' &> %s " "/tmp/guest_result"'
    cmd += f" {int(test_timeout)}"

    test_cmd = cmd
    # Run guest test with monitor
    tag = cmd_runner_monitor(
        test, vm, monitor_cmd, test_cmd, guest_path, timeout=test_timeout
    )

    # Result collecting
    result_list = [
        f"/tmp/guest_result_{tag}",
        f"/tmp/host_monitor_result_{tag}",
        f"/tmp/guest_monitor_result_{tag}",
    ]
    ...
    for i in result_list:
        if re.findall("monitor_result", i):
            result = utils_test.summary_up_result(
                i, ignore_pattern, head_pattern, row_pattern
            )
            fd = open(f"{i}.sum", "w")
            sum_info = {}
            head_line = ""
            for keys in result:
                ...
                for col in result[keys]:
                    ...
                    if col_sum in sum_info:
                        sum_info[col_sum] += f"\t{result[keys][col]}"
                    else:
                        sum_info[col_sum] = f"{col}\t{result[keys][col]}"
            fd.write(f"{head_line}\n")
            for keys in sum_info:
                fd.write(f"{sum_info[keys]}\n")
            fd.close()
            shutil.copy(f"{i}.sum", guest_results_dir)
...
def result_sum(topdir, params, guest_ver, resultsdir, test):
    ...
    kvm_ver = process.system_output(
        params.get("ver_cmd", "rpm -q qemu-kvm"), shell=True
    )
    host_ver = os.uname()[2]
                ...
                if (
                    ...
                    for i, pattern in enumerate(file_list):
                        if re.findall(pattern, file):
                            prefix = re.findall(
                                rf"{case_type}\.[\d\w_\.]+", file_dir_norpt
                            )[0]
    ...
    for prefix in results_files:
        ...
        result_context_file = open(results_files[prefix][0])
    ...
    for index, tag in enumerate(order_list):
        headline += f"{format_result(tag)}|"
        order_line += "DATA%d|" % index
    headline = headline.rstrip("|")
    order_line = order_line.rstrip("|")

    result_path = utils_misc.get_path(resultsdir, f"{case_type}-result.RHS")
    if os.path.isfile(result_path):
        result_file = open(result_path, "r+")
    else:
        result_file = open(result_path, "w")
        result_file.write(f"### kvm-userspace-version : {kvm_ver}\n")
    ...
    for category in matrix_order:
        ...
        for item in results_matrix[category]:
            if isinstance(results_matrix[category][item], dict):
                ...
                re_data = f"DATA{order_list.index(item)}"
                ...
                if tag in sum_marks:
                    sum_matrix = get_sum_result(sum_matrix, tmp_dic[tag], tag)
        if write_out_loop:
            result_file.write(f"{out_loop_line}\n")

    if sum_matrix:
        if case_type == "ffsb":
            sum_matrix["MBps_per_Hostcpu"] = (
                sum_matrix["Thro-MBps"] / sum_matrix["Hostcpu"]
            )
        ...
        for tag in sum_marks:
            headline += "%20s|" % tag
            line += f"{format_result(sum_matrix[tag])}|"

        result_file.write("{}\n".format(headline.rstrip("|")))
        result_file.write("{}\n".format(line.rstrip("|")))
    ...
    if no_table_results:
        ...
        for item in no_table_order:
            result_file.write(f"{item}: {no_table_results[item]}\n")
