...
def run(test, params, env):
    ...
    def send_data_from_guest_to_host():
        ...
        if params["os_type"] == "windows":
            ...
            cmd = f"dd if=/dev/zero of={vport_name} bs=1024 count=1"
        else:
            cmd = f"dd if=/dev/zero of=/dev/virtio-ports/{port.name} bs=1024 count=1"
        session.cmd(cmd)
        session.close()
    ...
    try:
        ...
        virtio_test.cleanup()
    vm.destroy()
    params[f"chardev_backend_{serial_id}"] = "tcp_socket"
