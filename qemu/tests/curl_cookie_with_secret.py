...
def run(test, params, env):
    ...
    def _get_tcpdump_pid(dump_file):
        cmd = f"ps -ef|grep tcpdump|grep {dump_file}|grep -v grep|awk '{{print $2}}'"
    ...
    dump_file = utils_misc.generate_tmp_file_name(f"{tag}_access_tcpdump", "out")
    ...
    try:
        ...
        with open(dump_file, "rb") as fd:
            for line in fd:
                ...
                if "Cookie: {}".format(img_params["curl_cookie_secret"]) in line:
                    test.log.info(
