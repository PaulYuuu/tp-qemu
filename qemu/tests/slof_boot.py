...
def run(test, params, env):
    ...
    def _verify_boot_status(boot_dev, content):
        ...
        if sub_child_addr:
            fail_info = f"Failed to boot from {boot_dev} device ({child_addr}@{sub_child_addr})."
            ret_info = f"Booted from {boot_dev} device ({child_addr}@{sub_child_addr}) successfully."
        else:
            fail_info = f"Failed to boot from {boot_dev} device(@{child_addr})."
            ret_info = f"Booted from {boot_dev} device(@{child_addr}) successfully."
    ...
    s, o = session.cmd_status_output(f"ping {extra_host_ip} -c 5")
    test.log.debug(o)
    if s:
