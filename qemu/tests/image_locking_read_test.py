...
def run(test, params, env):
    ...
    for image in images:
        ...
        try:
            ...
            image.start_vm({f"boot_drive_{image.tag}": "yes"})
        except virt_vm.VMCreateError:
            # add images in second chain to images so they could be deleted
            # in postprocess
            params["images"] += f" {image}"
            test.fail(f"fail to start vm from snapshot {image.image_filename}")
        else:
    ...
    error_context.context(
        f"create a temporary file: {tmpfile} in {image.image_filename}",
        test.log.info,
    )
    ...
    error_context.context(f"commit image {image.image_filename}", test.log.info)
    ...
    if not image.check_file(tmpfile, hash_val):
        test.fail(f"File {tmpfile}'s hash is different after commit")
