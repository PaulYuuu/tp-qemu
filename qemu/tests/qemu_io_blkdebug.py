...
def run(test, params, env):
    ...
    for errn in errn_list:
        log_filename = utils_misc.get_path(test.outputdir, f"qemu-io-log-{errn}")
        error_context.context("Write the blkdebug config file", test.log.info)
        template.set("inject-error", "event", f'"{err_event}"')
        template.set("inject-error", "errno", f'"{errn}"')
        ...
        if std_msg in output:
            ...
            fail_log += f"    qemu-io reports: '{output}',\n"
            fail_log += f"    os.strerror reports: '{std_msg}'"
            test.fail(fail_log)
