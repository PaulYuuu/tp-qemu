...
def run(test, params, env):
    ...
    def _start_allocator(vm, session, timeout):
        ...
        try:
            ...
            e_msg = f"Command ksm_overcommit_guest.py on vm '{vm.name}' failed: {str(details)}"
            test.fail(e_msg)
    ...
    def _execute_allocator(command, vm, session, timeout):
        ...
        try:
            ...
            e_msg = (
                f"Failed to execute command '{command}' on "
                f"ksm_overcommit_guest.py, vm '{vm.name}': {str(details)}"
            )
            test.fail(e_msg)
        return (match, data)
    ...
    def get_ksmstat():
        ...
        return int(float(utils_misc.normalize_data_size(f"{sharing_mem}K")))
    ...
    if os.path.exists("/sys/kernel/mm/ksm/run"):
        ...
        if os.path.exists(e_up):
            process.run(f"echo 'never' > {e_up}", shell=True)
        if os.path.exists(e_rh):
            process.run(f"echo 'never' > {e_rh}", shell=True)
        new_ksm = True
    else:
        try:
            ...
            test.fail(f"Failed to load KSM: {details}")
    ...
    for i in range(1, vmsc):
        ...
        if not lvms[i].is_alive():
            test.error(
                f"VM {lvms[i].name} seems to be dead; Test requires a" "living VM"
            )
