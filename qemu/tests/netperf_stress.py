...
def run(test, params, env):
    ...
    for server in netperf_server:
        ...
        if server in vms:
            ...
            s_info["ip"] = server_ip
            s_info["os_type"] = params.get(f"os_type_{server}", os_type)
            s_info["username"] = params.get(f"username_{server}", guest_username)
            s_info["password"] = params.get(f"password_{server}", guest_password)
            s_info["shell_client"] = params.get(f"shell_client_{server}", shell_client)
            s_info["shell_port"] = params.get(f"shell_port_{server}", shell_port)
            s_info["shell_prompt"] = params.get(f"shell_prompt_{server}", shell_prompt)
            s_info["linesep"] = params.get(f"linesep_{server}", linesep)
            s_info["status_test_command"] = params.get(
                f"status_test_command_{server}", status_test_command
            )
        else:
            if server == "localhost":
                s_info["ip"] = host_ip
                s_info["password"] = params.get(f"password_{server}", host_password)
            else:
                s_info["ip"] = server
                s_info["password"] = params.get(f"password_{server}", "redhat")
            s_info["os_type"] = params.get(f"os_type_{server}", "linux")
            s_info["username"] = params.get(f"username_{server}", "root")
            s_info["shell_client"] = params.get(f"shell_client_{server}", "ssh")
            s_info["shell_port"] = params.get(f"shell_port_{server}", "22")
            s_info["shell_prompt"] = params.get(
                f"shell_prompt_{server}", r"^\[.*\][\#\$]\s*$"
            )
            s_info["linesep"] = params.get(f"linesep_{server}", "\n")
            s_info["status_test_command"] = params.get(
                f"status_test_command_{server}", "echo $?"
            )
    ...
    for client in netperf_client:
        ...
        if client in vms:
            ...
            c_info["ip"] = client_ip
            c_info["os_type"] = params.get(f"os_type_{client}", os_type)
            c_info["username"] = params.get(f"username_{client}", guest_username)
            c_info["password"] = params.get(f"password_{client}", guest_password)
            c_info["shell_client"] = params.get(f"shell_client_{client}", shell_client)
            c_info["shell_port"] = params.get(f"shell_port_{client}", shell_port)
            c_info["shell_prompt"] = params.get(f"shell_prompt_{client}", shell_prompt)
            c_info["linesep"] = params.get(f"linesep_{client}", linesep)
            c_info["status_test_command"] = params.get(
                f"status_test_command_{client}", status_test_command
            )
        else:
            if client == "localhost":
                c_info["ip"] = host_ip
                c_info["password"] = params.get(f"password_{client}", host_password)
            else:
                c_info["ip"] = client
                c_info["password"] = params.get(f"password_{client}", "redhat")
            c_info["os_type"] = params.get(f"os_type_{client}", "linux")
            c_info["username"] = params.get(f"username_{client}", "root")
            c_info["shell_client"] = params.get(f"shell_client_{client}", "ssh")
            c_info["shell_port"] = params.get(f"shell_port_{client}", "23")
            c_info["shell_prompt"] = params.get(
                f"shell_prompt_{client}", r"^\[.*\][\#\$]\s*$"
            )
            c_info["linesep"] = params.get(f"linesep_{client}", "\n")
            c_info["status_test_command"] = params.get(
                f"status_test_command_{client}", "echo $?"
            )
    ...
    try:
        ...
        for protocol in test_protocols.split():
            ...
            t_option = f"{test_option} -t {protocol}"
