...
def run(test, params, env):
    ...
    def _check_meminfo(key):
        meminfo = session.cmd_output(f"grep {key} /proc/meminfo")
    ...
    if _check_meminfo("Hugepagesize") != params["expected_value"]:
        test.fail(
            "The hugepage size doesn't match, " "please check meminfo: {} ".format(
                _check_meminfo("Huge")
            )
        )
    ...
    if params.get("sub_type") == "hugepage_reset":
        ...
        error_context.context(
            f"Setup hugepage number to {origin_nr} in guest", test.log.info
        )
        ...
        if result_value != origin_nr:
            test.fail(f"Assigned nr {result_value} doesn't match expected {origin_nr}")
