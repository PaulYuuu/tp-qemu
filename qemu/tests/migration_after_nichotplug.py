...
def run(test, params, env):
    ...
    def check_nic_is_empty():
        ...
        if guest_is_not_windows:
            ...
            if status != 1:
                test.fail(f"nic is not empty with output: {output}")
        else:
            if status:
                test.error(
    ...
    if guest_is_not_windows and module:
        session_serial.cmd(f"modprobe {module}")
    if session_serial:
        session_serial.close()

    if with_unplug:
        nic_name = vm.virtnet[0].nic_name
        error_context.context(f"Hot unplug {nic_name} before migration", test.log.info)
        ...
        vm.params["nics"] += f" {nic_name}"
        vm.params[f"nic_model_{nic_name}"] = nic_info["nic_model"]
        ...
        if s != 0:
            test.fail(f"New nic failed ping test with output:\n {o}")
