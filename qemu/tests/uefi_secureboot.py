...
def run(test, params, env):
    ...
    if os_type == "linux":
        ...
        if session.cmd_status(check_pesign_cmd) != 0:
            ...
            if s != 0:
                test.cancel(
                    f'Install pesign failed with output: "{o}". '
                    "Please define proper source for guest"
                )
        vmlinuz = "/boot/vmlinuz-{}".format(session.cmd_output("uname -r"))
        check_sign_cmd = check_sign_cmd % vmlinuz
    sign_info = session.cmd_output(check_sign_cmd)
    signed = _check_signed()
    error_context.context(
        f"Guest signed status is {signed}, shutdown and reboot "
        "guest with secure boot",
        test.log.info,
    )
    session.close()
    vm.destroy()
