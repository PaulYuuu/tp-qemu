    ...
    for file_name in os.listdir(test_dirname):
        if re.search(test_file_pattern, file_name, re.I):
            ...
            error_context.context(
                f"Test check with command '{test_cmd}'.", test.log.info
            )
            ...
            if status:
                test.fail(
                    f"Failed to run '{test_cmd}', the error message is '{output}'"
                )
            error_context.context(
                f"The output of command '{test_cmd}':\n{output}", test.log.info
            )
    if "test_file" not in locals():
        test.error(f"Not found test file in '{test_dirname}', please check it.")
