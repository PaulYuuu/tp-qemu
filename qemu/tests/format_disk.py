...
def run(test, params, env):
    ...
    if os_type == "linux":
        ...
        drive_id = params[f"blk_extra_params_{drive_name}"].split("=")[1]
        ...
        if not drive_path:
            test.error(f"Failed to get '{drive_name}' drive path")
    ...
    if create_partition_cmd:
        ...
        if os_type == "windows" and has_dispart:
            ...
            for i in re.findall(r"Disk*.(\d+)\s+Offline", output):
                error_context.context(f"Set disk '{i}' to online status", test.log.info)
                set_online_cmd = params.get("set_online_cmd") % i
                status, output = session.cmd_status_output(
                    set_online_cmd, timeout=cmd_timeout
                )
                if status != 0:
                    test.fail(f"Can not set disk online {output}")
        ...
        if status != 0:
            test.fail(f"Failed to create partition with error: {output}")
    ...
    if format_cmd:
        if os_type == "linux":
            ...
            if not status:
                ...
                if status != 0:
                    test.fail(f"Failed to umount with error: {output}")
        ...
        error_context.context(f"Format the disk with cmd '{format_cmd}'", test.log.info)
        status, output = session.cmd_status_output(format_cmd, timeout=cmd_timeout)
        if status != 0:
            test.fail(f"Failed to format with error: {output}")

    mount_cmd = params.get("mount_cmd", "").format(drive_path)
    if mount_cmd:
        error_context.context(f"Mount the disk with cmd '{mount_cmd}'", test.log.info)
        ...
        if status != 0:
            ...
            test.fail(f"Failed to mount with error: {output}")
    ...
    if testfile_name:
        ...
        if status != 0:
            test.fail(f"Write to file error: {output}")
        ...
        if status != 0:
            test.fail(f"Read file error: {output}")
        if output.strip() != ranstr:
            test.fail(
                "The content written to file has changed, "
                f"from: {ranstr}, to: {output.strip()}"
            )
    ...
    if clean_partition_cmd:
        ...
        if status != 0:
            test.fail(f"Failed to clean partition with error: {output}")
