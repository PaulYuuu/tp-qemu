...
def run(test, params, env):
    ...
    if os_type == "linux":
        ...
        match_string = "mtu {}".format(params["mtu_value"])
        if match_string not in output:
            test.fail("host mtu {} not exposed to guest".format(params["mtu_value"]))
        ...
        if not int(guest_mtu_value) == mtu_value:
            test.fail(
                "Host mtu {} is not exposed to " "guest!".format(params["mtu_value"])
            )
