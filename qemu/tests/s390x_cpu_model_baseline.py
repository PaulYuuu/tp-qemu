...
def run(test, params, env):
    ...
    for i in range(len(cpu_models)):
        ...
        for j in range(i):
            ...
            test.log.debug(f"Test with args: {args}")
            ...
            if obtained_model != expected_model:
                msg = (
                    ...
                    " was chosen:"
                    f" {obtained_model} instead of expected {expected_model}."
                    f" Input model names: {older_model} and {newer_model}"
                )
            ...
            if not_found_expected or found_not_expected:
                msg = (
                    f"Expected to get intersection of props '{props1}'"
                    f" and '{props2}': '{expected_props}';"
                    f" but got '{props}'"
                )
                test_failures.append(msg)
    if test_failures:
        test.fail(
            "Some baselines didn't return as expected." f" Details: {test_failures}"
        )
