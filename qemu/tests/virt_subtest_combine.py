...
def run(test, params, env):
        ...
        for vm in vms:
            ...
            if params_vm_subtest.get("cmd"):
                error_context.context(
                    f"Try to log into guest '{vm.name}'.", test.log.info
                )
    ...
    for subtest in subtests:
        ...
        error_context.context(f"Run test {subtest}", test.log.info)
        ...
        if params_subtest.get("check_vm_status_after_test", "yes") == "yes":
            vms = env.get_all_vms()
            for vm in vms:
                error_context.context(f"Check {vm.name} status", test.log.info)
