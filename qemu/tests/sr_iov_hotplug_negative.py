...
def run(test, params, env):
    ...
    def make_pci_add_cmd(pa_pci_id, pci_addr="auto"):
        pci_add_cmd = (
            f"pci_add pci_addr={pci_addr} host host={pa_pci_id},if={pci_model}"
        )
    ...
    def make_device_add_cmd(pa_pci_id, pci_addr=None):
        ...
        pci_add_cmd = f"device_add id={device_id},driver=pci-assign,host={pa_pci_id}"
        if pci_addr is not None:
            pci_add_cmd += f",addr={pci_addr}"
        if params.get("hotplug_params"):
            assign_param = params.get("hotplug_params").split()
            for param in assign_param:
                value = params.get(param)
    ...
    for j in range(rp_times):
        ...
        try:
            msg = f"Adding pci device with command '{pci_add_cmd}'"  # pylint: disable=E0606
            ...
            if neg_msg:
                ...
                if neg_msg not in str(err):
                    msg = f"Could not find '{neg_msg}' in"
                    msg += f" command output '{add_output}'"
                    test.fail(msg)
