...
def run(test, params, env):
    ...
    def verify_elapsed_time():
        ...
        get_time_cmd = f"for (( i=0; i<{guest_cpu}; i+=1 ));"
        ...
        if times_list:
            test.fail(
                f"Unexpected time drift found: Detail: '{output}' \n timeslist: {times_list}"
            )
    ...
    if "kvm-clock" not in session.cmd(cmd):
        ...
        if "clocksource=" not in session.cmd(f"cat {grub_file}"):
        ...
        try:
            ...
            test.error(
                ...
                f" pattern is '{kernel_cfg_pattern}', detail: '{detail}'"
            )
