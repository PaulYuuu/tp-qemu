...
def run(test, params, env):
    ...
    def _create_image():
        ...
        if result.exit_status != 0:
            test.fail(
                f"Failed to clone the system image, error: {result.stderr.decode()}"
            )
    ...
    def _start_vm_without_image():
        ...
        try:
            ...
            # let VT remove it
            params["images"] = " {}".format(params["local_image_tag"])
        return vm

    def _make_ncat_cmd():
        ncat = ""
        if params.get("nbd_unix_socket_{}".format(params["nbd_image_tag"])):
