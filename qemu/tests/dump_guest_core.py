...
def run(test, params, env):
    ...
    def check_env():
        ...
        if host_kernel_version != guest_kernel_version:
            test.cancel(
                ...
                f"to same version.The host kernel version is {host_kernel_version}"
                f"The guest kernel version is {guest_kernel_version}"
            )
    ...
    def check_core_file(arch):
        ...
        command = (
            f'echo -e "source {dump_guest_memory_file}\nset height 0\ndump-guest-memory'
            f' {vmcore_file} {arch}\nbt\nquit" > {gdb_command_file}'
        )
    ...
    def check_vmcore_file():
        ...
        """
        process.run(f'echo -e "bt\ntask 0\ntask 1\nquit" > {crash_script}', shell=True)
