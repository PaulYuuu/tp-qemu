    ...
    try:
        ...
        if prepare_op:
            error_context.context(
                f"Do preparation operation: '{prepare_op}'", test.log.info
            )
        ...
        if start_bg_process:
            ...
            error_context.context(
                f"Start a background process: '{bg_cmd}'", test.log.info
            )
        ...
        if check_op:
            error_context.context(f"Do check operation: '{check_op}'", test.log.info)
            op_timeout = float(params.get("check_op_timeout", 60))
            s, o = session.cmd_status_output(check_op, timeout=op_timeout)
            if s != 0:
                test.fail(
                    "Something wrong after stop continue, " f"check command report: {o}"
                )
    finally:
        try:
            ...
            if clean_op:
                error_context.context(
                    f"Do clean operation: '{clean_op}'", test.log.info
                )
            ...
            test.log.warn("Exception occur when clean test environment: %s", details)
