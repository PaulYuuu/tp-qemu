...
def process_lspci(lspci):
    ...
    return [
        {"class_addr": info[0], "class_pciid": f"{info[1]}:{info[2]}"} for info in lspci
    ]
...
def verify_qdev_vs_qtree(qdev_info, qtree_info):
    ...
    for dev_id, device in qtree_devices.items():
        if dev_id not in qdev_devices:
            errors += f"Device {dev_id} is in qtree but not in qdev.\n"
            continue
        for key, value in device.items():
            err = ""
            if qdev_devices[dev_id][key] != value:
                err += f"  {qdev_devices[dev_id][key]} != {value}\n"
            if err:
                errors += f"Device {dev_id} properties mismatch:\n{err}"

    for dev_id in qdev_devices:
        if dev_id not in qtree_devices:
            errors += f"Device {dev_id} is in qdev but not in qtree\n"
    ...
    for device in qtree_devices_noid:
        for qdev_device in qdev_devices_noid:
            ...
            errors += f"No match in qdev for device without id {device}\n"
    for device in qdev_devices_noid:
        errors += f"No match in qtree for device without id {device}\n"
...
def verify_lspci(info_lspci, info_qtree):
    ...
    for lspci_dev in info_lspci:
        if lspci_dev not in info_qtree:
            errors += f"Device {lspci_dev} is in lspci but not in qtree\n"

    for qtree_dev in info_qtree:
        if qtree_dev not in info_lspci:
            errors += f"Device {qtree_dev} is in qtree but not in lspci\n"
...
def add_bus(qdev, params, bus_type, name, parent_bus):
    ...
    if bus_type == "bridge":
        ...
        bus_type = "ioh3420"
    params["pci_controllers"] += f" {name}"
    params[f"type_{name}"] = bus_type
    params[f"pci_bus_{name}"] = parent_bus.name
...
def add_device_usb(params, name_idxs, parent_bus, addr, device):
    ...
    params["usbs"] += f" {name}"
    params[f"pci_bus_{name}"] = parent_bus
    params[f"pci_addr_{name}"] = addr
    params[f"usb_type_{name}"] = device[1]
    if not params.get(f"reserved_slots_{parent_bus}"):
        params[f"reserved_slots_{parent_bus}"] = ""
    params[f"reserved_slots_{parent_bus}"] += f" 0x{addr:x}-0x0"
...
def add_virtio_disk(params, name_idxs, parent_bus, addr):
    ...
    params["images"] += f" {name}"
    params[f"image_name_{name}"] = f"images/{name}"
    params[f"pci_bus_{name}"] = parent_bus
    params[f"drive_bus_{name}"] = addr
    params[f"drive_format_{name}"] = "virtio"
    params[f"force_create_image_{name}"] = "yes"
    params[f"remove_image_{name}"] = "yes"
    params[f"image_size_{name}"] = "1M"
    if not params.get(f"reserved_slots_{parent_bus}"):
        params[f"reserved_slots_{parent_bus}"] = ""
    params[f"reserved_slots_{parent_bus}"] += f" 0x{addr:x}-0x0"
...
def run(test, params, env):
    ...
    if errors:
        test.fail(
            f"Errors occurred while comparing {errors[:-2]}. Please check"
            " the log for details."
        )
