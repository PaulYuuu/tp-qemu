    ...
    def check_filter_nodes_name(self, during_stream=True):
        ...
        if during_stream:
            for block in blocks_info:
                ...
                self.test.fail(
                    "Filter node name '{}' is not set as expected"
                    "during stream".format(self.params["filter_node_name"])
                )
        else:
            for block in blocks_info:
                block_node_name = block["inserted"].get("node-name")
                ...
                self.test.fail(
                    "Filter node name '{}' set after stream".format(
