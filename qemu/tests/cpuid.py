...
def run(test, params, env):
    ...
    def test_qemu_cpu_models_list(self):
        ...
        if missing:
            test.fail(f"Some CPU models not in QEMU CPU model list: {missing!r}")
    ...
    def default_vendor(self):
        ...
        for cpu_model in cpu_models:
            ...
            if guest_vendor != vendor:
                test.fail(
                    f"Guest vendor [{guest_vendor}], doesn't match "
                    f"required vendor [{vendor}] for CPU [{cpu_model}]"
                )
    ...
    def custom_vendor(self):
        ...
        try:
            ...
            if guest_vendor0 != vendor:
                test.fail(
                    f"Guest vendor[0] [{guest_vendor0}], doesn't match "
                    f"required vendor [{vendor}] for CPU [{cpu_model}]"
                )
            if guest_vendor80000000 != vendor:
                test.fail(
                    f"Guest vendor[0x80000000] [{guest_vendor80000000}], "
                    "doesn't match required vendor "
                    f"[{vendor}] for CPU [{cpu_model}]"
                )
        except:
            has_error = True
            if xfail is False:
    ...
    def custom_level(self):
        ...
        try:
            ...
            if guest_level != level:
                test.fail(
                    f"Guest's level [{guest_level}], doesn't match "
    ...
    def custom_family(self):
        ...
        try:
            ...
            if guest_family != family:
                test.fail(
                    f"Guest's family [{guest_family}], doesn't match "
    ...
    def custom_model(self):
        ...
        try:
            ...
            if guest_model != model:
                test.fail(
                    f"Guest's model [{guest_model}], doesn't match "
    ...
    def custom_stepping(self):
        ...
        try:
            ...
            if guest_stepping != stepping:
                test.fail(
                    f"Guest's stepping [{guest_stepping}], doesn't match "
                    f"required stepping [{stepping}]"
                )
        except:
            has_error = True
            if xfail is False:
    ...
    def custom_xlevel(self):
        ...
        try:
            ...
            if guest_xlevel != xlevel:
                test.fail(
                    f"Guest's xlevel [{guest_xlevel}], doesn't match "
                    f"required xlevel [{xlevel}]"
                )
        except:
            has_error = True
            if xfail is False:
    ...
    def custom_model_id(self):
        ...
        try:
            out = get_guest_cpuid(self, cpu_model, f"model_id='{model_id}'")
            guest_model_id = cpuid_to_model_id(out)
            if guest_model_id != model_id:
                test.fail(
                    f"Guest's model_id [{guest_model_id}], doesn't match "
                    f"required model_id [{model_id}]"
                )
        except:
            has_error = True
            if xfail is False:
        ...
        for i in regs:
            for shift in range(0, 4):
                ...
                if isprintable(c):
                    ...
                    signature = f"{signature}\\x{ord(c):02x}"
    ...
    def cpuid_signature(self):
        ...
        try:
            ...
            if _signature != signature:
                test.fail(
                    f"Guest's signature [{_signature}], doesn't"
                    f"match required signature [{signature}]"
                )
        except:
            has_error = True
            if xfail is False:
    ...
    def cpuid_bit_test(self):
        ...
        try:
            ...
            for i in bits:
                if (r & (1 << int(i))) == 0:
                    test.fail(f"CPUID({leaf}.{idx}).{reg}[{i}] is not set")
        except:
            has_error = True
            if xfail is False:
                raise
    ...
    def cpuid_reg_test(self):
        ...
        try:
            ...
            test.log.debug("CPUID(%s.%s).%s=0x%08x", leaf, idx, reg, r)
            if r != val:
                test.fail(f"CPUID({leaf}.{idx}).{reg} is not 0x{val:08x}")
        except:
            has_error = True
            if xfail is False:
                raise
    ...
    def check_cpuid_dump(self):
        ...
        ref_file = os.path.join(
            ...
            machine_type,
            f"{full_cpu_model_name}-dump.txt",
        )
        if not os.path.exists(ref_file):
            test.cancel(f"no cpuid dump file: {ref_file}")
        reference = open(ref_file).read()
        if not reference:
            test.cancel(f"no cpuid dump data on file: {ref_file}")
        reference = parse_cpuid_dump(reference)
        if reference is None:
            test.cancel(f"couldn't parse reference cpuid dump from file; {ref_file}")
        ...
        try:
            ...
            if (
                ...
                test.cancel(f"Can't run CPU model {full_cpu_model_name} on this host")
            else:
                raise
    ...
    if test_type not in locals():
        test.error(f"Test function '{test_type}' is not defined in test")
