...
class BlockCommit(block_copy.BlockCopy):
    def start(self):
        ...
        self.test.log.info(
            f"block commit job running, with limited speed {default_speed} B/s"
        )
    ...
    def create_snapshots(self):
        ...
        for snapshot in snapshots:
            ...
            self.test.log.info(f"snapshot {snapshot}, base {image_file}")
            device = self.vm.live_snapshot(image_file, snapshot, image_format)
            if device != self.device:
                image_file = self.get_image_file()
                self.test.log.info(
                    f"expect file: {snapshot}, opening file: {image_file}"
                )
                self.test.fail(f"create snapshot '{snapshot}' failed")
