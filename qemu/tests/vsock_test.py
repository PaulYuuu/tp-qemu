...
def compile_nc_vsock(test, vm, session):
    ...
    rm_cmd = f"rm -rf {bin_path}*"
    session.cmd(rm_cmd)
    process.system(rm_cmd, shell=True, ignore_status=True)
    cmd_cp = f"cp {src_file} {nc_vsock_dir}"
    process.system(cmd_cp)
    vm.copy_files_to(src_file, nc_vsock_dir)
    compile_cmd = f"cd {nc_vsock_dir} && gcc -o {nc_vsock_bin} {nc_vsock_c}"
...
def vsock_listen(tool_bin, port, session):
    ...
    lstn_cmd = None
    if "ncat" in tool_bin:
        lstn_cmd = f"{tool_bin} --vsock -l {port}"

    if "nc-vsock" in tool_bin:
        lstn_cmd = f"{tool_bin} -l {port}"
...
def check_received_data(test, session, pattern):
    ...
    try:
        ...
        if isinstance(e, aexpect.ExpectTimeoutError):
            test.fail(
                f"Does not receive expected content: {pattern}, output"
                f" of session: {e.output}"
            )
        else:
            test.fail(str(e))
...
def vsock_connect(tool_bin, guest_cid, port):
    ...
    conn_cmd = None
    if "ncat" in tool_bin:
        conn_cmd = f"{tool_bin} --vsock {guest_cid} {port}"
    if "nc-vsock" in tool_bin:
        conn_cmd = f"{tool_bin} {guest_cid} {port}"
    if conn_cmd is None:
        raise ValueError(f"unexpected test tool: {tool_bin}")
    ...
    return aexpect.Expect(
        ...
        output_params=(f"vsock_{guest_cid}_{port}",),
    )
...
def send_data_from_guest_to_host(
    ...
    cmd_generate = f"dd if=/dev/urandom of={tmp_file} count={file_size} bs=1M"
    guest_session.cmd_status(cmd_generate, timeout=600)
    port = random.randrange(1, 6000)
    cmd_transfer = None
    if "ncat" in tool_bin:
        cmd_transfer = f"{tool_bin} --vsock --send-only -l {port} < {tmp_file}"
    if "nc-vsock" in tool_bin:
        cmd_transfer = f"{tool_bin} -l {port} < {tmp_file}"
    if cmd_transfer is None:
        raise ValueError(f"unexpected test tool: {tool_bin}")
    error_context.context(
        f"Transfer file from guest via command: {cmd_transfer}", LOG_JOB.info
    )
    guest_session.sendline(cmd_transfer)
    cmd_receive = None
    if "ncat" in tool_bin:
        cmd_receive = f"{tool_bin} --vsock {guest_cid} {port} > {tmp_file}"
    if "nc-vsock" in tool_bin:
        cmd_receive = f"{tool_bin} {guest_cid} {port} > {tmp_file}"
...
def run(test, params, env):
    ...
    def clean(tmp_file):
        ...
        cmd_rm = f"rm -rf {tmp_file}"
        if vsock_test_tool == "nc_vsock":
            cmd_rm += f"; rm -rf {tool_bin}*"
    ...
    tmp_file = f"/tmp/vsock_file_{utils_misc.generate_random_string(6)}"
    ...
    if md5_received != md5_origin:
        ...
        test.fail(
            ...
            f" is {md5_origin}, while the md5 value received on host is {md5_received}"
        )
    clean(tmp_file)
