...
def run(test, params, env):
    ...
    def _get_qemu_vmrss():
        ...
        used_mem_cmd = (
            f"cat {qemu_stat_file} | grep VmRSS | awk -F ':\t* *' '{{print $2}}'"
        )
    ...
    test.log.info(
        f"The qemu-kvm process used mem size before querying blocks is {used_mem_size_before}"
    )
    ...
    test.log.info(
        f"The qemu-kvm process used mem size after querying blocks is {used_mem_size_after}"
    )
    test.log.info("Check whether the used memory size is increased.")
    if int(used_mem_size_after) > int(used_mem_size_before) * 1.2:
        test.fail(
            f"The used memory size before is {used_mem_size_before}, but after checking the blocks "
            f"for 1 hour, it increased to {used_mem_size_after}. There should be memory leaks, "
            "check please."
        )
