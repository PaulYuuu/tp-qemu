...
def add_virtserial_device(vm, params, serial_id, chardev_id):
    ...
    bus_type = f"virtio-serial-{controller_suffix}"
...
def run(test, params, env):
    ...
    def test_multi_open():
        ...
        guest_worker.cmd(f"virt.close('{port.name}')", 10)
        guest_worker.cmd(f"virt.open('{port.name}')", 10)
        (match, data) = guest_worker._cmd(f"virt.open('{port.name}')", 10)
    ...
    def test_polling():
        ...
        # Poll (OUT)
        port.open()
        guest_worker.cmd(f"virt.poll('{port.name}', {select.POLLOUT})", 2)
        ...
        # Poll (HUP)
        guest_worker.cmd(f"virt.recv('{port.name}', 4, 1024, False)", 10)
        guest_worker.cmd(f"virt.poll('{port.name}', {select.POLLHUP})", 2)
    ...
    def test_sigio():
        ...
        guest_worker.cmd(f"virt.asynchronous('{port.name}', True, 0)", 10)

        # Test sigio when port open
        guest_worker.cmd(
            f"virt.set_pool_want_return('{port.name}', select.POLLOUT)", 10
        )
        port.open()
        match, data = guest_worker._cmd(
            f"virt.get_sigio_poll_return('{port.name}')", 10
        )
        if match == 1:
            test.fail(f"Problem with HUP on console port:\n{data}")

        # Test sigio when port receive data
        guest_worker.cmd(
            f"virt.set_pool_want_return('{port.name}', select.POLLOUT |"
            " select.POLLIN)",
            10,
        )
        port.sock.sendall(b"0123456789")
        guest_worker.cmd(f"virt.get_sigio_poll_return('{port.name}')", 10)

        # Test sigio port close event
        guest_worker.cmd(
            f"virt.set_pool_want_return('{port.name}', select.POLLHUP |"
            " select.POLLIN)",
            10,
        )
        port.close()
        ...
        guest_worker.cmd(f"virt.clean_port('{port.name}')", 10)
        port.close()
        guest_worker.cmd(
            f"virt.set_pool_want_return('{port.name}', select.POLLOUT)", 10
        )
        ...
        guest_worker.cmd(f"virt.asynchronous('{port.name}', False, 0)", 10)
        virtio_test.cleanup(vm, guest_worker)
    ...
    def test_lseek():
        ...
        guest_worker.cmd(f"virt.lseek('{port.name}', 0, 0)", 10)
        virtio_test.cleanup(vm, guest_worker)
    ...
    def test_rw_host_offline():
        ...
        guest_worker.cmd(f"virt.recv('{port.name}', 0, 1024, False)", 10)
        match, tmp = guest_worker._cmd(f"virt.send('{port.name}', 10, True)", 10)
    ...
    def test_rw_host_offline_big_data():
        ...
        port.clean_port()
        port.close()
        guest_worker.cmd(f"virt.clean_port('{port.name}'),1024", 10)
        match, tmp = guest_worker._cmd(
            f"virt.send('{port.name}', (1024**3)*3, True, " "is_static=True)", 30
        )
        if match is not None:
            test.fail(
                "Write on guest while host disconnected "
    ...
    def test_rw_blocking_mode():
        ...
        port.open()
        guest_worker.cmd(f"virt.blocking('{port.name}', True)", 10)
        # Recv should timed out
        match, tmp = guest_worker._cmd(f"virt.recv('{port.name}', 10, 1024, False)", 10)
        if match == 0:
            test.fail("Received data even when none was sent\n" f"Data:\n{tmp}")
        elif match is not None:
            test.fail(f"Unexpected fail\nMatch: {match}\nData:\n{tmp}")
    ...
    def test_rw_nonblocking_mode():
        ...
        port.open()
        guest_worker.cmd(f"virt.blocking('{port.name}', False)", 10)
        ...
        time.sleep(0.01)
        try:
            guest_worker.cmd(f"virt.recv('{port.name}', 10, 1024, False)", 10)
    ...
    def test_basic_loopback():
        ...
        guest_worker.cmd(
            f"virt.loopback(['{send_port.name}'], ['{recv_port.name}'], 1024, virt.LOOP_NONE)",
            10,
        )
        ...
        while i <= 10:
            ...
            if len(tmp) >= len(data):
                break
        if tmp != data:
            test.fail(f"Incorrect data: '{data}' != '{tmp}'")
    ...
    def test_loopback():
        ...
        for param in test_params.split(";"):
            if not param:
                continue
            error_context.context(f"test_loopback: params {param}", test.log.info)
            # Prepare
            param = param.split(":")
            idx_serialport = 0
            idx_console = 0
    ...
    def test_interrupted_transfer():
        ...
        def _replug_loop():
            ...
            def _port_unplug(port_idx):
                ...
                if not portdev:
                    test.error(f"No port named {dev.qemu_id}")
                port_property = dict(
        ...
        def _disconnect():
            ...
            for i in _host:
                ...
                ports[i].close()
            for i in _guest:
                guest_worker.cmd(f"virt.close('{ports[i].name}')", 10)
            time.sleep(intr_time)
            for i in _host:
        ...
        def _port_replug(device, port_idx):
            ...
            if not portdev:
                test.error(f"No port named {port.qemu_id}")
            chardev = portdev.get_param("chardev")
            out, ver_out = vm.devices.simple_unplug(portdev, vm.monitor)
            if not ver_out:
                test.error(
                    f"The device {port.qemu_id} isn't hotplugged well, "
                    f"result: {out}"
                )
            time.sleep(intr_time)
            if not chardev:
                test.error(f"No chardev in guest for port {port.qemu_id}")
        ...
        if interruption == "stop":
            ...
            test.cancel(
                f"virtio_console_interruption = '{interruption}' " "is unknown."
            )
        ...
        guest_worker.cmd(
            f"virt.loopback(['{send_pt.name}'], ['{recv_pt.name}'], {buflen}, virt.LOOP_"
            "RECONNECT_NONE)",
            10,
        )
        ...
        try:
            for i in range(no_repeats):
                error_context.context(f"Interruption nr. {i}")
                threads[1].sendidx = acceptable_loss
                interruption()
                count = threads[1].idx
    ...
    def test_perf():
        ...
        for param in test_params.split(";"):
            if not param:
                continue
            error_context.context(f"test_perf: params {param}", test.log.info)
            EXIT_EVENT.clear()
            # Prepare
            param = param.split(":")
            duration = test_time
            ...
            try:
                ...
                for _ in range(60):
                    if guest_worker._cmd(
                        f"virt.poll('{port.name}', {select.POLLIN})", 10
                    )[0]:
                        break
                    time.sleep(1)
                else:
    ...
    def _tmigrate(use_serialport, no_ports, no_migrations, blocklen, offline):
        ...
        for j in range(no_migrations):
            error_context.context(f"Performing migration number {j}/{no_migrations}")
        ...
        if err:
            msg = f"test_migrate: error occurred in threads: {err[:-2]}."
            test.log.error(msg)
            test.fail(msg)
    ...
    def _virtio_dev_add(vm, pci_id, port_id, console="no"):
        ...
        if not ver_out:
            test.error(f"The virtioserialport isn't hotplugged well, result: {result}")
    ...
    def test_hotplug():
        ...
        for bus_id in range(1, 5):  # count of pci device
            ...
            if not ver_out:
                test.error(
                    f"The virtio serial pci isn't hotplugged well, log: {result}"
                )
        ...
        guest_worker.cmd(
            f"virt.loopback(['{consoles[1][0].name}'], ['{consoles[1][1].name}'], 1024,"
            "virt.LOOP_POLL)",
            10,
        )
        funcatexit.register(env, params.get("type"), __set_exit_event)
        ...
        if not (ver_out and ver_out_):
            test.error(
                f"The ports aren't hotunplugged well, log: {result}\n, {result_}"
            )
    ...
    def test_hotplug_virtio_pci():
        ...
        for i in range(int(params.get("virtio_console_loops", 10))):
            ...
            if not ver_out:
                test.error(
                    f"The device {new_dev.aid} isn't hotplugged well, " f"result: {out}"
                )
            time.sleep(pause)
            # Unplug
            out, ver_out = vm.devices.simple_unplug(new_dev, vm.monitor)
            if ver_out is False:
                test.fail(f"Device not unplugged. Iteration: {i}, result: {out}")
    ...
    def test_rmmod():
        ...
        guest_worker = qemu_virtio_port.GuestWorker(vm)
        guest_worker.cmd(f"virt.clean_port('{port.name}'),1024", 2)
        virtio_test.cleanup(vm, guest_worker)
    ...
    def test_stressed_restart():
        ...
        process = []
        for port in ports[1:]:
            guest_worker.cmd(f"virt.close('{port.name}')", 2)
            guest_worker.cmd(f"virt.open('{port.name}')", 2)
            try:
                process.append(
                    Popen(
                        f"dd if=/dev/random of='{port.path}' bs=4096 " "&>/dev/null &"
                    )
                )
            except Exception:
                pass
        ...
        guest_worker._cmd(
            f"virt.send('{ports[0].name}', 1024**3, True, is_static=True)", 1
        )
    ...
    error_context.context(f"Executing test: {_fce}", test.log.info)
    if _fce not in locals():
        test.cancel(
            f"Test {_fce} doesn't exist. Check 'virtio_console_"
            "test' variable in subtest.cfg"
        )
    else:
        try:
            fce = locals()[_fce]
