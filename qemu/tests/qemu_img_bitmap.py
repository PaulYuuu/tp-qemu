...
def run(test, params, env):
    ...
    def _qemu_io(img, cmd):
        ...
        try:
            ...
            test.fail(f"qemu-io to '{img.image_filename}' failed: {str(err)}.")
    ...
    def _map_nbd_bitmap(nbd_server, nbd_port, bitmap_name):
        ...
        qemu_map_cmd = f"qemu-img map --output=json {map_from}"
    ...
    def _check_bitmap_add(img, bitmap_name):
        ...
        add_bitmap_info = [
            {"flags": ["auto"], "name": f"{bitmap_name}", "granularity": 65536}
        ]
        info_output = json.loads(img.info(output="json"))
        if "bitmaps" not in info_output["format-specific"]["data"]:
            test.fail(f"Add bitmap failed, and image info is '{info_output}'")
    ...
    if info_output["format-specific"]["data"]["bitmaps"][0]["flags"]:
        test.fail(f"Disable bitmap failed, and image info is '{info_output}'")
    ...
    if "auto" not in info_output["format-specific"]["data"]["bitmaps"][0]["flags"]:
        test.fail(f"Enable bitmap failed, and image info is '{info_output}'")
    ...
    try:
        ...
        if match_info not in res:
            test.fail(f"The dumped info is not correct, and the info is {res}")
    finally:
    ...
    try:
        ...
        if match_info in res:
            test.fail(f"Clear the bitmap data failed, and the dumped info is {res}")
    finally:
        nbd_export.stop_export()
    ...
    if "bitmaps" in info_output["format-specific"]["data"]:
        test.fail(f"Remove bitmap failed, and image info is '{info_output}'")
