...
def run(test, params, env):
    ...
    def get_win_drive_letters_after_reboot():
        ...
        if diff_num != 0:
            test.error(
                "No found the corresponding drive letters " f"in {diff_num} disks."
            )
    ...
    def _check_vm_status():
        ...
        if not getattr(vm, f"wait_for_{action}")(timeout=362):
    ...
    if not utils_misc.wait_for(
        ...
        test.error(f"The {stress_name} stress is not alive.")
    time.sleep(sleep_time)
    if not is_stress_alive(session, stress_name):
        test.error(f"The {stress_name} stress is not alive after {sleep_time}.")
