...
def run(test, params, env):
    ...
    if vsock_test_tool == "ncat":
        ...
        cmd_receive = f"{tool_bin} --vsock {host_cid} {vsock_port} > {tmp_file} &"

    if vsock_test_tool == "nc_vsock":
        tool_bin = compile_nc_vsock_guest(test, vm, session)
        cmd_receive = f"{tool_bin} {host_cid} {vsock_port} > {tmp_file} &"
    ...
    try:
        ...
        session.cmd_output_safe(f"rm -rf {tmp_file}")
        if vsock_test_tool == "nc_vsock":
            session.cmd_output_safe(f"rm -rf {tool_bin}*")
