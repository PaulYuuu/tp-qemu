...
def run(test, params, env):
    ...
    try:
        ...
        msg += f"Output from check System ID command: {output}"
        test.fail(msg)
    test.log.info("VM guest System ID is: %s", sid)
    sids[sid] = [f"pre_{vm.name}"]
    file_dir = tmp_path + unattended_file
    sysprep_cmd = sysprep_cmd % file_dir
    error_context.context(f"Run sysprep command in guest. {sysprep_cmd}", test.log.info)
    ...
    for vm_i in vms:
        ...
        try:
            ...
            msg = f"Fail to get System ID of {vm_i.name}"
            msg += f"Output from check System ID command: {output}"
            test.error(msg)
    ...
    if sid_same:
        msg = ""
        for sid in sid_same:
            msg += "VM(s): {} have duplicate System ID: {}\n".format(
                " ".join(sids[sid]),
                sid,
            )
        test.fail(msg)
