...
def run(test, params, env):
    ...
    check_pid_command = f"pgrep -f {host_script}"
    ...
    if module and check_module:
        error_context.context(f"Load module {module}", test.log.info)
    ...
    for port in params.objects("serials"):
        ...
        for repeat in range(params.get_numeric("repeat_times", 1)):
            repeat += 1
            if module and check_module:
                error_context.context(f"Unload module {module}", test.log.info)
                session.cmd(f"modprobe -r {module}")
                time.sleep(1)
            ...
            if port_params.get("unplug_chardev") == "yes":
                error_context.context(
                    f"Unplug chardev '{port}' for virtio port '{chardev_qid}'",
                    test.log.info,
                )
        ...
        if difference:
            ...
            result = process.system(f"kill -9 {difference.pop()}", shell=True)
            if result != 0:
