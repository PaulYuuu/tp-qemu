...
def run(test, params, env):
    """
    Bitmaps merge to a non-enough space source.
    Steps:
        1) Create lvm
        2) Create base image on lvm
        3) Add 8 bitmaps to base
        4) Create a snapshot image
        5) Add a bitmap:bitmap_good to snapshot image
        6) Fullwrite snapshot image
        7) Commit snapshot to base,commit successfully
        8) Add a bitmap:bitmap_good to base
        9) Merge snapshot's bitmap to base image, bitmap
           merge will fail with error.
    ...
    try:
        ...
        try:
            ...
            if not all(msg in err_msg for msg in err_msg_cfg):
                test.fail("Not all expected msg are caught here")
        else:
            test.fail("Bitmaps merged with success unexpectedly")
