...
def run(test, params, env):
    ...
    def check_socket_group():
        ...
        if group_name != socket_group:
            test.fail(
                f"Socket-group name is not correct.\nIt should be {socket_group},but"
                f" the output is {group_name}"
            )
    ...
    def viofs_svc_create(cmd):
        ...
        if sc_create_s != 0:
            test.fail(f"Failed to register virtiofs service, output is {sc_create_o}")
    ...
    def viofs_svc_stop_start(action, cmd, expect_status):
        ...
        """
        error_context.context(f"Try to {action} VirtioFsSvc service.", test.log.info)
        session.cmd(cmd)
        output = session.cmd_output(viofs_sc_query_cmd)  # pylint: disable=E0606
        if expect_status not in output:
            test.fail(f"Could not {action} VirtioFsSvc service, " f"detail: '{output}'")
    ...
    def start_multifs_instance():
        ...
        error_context.context(
            f"MultiFS-{fs}: Start virtiofs instance with"
            f" tag {fs_target} to {fs_volume_label}.",
            test.log.info,
        )
        ...
        if re.search("KO.*error", output, re.I):
            test.fail(
                f"MultiFS-{fs}: Start virtiofs instance failed, " f"output is {output}."
            )
    ...
    if security_label_test:
        ...
        error_context.context(
            f"Set selinux to {se_mode} status on host before"
            " starting virtiofsd and vm.",
            test.log.info,
        )
        ...
        if se_mode_host_before.lower() != se_mode:
            try:
                ...
                test.cancel("Setting selinux failed on host with" f" {str(err_msg)}.")
    try:
        vm = None
        if (
            cmd_xfstest
        ...
        for fs in params.objects("filesystems"):
            ...
            if os_type == "linux":
                ...
                if not cmd_xfstest:
                    error_context.context(
                        f"Mount virtiofs target {fs_target} to {fs_dest} inside"
                        " guest.",
                        test.log.info,
                    )
            ...
            try:
                if cmd_dd:
                    error_context.context(
                        f"Creating file under {fs_dest} inside " "guest.",
                        test.log.info,
                    )
                ...
                if folder_test == "yes":
                    error_context.context(
                        f"Folder test under {fs_dest} inside " "guest.", test.log.info
                    )
                ...
                if case_insensitive_test:
                    ...
                    def file_check(cmd):
                        ...
                        if s:
                            test.fail("Case insensitive failed," f" the output is {o}")
                    ...
                    session.cmd(f"echo hello > {guest_file}", io_timeout)
                    ...
                    cmd_check_file = f"dir {guest_file_full_path}"
                    ...
                    process.system(f"touch {host_data}", io_timeout)
                    time.sleep(2)
                    error_context.context(
                ...
                if cmd_symblic_file:
                    error_context.context(
                        f"Symbolic test under {fs_dest} inside " "guest.",
                        test.log.info,
                    )
                    ...
                    if os_type == "linux":
                        ...
                        content = session.cmd_output(f"type {test_file}").strip()
                        ...
                        if link_guest != content or link_host != content:
                            test.fail(
                                ...
                                f" the real content is {content}\n"
                                f"the link file content in guest is {link_guest}\n"
                                f"the link file content in host is {link_host}."
                            )
                ...
                if fio_options:
                    error_context.context(f"Run fio on {fs_dest}.", test.log.info)
                ...
                if iozone_options:
                    error_context.context(
                        f"Run iozone test on {fs_dest}.", test.log.info
                    )
                ...
                if cmd_pjdfstest:
                    error_context.context(f"Run pjdfstest on {fs_dest}.", test.log.info)
                    ...
                    for case in cases_in_output.keys():
                        for ig_case in ignore_cases:
                            if ig_case in case:
                                error_context.context(
                                    f"Warn: {ig_case} was failed!", test.log.debug
                                )
                ...
                if cmd_get_stdev:
                    ...
                    cmd_touch_file = (
                        f"touch {file_in_local_host} && touch {file_in_nfs_host}"
                    )
                ...
                if git_init_cmd:
                    if os_type == "windows":
                        ...
                        if (
                            ...
                            if status:
                                test.error(
                                    f"Failed to install git, status={status}, output={output}"
                                )
                    ...
                    error_context.context(f"Git init test in {fs_dest}", test.log.info)
                    status, output = session.cmd_status_output(git_init_cmd % fs_dest)
                    if status:
                        test.fail(f"Git init failed with {output}")
                ...
                if getfattr_cmd:
                    ...
                    def check_attribute(object, xattr, get_type=True, side="guest"):
                        ...
                        full_pattern = re.compile(rf"{format}")
                        if side == "host":
                            xattr_content = (
                        ...
                        if not result:  # pylint: disable=E0606
                            test.fail(
                                f"Attribute is not correct, the pattern is {full_pattern}\n"
                                f" the attribute is {xattr_content}."
                            )
                        if get_type:
                    ...
                    def check_security_label(file, folder, xattr_name):
                        ...
                        if not context_type_file == context_type_folder:
                            test.fail(
                                ...
                                f"Shared folder context type is {context_type_folder}"
                            )
                    ...
                    session.cmd(f"touch {file_new_in_guest}")
                    time.sleep(1)
                    ...
                    process.run(f"touch {file_new_in_host}", timeout=60)
                    time.sleep(1)
                    ...
                    if s:
                        test.fail(
                            ...
                            f" on virtiofs fs, the output is {o}"
                        )
                ...
                if winfsp_test_cmd:
                    ...
                    try:
                        ...
                        if status != 0:
                            test.fail(f"Winfsp-test failed, the output is {output}")
                    finally:
                        session.cmd("cd /d C:\\")
                ...
                if params.get("stop_start_repeats") and os_type == "windows":
                    ...
                    if s:
                        test.fail(f"IO test failed, the output is {o}")
        ...
        if setup_local_nfs:
            ...
            for fs in params.objects("filesystems"):
                ...
                nfs_local.cleanup()
        if setup_filesystem_on_host:
            cmd = f"if losetup -l {loop_device};then losetup -d {loop_device};fi;"
            cmd += f"umount -l {fs_source};"
        ...
        if security_label_test:
            if se_mode_host_before != se_mode:
                try:
                    ...
                    test.fail(f"Restore selinux failed with {str(err_msg)} on" "host.")
            if os_type == "linux" and not se_mode_guest_before == se_mode:
                test.log.info(
                    "Need to change selinux mode back to" f" {se_mode_guest_before}."
                )
