...
class LiveBackup(block_copy.BlockCopy):
    ...
    def generate_backup_params(self):
        ...
        for key, value in backup_params.items():
            if key not in ["image_name", "image_format"]:
                self.params[f"{key}_{backup_image}"] = value
        self.params[f"image_name_{backup_image}"] = f"images/{backup_image}"
        self.params[f"image_format_{backup_image}"] = self.backup_format
    ...
    def create_backup(self, sync, backup_image_name=""):
        ...
        if sync == "full":
            ...
            backup_image_name = f"images/{self.image_chain[0]}.{backup_format}"
        ...
        if not self.get_status():
            self.test.fail(f"{sync} backup job not found")
    ...
    def get_image_size(self, image):
        ...
        if self.vm:
            pids = process.getoutput(
                f"lsof {qemu_image.image_filename} |grep -v PID|awk '{{print $2}}'"
            )
    ...
    def reopen(self):
        ...
        image_name = self.params.get(f"image_name_{image_chain[-1]}")
        super().reopen(image_name)
    ...
    def verify_job_info(self):
        ...
        for check_param in check_params:
            ...
            if str(value) != target_value:
                self.test.fail(
                    f"{check_param} unmatched. Target is {target_value}, result is {value}"
                )
    ...
    def verify_efficiency(self):
        ...
        if not self.incremental_backup_time < self.full_backup_time:
            self.test.fail(
                f"incremental backup time {self.incremental_backup_time}, "
                f"larger than full backup time {self.full_backup_time} "
            )
        ...
        if incre_size > full_size:
            self.test.fail(
                f"incremental backup image size {incre_size}, "
                f"larger than full backup image size {full_size} "
            )
