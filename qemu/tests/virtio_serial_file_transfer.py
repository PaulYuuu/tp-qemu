...
def get_abstract_address(hostfile):
    ...
    """
    find_cmd = f"cat /proc/net/unix | grep '{hostfile}'"
        ...
        if isinstance(port, qemu_virtio_port.VirtioSerial):
            if port.name == port_name:
                ...
                if port.port_type == "unix_socket":
                    ...
                    hostfile = f"{port.hostfile[0]}:{port.hostfile[1]}"
                elif port.port_type == "pty":
                    hostfile = re.findall(
                        f"{port_name}: filename=pty:(/dev/pts/\\d)?", chardev_info
                    )[0]
...
def generate_data_file(dir_name, file_size=0, session=None):
    ...
    """
    data_file = os.path.join(dir_name, f"tmp-{utils_misc.generate_random_string(8)}")
...
def _transfer_data(session, host_cmd, guest_cmd, timeout, sender):
    ...
    try:
        ...
        error_context.context(f"Send host command: {host_cmd}", LOG_JOB.info)
...
def transfer_data(
    ...
    try:
        ...
        host_cmd = f"{python_bin} {host_script} -t {port_type} -s {port_path} -f {host_file_name} -a {host_action}"
        guest_script = os.path.join(guest_path, params["guest_script"])
        python_bin = params.get("python_bin", python_bin)
        guest_cmd = f"{python_bin} {guest_script} -d {port_name} -f {guest_file_name} -a {guest_action}"
        ...
        if clean_file:
            ...
            session.cmd(f"{clean_cmd} {guest_file_name}")
        session.close()
    return result
