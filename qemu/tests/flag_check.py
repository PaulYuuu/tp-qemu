...
def run(test, params, env):
    ...
    4. get extra flags
       ...
       c. params check: check lack flag in host include unknown flag
    ...
    if params.get("start_vm") == "no" and "unknown,check" in extra_flags:
        ...
        if params["qemu_output"] not in output:
            test.fail("no qemu output: {}".format(params["qemu_output"]))
    else:
        vm.verify_alive()
        ...
        error_context.context(f"Get model {qemu_model} support flags", test.log.info)

        # Get flags for every reg from model's info
        models_info = process.system_output(f"cat {cpuinfo_file}").split("x86")
        ...
        if model_info:
            for reg in reg_list:
                ...
                if reg_flags:
                    model_support_flags += f" {reg_flags}"
        ...
        # out_flags is defined in dump file, but not in guest
        out_flags = params.get("out_flags", " ").split()
        out_flags = set(map(cpu.Flag, out_flags))
        # no_check_flags is defined in all_support_flags, but not in guest and host
        no_check_flags = params.get("no_check_flags", " ").split()
        no_check_flags = set(map(cpu.Flag, no_check_flags))
        # option_flags are generated by kernel or kvm, which are not defined in
        ...
        if "check" in extra_flags and "unknown" not in extra_flags:
            ...
            if miss_warn:
                test.fail(f"no warning for lack flag {miss_warn}")
