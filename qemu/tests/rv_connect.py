...
def launch_rv(test, client_vm, guest_vm, params):
    ...
    if qemu_ticket:
        guest_vm.monitor.cmd(f"set_password spice {qemu_ticket}")
    ...
    if display == "spice":
        ...
        if guest_vm.get_spice_var("spice_ssl") == "yes":
            ...
            if ssltype == "invalid_implicit_hs":
                spice_url = (
                    rf" spice://{hostname}?tls-port={host_tls_port}\&port={host_port}"
                )
            else:
                spice_url = (
                    rf" spice://{host_ip}?tls-port={host_tls_port}\&port={host_port}"
                )
            ...
            if rv_parameters_from == "menu":
                ...
                # without spice url
                line = f"spice://{host_ip}?port={host_port}"
            elif rv_parameters_from == "file":
                pass
            else:
                cmd += f" spice://{host_ip}?port={host_port}"
    ...
    if proxy:
        ...
        if len(split) > 1:
            ...
            host_port = "3128"
        if rv_parameters_from != "file":
            client_session.cmd(f"export SPICE_PROXY={proxy}")
