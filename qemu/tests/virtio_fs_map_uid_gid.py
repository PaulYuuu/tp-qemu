...
def run(test, params, env):
    ...
    def get_sub_uid_gid(id_type):
        ...
        """
        output = (
            process.system_output(f"cat /etc/{id_type} |grep {user_name}", shell=True)
            .decode()
            .strip()
        )
    ...
    def create_file_in_guest(user_guest):
        ...
        error_context.context(
            "Create a file in shared dir " f"with {user_guest} user.", test.log.info
        )
        if user_guest != "root":
            if session.cmd_status(f"id {user_guest}") == 0:
                session.cmd(del_user_cmd % user_guest)
            session.cmd(add_user_cmd % user_guest)
            guest_user_session = vm.wait_for_login()
            guest_user_session.cmd(f"su {user_guest}")
    ...
    def get_expect_user_id(user_guest):
        ...
        """
        error_context.context(
            f"Get the user {user_guest}'s uid:gid from guest.", test.log.info
        )
        user_uid = session.cmd_output(f"id {user_guest} -u").strip()
        user_gid = session.cmd_output(f"id {user_guest} -g").strip()

        expect_id = {
            "guest": user_uid + ":" + user_gid,
            "host": f"{str(int(uid_begin) + int(user_uid))}:{str(int(gid_begin) + int(user_gid))}",
        }
        test.log.info(f"The expected file's id is {expect_id} for {user_guest}")
        return expect_id
    ...
    def get_file_owner_id_guest(file_name):
        ...
        """
        error_context.context(
            f"Get the file {file_name}'s uid:gid in guest.", test.log.info
        )
        cmd_get_file = "ls -l %s"
        guest_file = os.path.join(fs_dest, file_name)
        ...
        s, o = session.cmd_status_output(f"id {owner_guest} -u")
        if s:
            uid = owner_guest
        else:
            uid = o.strip()
        s, o = session.cmd_status_output(f"id {group_guest} -g")
        if s:
            gid = group_guest
        else:
            gid = o.strip()
    ...
    def get_file_owner_id_host(file_name):
        ...
        """
        error_context.context(
            f"Get the file {file_name}'s uid:gid in host.", test.log.info
        )
        cmd_get_file = "ls -l %s"
        ...
        if process.system(f"id {owner_host} -u", shell=True, ignore_status=True):
            uid = owner_host
        else:
            uid = (
                process.system_output(f"id {owner_host} -u", shell=True)
                .decode()
                .strip()
            )
        if process.system(f"id {group_host} -g", shell=True, ignore_status=True):
            gid = group_host
        else:
            gid = (
                process.system_output(f"id {group_host} -g", shell=True)
                .decode()
                .strip()
            )
        return uid + ":" + gid
    ...
    try:
        ...
        if process.system(f"id {user_name}", shell=True, ignore_status=True) == 0:
        ...
        cmd_run_virtiofsd += f" --shared-dir {shared_dir}"
        cmd_run_virtiofsd += fs_binary_extra_options
        error_context.context(
            f"Running daemon command {cmd_run_virtiofsd} with {user_name} user.",
            test.log.info,
        )
        ...
        if session.cmd_status(f"id {common_guest_user}") == 0:
