...
def run(test, params, env):
    ...
    def run_stress():
        ...
        if params["os_type"] == "linux":
            params["stress_args"] = (
                f"--vm {mem // 512} --vm-bytes 256M --timeout {stress_duration}"
            )
            ...
            test_install_cmd = f'dir "{install_path}" | findstr /I heavyload'
            heavyload_install()
    ...
    if params.get("on_numa_node"):
        for target_node in host_numa_node.get_online_nodes_withmem():
            ...
            if int(node_mem_free) > mem:
                ...
                params["qemu_command_prefix"] = f"numactl --membind={target_node}"
                params[f"target_num_node{target_node}"] = origin_nr
                break
            test.log.info(
