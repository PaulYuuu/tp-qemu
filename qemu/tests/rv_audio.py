...
def run(test, params, env):
    ...
    if guest_session.cmd_status("ls {}".format(params.get("audio_tgt"))):
        test.log.info(params.get("audio_src"))
        test.log.info(params.get("audio_tgt"))
        guest_vm.copy_files_to(params.get("audio_src"), params.get("audio_tgt"))
    if client_session.cmd_status("ls {}".format(params.get("audio_tgt"))):
        client_vm.copy_files_to(params.get("audio_src"), params.get("audio_tgt"))

    if params.get("rv_record") == "yes":
        test.log.info("rv_record set; Testing recording")
        player = client_vm.wait_for_login(timeout=int(params.get("login_timeout", 360)))
        ...
        player = guest_vm.wait_for_login(timeout=int(params.get("login_timeout", 360)))
    ...
    player.cmd(
        "aplay {} &> /dev/null &".format(params.get("audio_tgt")),
        timeout=30,
    )
    ...
    recorder_session.cmd(
        "arecord -d {} -f cd -D hw:0,1 {}".format(  # records
            params.get("audio_time", "200"),  # duration
            params.get("audio_rec"),
        ),  # target
        timeout=500,
    )
