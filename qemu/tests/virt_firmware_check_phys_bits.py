...
def run(test, params, env):
    ...
    if not host_phys_bits.isdigit():
        test.error(
            f"Failed to get host phys-bits, the actual output is '{host_phys_bits}'"
        )
    ...
    try:
        error_context.context(
            f"Start the vm with host-phys-bits-limit={host_phys_bits_limit}.",
            test.log.info,
        )
        ...
        if err_msg:
            if err_msg not in str(e):
                test.fail(
                    f"Boot a vm with invalid phys-bits '{host_phys_bits_limit}', "
                    "the error message is not the expected value "
                    f"'{err_msg}'. The actual output is '{str(e)}'."
                )
        elif ignored_err_msg:
            if not re.search(ignored_err_msg, str(e), re.S | re.I):
                test.fail(
                    f"Boot a vm with phys-bits '{host_phys_bits_limit}', the ignored "
                    "error message is not the expected value "
                    f"'{ignored_err_msg}'. The actual output is '{str(e)}'."
                )
        else:
            test.error(f"Failed to create a vm, the error is '{str(e)}'")
    else:
        if err_msg:
            test.fail(
                "Start the vm unexpectedly with "
                f"host-phys-bits-limit={host_phys_bits_limit}."
            )
        ...
        if sev_status or sev_es_status:
            ...
            if not host_memory_encryption_bits.isdigit():
                test.error(
                    ...
                    f"actual output is '{host_memory_encryption_bits}'"
                )
        ...
        if not re.search(phys_bits_msg, logs, re.S | re.I):
            test.fail(
                f"Not found phys-bits message '{phys_bits_msg}' in "
                "virt firmware log."
            )
