...
def run(test, params, env):
    ...
    def set_cpu_affinity(pid, mask):
        ...
        """
        tids = process.run(
            f"ps -L --pid={pid} -o lwp=", verbose=False, ignore_status=True
        ).stdout_text.split()
        prev_masks = {}
        for tid in tids:
            prev_mask = process.run(
                f"taskset -p {tid}", verbose=False
            ).stdout_text.split()[-1]
            prev_masks[tid] = prev_mask
            process.system(f"taskset -p {mask} {tid}", verbose=False)
        children = process.run(
            f"ps --ppid={pid} -o pid=", verbose=False, ignore_status=True
        ).stdout_text.split()
    ...
    def restore_cpu_affinity(prev_masks):
        ...
        for tid, mask in prev_masks.items():
            process.system(
                f"taskset -p {mask} {tid}", verbose=False, ignore_status=True
            )
    ...
    if abs(drift) > drift_threshold:
        test.fail(f"Time drift too large: {drift:.2f}%")
    if abs(drift_total) > drift_threshold_after_rest:
        test.fail(f"Time drift too large after rest period: {drift_total:.2f}%")
