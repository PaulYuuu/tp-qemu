...
def run(test, params, env):
    ...
    if process.system("rpm -q libtpms-devel", shell=True, ignore_status=True) != 0:
        try:
            process.system(
                f"cd /home/ && brew download-build --rpm {libtpms_devel_rpm}",
                shell=True,
            )
            libtpms_devel_rpm = libtpms_devel_rpm + ".rpm"
            process.system(f"rpm -i /home/{libtpms_devel_rpm}", shell=True)
        except Exception:
    ...
    try:
        ...
        for test_case in params.get("test_case").split(";"):
            ...
            process.system(f"cd {repo_dir} && {build_execute_cmd}", shell=True)
    finally:
        process.system(
            f"rm -f /home/{libtpms_devel_rpm}*", shell=True, ignore_status=True
        )
