...
def wait_plug_disks(session, action, disks_before_plug, excepted_num, windows, test):
    ...
    if not utils_misc.wait_for(
        ...
        test.fail(
            f"Failed to {action} devices from guest, need to {action}: {excepted_num}, "
            f"actual {action}: {len(disks_before_plug ^ all_disks)}"
        )
...
def run(test, params, env):
    ...
    def run_sub_test(test_name):
        ...
        error_context.context(f"Running sub test '{test_name}'.", test.log.info)
    ...
    def plug_block_devices(action, plug_devices):
        ...
        for dev in plug_devices:
            ...
            if ret[1] is False:
                test.fail(f"Failed to {action} device '{dev}', {ret[0]}.")
    ...
    def format_disk_win():
        ...
        error_context.context(f"Format disk {new_disk} in windows.", test.log.info)  # pylint: disable=E0606
        session = vm.wait_for_login(timeout=timeout)
        if disk_index is None and disk_letter is None:
            drive_letters.append(
                utils_disk.configure_empty_windows_disk(
                    session, new_disk, params[f"image_size_{img}"]
                )[0]
            )
    ...
    def get_disk_size(did):
        ...
        if windows:
            ...
            cmd = f"echo %s > {script} && diskpart /s {script} && del /f {script}"
    ...
    def check_disk_size(did, excepted_size):
        ...
        """
        error_context.context(
            f"Check whether the size of the disk[{did}] hot plugged is equal to "
            f"excepted size({excepted_size}).",
            test.log.info,
        )
        value, unit = re.search(r"(\d+\.?\d*)\s*(\w?)", excepted_size).groups()
        if utils_numeric.normalize_data_size(get_disk_size(did), unit) != value:
            test.fail(
                f"The size of disk {did} is not equal to excepted size({excepted_size})."
            )
    ...
    for iteration in range(int(params.get("repeat_times", 3))):
        try:
            ...
            if pid:
                ...
                process.getoutput(
                    f"timeout 20 strace -tt -T -v -f -s 32 -p {pid} -o "
                    f"{logdir}/strace.log"
                )
            else:
