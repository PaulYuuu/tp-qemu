...
def run(test, params, env):
    ...
    node_ids = [params[f"numa_nodeid_{node}"] for node in numa_nodes]
    node_cpus_mapping = assign_numa_cpus(node_ids, alignment)
    for node in numa_nodes:
        params[f"numa_cpus_{node}"] = ",".join(
            node_cpus_mapping[params[f"numa_nodeid_{node}"]]
        )
    ...
    for vcpu_dev in vcpu_devices:
        error_context.context(f"hotplug vcpu device: {vcpu_dev}", test.log.info)
    ...
    if os_type == "linux":
        ...
        for node_id, node_cpus in node_cpus_mapping.items():
            try:
                ...
                test.error(f"Could not find node {node_id} in guest.")
        test.log.info("Number of each CPU in guest matches what we assign.")

        for vcpu_dev in vcpu_devices[::-1]:
            error_context.context(f"hotunplug vcpu device: {vcpu_dev}", test.log.info)
