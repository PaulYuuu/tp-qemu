...
def run(test, params, env):
    ...
    def verify_guest_clock_source(session, expected):
        ...
        if expected not in session.cmd(clocksource_cmd):
            test.fail(f"Guest doesn't use '{expected}' clocksource")
    ...
    for vmid, se in enumerate(sessions):
        ...
        process.system(
            f"taskset -cp {host_cpu_list[cpu_id]} {vm_obj[vmid].get_pid()}",
            shell=True,
        )
        ...
        if clocksource not in currentsource:
            error_context.context(
                f"Update guest kernel cli to {clocksource}", test.log.info
            )
    ...
    if fail_offset:
        test.fail(
            f"The time drift of following guests {fail_offset} are larger than 5s."
        )
