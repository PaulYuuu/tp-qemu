...
def run(test, params, env):
    ...
    def verify_bitmap_counts(vm, source_node, bitmap_count):
        ...
        if source_node not in bitmaps_dict:
            raise test.fail(f"device '{source_node}' not found!")
        bitmap_len = len(bitmaps_dict[source_node])
        msg = f"bitmap count mismatch, {bitmap_len} != {bitmap_count}"
    ...
    source_node = f"drive_{source_image}"
    target_node = f"drive_{target_image}"
