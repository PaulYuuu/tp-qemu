...
class QemuGuestAgentUpdateTest(QemuGuestAgentBasicCheckWin):
    ...
    def gagent_check_pkg_update(self, test, params, env):
        ...
        def _get_pkg_download_cmd():
            ...
            qga_html_download_cmd = f"wget {qga_url} -O {qga_html}"
            process.system(
                qga_html_download_cmd, float(params.get("login_timeout", 360))
            )

            with open(qga_html) as f:
                lines = f.readlines()
            list_qga = []
            for line in lines:
            ...
            qga_pattern = re.compile(r"{}".format(params["qga_pattern"]))
            ...
            qga_url_pre = rf"{qga_url}/{qga_pre_pkg}/{self.qemu_ga_pkg}"
            qga_host_path = params["gagent_host_path"]
            params["gagent_download_cmd"] = f"wget {qga_url_pre} -O {qga_host_path}"
