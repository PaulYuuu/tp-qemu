...
def setup_test_environment(test, params, vm, session):
    ...
    for cmd in setup_cmds:
        ...
        if status:
            test.error(f"Command '{cmd}' failed, status: {status}, output: {output}")
...
def trigger_crash(test, vm, params):
    ...
    if crash_method == "nmi":
        ...
        try:
            ...
            if status:
                test.error(
                    f"Command '{cmd}' failed, status: {status}, output: {output}"
                )
        ...
        test.cancel(
            f"Crash trigger method {crash_method} not supported, "
            "please check cfg file for mistake."
        )
