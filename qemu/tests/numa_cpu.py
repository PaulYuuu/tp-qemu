...
def run(test, params, env):
        ...
        if vm_arch != "aarch64":
            ...
            if new_specified_cpus != expected_cpus:
                test.fail(
                    ...
                    f"numa_cpu_set is {new_specified_cpus}, but expected result is: {expected_cpus}"
                )
    ...
    if os_type == "linux":
        ...
        if numa_cpu_guest != expected_cpus:
            test.fail(
                f"cpu ids for each node in guest os is: {numa_cpu_guest}, but the "
                f"expected result is: {expected_cpus}"
            )
