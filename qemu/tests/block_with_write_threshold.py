...
def run(test, params, env):
    ...
    def set_block_write_threshold(monitor, node_name, size):
        ...
        error_context.context(
            f"Set block write threshold to {size} for the block " "drive in QMP.",
            test.log.info,
        )
        monitor.cmd(
    ...
    def get_data_disk(session):
        """Get the data disk."""
        if is_linux:
            extra_params = params[f"blk_extra_params_{data_img_tag}"]
    ...
    data_img_size = params[f"image_size_{data_img_tag}"]
