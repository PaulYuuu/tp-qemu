...
def run(test, params, env):
    ...
    def get_user_uid_gid(user_name):
        ...
        """
        output = process.system_output(f"id {user_name}", shell=True).decode().strip()
    ...
    def enable_uid_gid(uid_gid_value):
        ...
        if s:
            test.fail(f"Fail command: {enable_cmd}. Output: {o}.")
    ...
    def check_file_uid_gid(volume_letter, expect_id):
        ...
        output = (
            process.system_output(f"ls -l {host_file}", shell=True).decode().strip()
        )
        owner = output.split()[2]
        group = output.split()[3]
        if process.system(f"id {owner} -u", shell=True, ignore_status=True):
            uid = owner
        else:
            uid = process.system_output(f"id {owner} -u", shell=True).decode().strip()
        if process.system(f"id {group} -g", shell=True, ignore_status=True):
            gid = group
        else:
            gid = process.system_output(f"id {group} -g", shell=True).decode().strip()
        uid_gid_host = uid + ":" + gid
        if uid_gid_host != expect_id:
            test.fail(
                "Check file owner/group failed, "
                f"real value is {uid_gid_host}, "
                f"expected value is {expect_id}"
            )
        ...
        if process.system(f"id {user_name}", shell=True, ignore_status=True) == 0:
            ...
            if s:
                if "is currently used by process" in o:
                    ...
                    test.fail("Unknown error when deleting the " f"user: {o}")
        if (
            process.system(
                f"grep {user_name} /etc/group", shell=True, ignore_status=True
            )
            == 0
        ):
            add_user_cmd = f"useradd -g {user_name} {user_name}"
    ...
    for test_value, expect_value in dict_ids.items():
        error_context.context(
            f"Set host UID:GID={test_value} to viofs" " service.", test.log.info
        )
