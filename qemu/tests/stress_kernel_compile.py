...
def run(test, params, env):
    ...
    def kernelcompile(session, vm_name):
        ...
        get_kernel_cmd = f"wget {path}"
        try:
            status, output = session.cmd_status_output(get_kernel_cmd, timeout=240)
            if status != 0:
                test.log.error(output)
                test.fail(f"Fail to download the kernel in {vm_name}")
            else:
    ...
    for tag in range(1, guest_number):
        params["vms"] += f" stress_guest_{tag}"
    ...
    if vmem < 256:
        test.cancel(
            ...
            f" Please try less than {guest_number} guests"
            " in this host."
        )
    params["mem"] = vmem
    params["start_vm"] = "yes"
    ...
    for vm_name in params["vms"].split():
        ...
        if not session:
            test.fail(f"Could not log into guest {vm_name}")
