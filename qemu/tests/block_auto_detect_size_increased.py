...
def run(test, params, env):
    ...
    def get_disk_size_by_diskpart(index):
        ...
        pattern = rf"Disk\s+{index}\s+Online\s+(\d+\s+\w+)\s+\d+\s+\w+"
    ...
    def check_disk_size(index):
        ...
        if normalize_data_size(size, u) != v:
            test.fail(f"The size of disk {index} is not equal to {img_resize_size}")
    ...
    vm.copy_files_to("/home/dd_file", f"{mpoint}:\\dd_file")
