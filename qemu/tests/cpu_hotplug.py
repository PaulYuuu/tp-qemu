...
def run(test, params, env):
    ...
    for cpuid in range(current_cpus, total_cpus):
        error_context.context(f"hot-pluging vCPU {cpuid}", test.log.info)
    ...
    if total_cpus_monitor != total_cpus:
        test.fail(
            f"Monitor reports {total_cpus_monitor} CPUs, when VM should have"
            f" {total_cpus}"
        )
    ...
    for i in range(onoff_iterations):
        session.cmd(f"echo {irq_mask} > /proc/irq/{irq}/smp_affinity")
        for online_file in online_files:
            session.cmd(f"echo 0 > {online_file}")
        for online_file in online_files:
            session.cmd(f"echo 1 > {online_file}")
