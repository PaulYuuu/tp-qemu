...
def run(test, params, env):
    ...
    error_context.context(f"Try to get qmp events in {timeout} seconds!", test.log.info)
    ...
    while time.time() < end_time:
        for monitor in qmp_monitors:
            ...
            if event:
                txt = f"Monitr {monitor.name} "
                txt += f"receive qmp {event_check} event notification"
                test.log.info(txt)
                qmp_num -= 1
    ...
    if qmp_num > 0:
        ...
        for monitor in qmp_monitors:
            err += f"{monitor.name} "
        err += f" did not receive qmp {event_check} event notification."
        err += f" ip link show command output in guest: {output}"
        test.fail(err)

    if post_cmd:
        for nic in vm.virtnet:
            post_cmd = post_cmd % nic.device_id
            error_context.context(f"Run post_cmd '{post_cmd}'", test.log.info)
            ...
            if post_cmd_check:
                if post_cmd_check not in str(output):
                    err = f"Did not find '{post_cmd_check}' in "
                    err += f"'{post_cmd}' command's output: {output}"
                    test.fail(err)
