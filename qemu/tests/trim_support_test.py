...
def run(test, params, env):
    ...
    if not disk_index:
        test.error(f"Failed to get the disk index of size {image_size_str}")
    ...
    if status:
        test.error(f"Error when trim the volume, status={status}, output={output}")
    if event_id:
        time.sleep(10)
        session = vm.reboot(session)
        if query_system_events(params["filter_options"]):
            test.fail(
                "Disk corruption after trim for {}".format(params.get("block_size"))
            )
    ...
    if params["retrim_size_check"] == "yes":
        ...
        if new_size is None:
            test.error(f"Data disk size is not smaller than: {ori_size}MB")
