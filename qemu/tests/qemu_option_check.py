...
def run(test, params, env):
    ...
    def get_device_option(qemu_binary, device_name):
        ...
        device_support_option = process.system_output(
            f"{qemu_binary} -device {device_name},? 2>&1",
            timeout=10,
            ignore_status=True,
            shell=True,
        )
        device_support_option = device_support_option.decode().strip()
        if not re.findall(rf"{device_name}\.(.*)=(.*)", device_support_option):
    ...
    error_context.context(
        f"Get qemu support {device_name} device options", test.log.info
    )
