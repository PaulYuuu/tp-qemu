...
def run(test, params, env):
    ...
    def _verify_image_backing_file(info_output, base):
        ...
        if backing_filename != backing_filename_desired:
            test.fail(
                f"backing image name mismatch, got {backing_filename}, expect {backing_filename_desired}"
            )
        if backing_format:
            backing_format_desired = base.image_format
            if backing_format != backing_format_desired:
                test.fail(
                    f"backing image format mismatch, got {backing_format}, expect {backing_format_desired}"
                )
    ...
    def _verify_qcow2_compatible(info_output, image):
        ...
        if compat != compat_desired:
            test.fail(
                f"{image.tag} image compat version mismatch, got {compat}, expect {compat_desired}"
            )
