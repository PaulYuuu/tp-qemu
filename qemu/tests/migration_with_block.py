...
def run(test, params, env):
    ...
    def unpack_package(session, src, dst):
        """Unpack the package."""
        session.cmd(f"tar -xvf {src} -C {dst}")
    ...
    def run_stressapptest(timeout):
        ...
        try:
            ...
            sub_session.cmd(f"{stress_bin_path} {stress_options}", timeout)
        finally:
    ...
    def get_iso_size(iso_file):
        ...
        return int(
            process.system_output(
                f"ls -l {iso_file} | awk '{{print $5}}'", shell=True
            ).decode()
        )
    ...
    def check_cdrom_info_by_qmp(check_items):
        ...
        error_context.context(
            f'Check if the info "{str(check_items)}" are match with the output of query-block.',
            test.log.info,
        )
        ...
        for key, val in check_items.items():
            if blocks[device_name][key] == val:  # pylint: disable=E0606
                continue
            test.fail(f'No such "{key}: {val}" in the output of query-block.')
    ...
    def eject_cdrom():
        ...
        if check_block(orig_img_name):  # pylint: disable=E0606
            test.fail(f"Failed to eject cdrom {orig_img_name}. ")
    ...
    def change_cdrom():
        ...
        if not check_block(new_img_name):
            test.fail(f"Fail to change cdrom to {new_img_name}.")
    ...
    def check_vm_status(timeout=600):
        ...
        if not getattr(vm, f"wait_for_{action}")(timeout=timeout):
            test.fail(f"Failed to {action} vm.")
    ...
    def bg_stress_is_alive(session, name):
        ...
        return session.cmd_output(f"pgrep -xl {name}")
    ...
    error_context.context(f"Boot guest {src_desc} on src host.", test.log.info)
    ...
    if with_cdrom:
        ...
        if device_name is None:
            test.fail(f"Failed to get device using image {orig_img_name}.")
    ...
    error_context.context(f"Boot guest {dst_desc} on dst host.", test.log.info)
    ...
    if do_stress_background:
        if bg_stress_is_alive(session, stress_name):
            if kill_bg_stress:
                session.cmd(f"killall {stress_name}")
            else:
