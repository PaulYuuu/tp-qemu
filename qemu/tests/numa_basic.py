...
def run(test, params, env):
    ...
    for node_id in node_list:
        error_context.base_context(
            f"Bind qemu process to numa node {node_id}", test.log.info
        )
        vm = f"vm_bind_to_{node_id}"
        params["qemu_command_prefix"] = f"numactl --cpunodebind={node_id}"
        ...
        if node_used_most != node_id:
            test.fail(
                ...
                f"Expect: {node_id}, used: {node_used_most}"
            )
