...
class BlockdevIncbkModRdonlyBitmapTest(BlockdevLiveBackupBaseTest):
    ...
    def prepare_test(self):
        ...
        self._error_msg = f"(core dumped)|{self.main_vm.get_pid()} Aborted"
    ...
    def modify_readonly_bitmaps(self):
        for act in ["block-dirty-bitmap-clear", "block-dirty-bitmap-remove"]:
            ...
            try:
                ...
                self.test.fail(f"{act} succeeded unexpectedly")
    ...
    def restart_vm_with_readonly_data_image(self):
        ...
        self.params[f"image_readonly_{self._source_images[0]}"] = "on"
        self.prepare_main_vm()
        self._error_msg += f"|{self.main_vm.get_pid()} Aborted"
