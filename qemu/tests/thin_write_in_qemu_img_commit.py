...
def run(test, params, env):
    ...
    def _verify_map_output(output):
        ...
        if expected not in json.loads(output.stdout_text):
            test.fail(
                "Commit failed, data from 0 to {} are not zero".format(
                    params["write_size"]
                )
            )
    ...
    _qemu_io(img_base, "write -P 1 0 {}".format(params["image_size_base"]))

    _create_external_snapshot(snapshot)
    img_sn = _get_image_object(snapshot)
    _qemu_io(img_sn, "write -z 0 {}".format(params["write_size"]))
