...
def run(test, params, env):
    ...
    def _prepare():
        ...
        if image_params.get("create_description_cmd"):
            params[f"nbd_export_description_{tag}"] = (
                process.run(
        ...
        if image_params.get("create_image_cmd"):
            params[f"create_image_cmd_{tag}"] = image_params["create_image_cmd"].format(
                desc=params[f"nbd_export_description_{tag}"],
    ...
    def _make_qemu_io_cmd():
        ...
        nbd_image_format = "-f {}".format(nbd_image_params["image_format"])
        ...
        if tls_obj or sec_obj:
            nbd_image_format = ""
            nbd_image_filename = (
                f"'{qemu_storage.get_image_json(nbd_image, nbd_image_params, None)}'"
            )
    ...
    try:
        ...
        if result.exit_status != 0:
            test.fail(f"Failed to execute qemu-io, error: {result.stderr.decode()}")

        if params.get("msg_check"):
            if params["msg_check"] not in result.stdout.decode().strip():
                test.fail(
                    "Failed to read message({}) from output({})".format(
