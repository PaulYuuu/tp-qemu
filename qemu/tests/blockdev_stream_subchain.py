...
class BlockdevStreamSubChainTest(BlockDevStreamTest):
    ...
    def snapshot_test(self):
        ...
        for idx in range(1, len(chain)):
            backup_utils.blockdev_snapshot(
                self.main_vm, f"drive_{chain[idx - 1]}", f"drive_{chain[idx]}"
            )
    ...
    def _check_backing(self, backing):
        ...
        try:
            ...
            if not self._is_same_file(backing["backing"]["file"], data_image_opts):
                self.test.fail(f"Failed to get backing file for {self._base_node_tag}")
        except Exception as e:
            self.test.fail(f"Failed to get backing chain: {str(e)}")
    ...
    def check_backing_chain(self):
        ...
        for item in out:
            if self.base_tag in item["qdev"]:
                ...
                if not backing:
                    self.test.fail(
                        f"Failed to get backing_file for qdev {self.base_tag}"
                    )
                backing_dict = json.loads(backing[5:])
                self._check_backing(backing_dict)
                break
        else:
            self.test.fail(f"Failed to find {self.base_tag}")
    ...
    def clone_vm_with_snapshot(self):
        ...
        self.clone_vm.params[f"image_chain_{self.snapshot_tag}"] = (
            f"{self.base_tag} {self._base_node_tag} {self.snapshot_tag}"
        )
