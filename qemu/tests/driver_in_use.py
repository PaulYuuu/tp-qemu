...
def check_bg_running(vm, params):
    ...
    if params["os_type"] == "linux":
        output = session.cmd_output_safe(f"pgrep -l {target_process}")
    else:
...
def run(test, params, env):
    ...
    def run_bg_test_simu(bg_stress_test):
        ...
        """
        error_context.context(f"Run test {bg_stress_test} background", test.log.info)
        stress_thread = None
        ...
        if params.get("set_bg_stress_flag", "no") == "yes":
            ...
            if not utils_misc.wait_for(
                ...
                err = f"Fail to start {bg_stress_test} test"
                test.error(err)
        env["bg_status"] = 1
    ...
    error_context.context(
        f"Run {main_test} {run_bg_flag} {bg_stress_test}", test.log.info
    )
