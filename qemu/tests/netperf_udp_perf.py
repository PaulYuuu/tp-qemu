...
def run(test, params, env):
    ...
    cmd = "ifconfig {} {} up".format(params.get("client_physical_nic"), client_ip)
...
def start_test(
    ...
    fd = open(f"{resultsdir}/netperf-udp-perf.result.{time.time()}.RHS", "w")
    ...
    def thread_cmd(
        ...
        option = f"{client_path} -t UDP_STREAM -w {burst_time} -b {numbers_per_burst} -H {server} -l {test_duration}"
    ...
    for i in burst_time.split():
        for j in numbers_per_burst.split():
            ...
            finished_result = netperf_base.ssh_cmd(client, f"cat {fname}")
            f = open(fname, "w")
            ...
            process.system_output(
                f"rm -f {fname}", verbose=False, ignore_status=True, shell=True
            )
            netperf_base.ssh_cmd(client, f"rm -f {fname}")
