...
def run(test, params, env):
    ...
    if params.get("start_vm") == "no":
        if parameter_value == "random":
            ...
            if random_ignore_str:
                args += (f"ignore_str={random_ignore_str}",)
            if convert_str:
                args += (f"convert_str={convert_str}",)
    ...
    if params.get("check_in_qtree") == "yes":
        ...
        try:
            ...
            for qdev in qtree.get_nodes():
                ...
                test.fail(
                    f"Can not find property '{qtree_check_option}' from info qtree where '{keyword}' is "
                    f"'{qtree_check_value}'"
                )
            ...
            if (
                ...
                test.fail(
                    ...
                    f"command line: '{qtree_value}' vs '{parameter_value_raw}'"
                )
    ...
    for check_cmd in check_cmds.split():
        ...
        error_context.context(f"Check option with command {cmd}", test.log.info)
        _, output = session.cmd_status_output(cmd)
        if not re.findall(rf"{pattern}", output):
            failed_log += (
                f"Can not find option {params_name} from guest."
                f" Guest output is '{output}'"
            )

        if sg_vpd_cmd:
            error_context.context(
                f"Check serial number length with command {sg_vpd_cmd}", test.log.info
            )
            ...
            if actual_len != expected_len:
                test.fail(
                    ...
                    f" Guest output serial number is {actual_len}"
                )
