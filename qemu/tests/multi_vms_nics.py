...
def run(test, params, env):
    ...
    def ping(session, nic, dst_ip, strick_check, flood_minutes):
        ...
        for size in packet_size:
            error_context.context(f"Ping with packet size {size}", test.log.info)
            ...
            if strict_check:
                ...
                if ratio != 0:
                    test.fail(f"Loss ratio is {ratio} for packet size" f" {size}")
            else:
                if status != 0:
        ...
        if strick_check == "yes":
            ratio = utils_test.get_loss_ratio(output)
            if ratio != 0:
                test.fail(f"Packet loss ratio is {ratio} after flood")
        else:
            if status != 0:
                test.fail(f"Ping returns non-zero value {output}")
    ...
    def file_transfer(session, src, dst):
        ...
        cmd = f"dd if=/dev/urandom of={src_path} bs=100M count=1"
        cmd = params.get("file_create_cmd", cmd)

        error_context.context(f"Create file by dd command, cmd: {cmd}", test.log.info)
        session.cmd(cmd)

        transfer_timeout = int(params.get("transfer_timeout"))
        log_filename = f"scp-from-{src}-to-{dst}.log"
        error_context.context(f"Transfer file from {src} to {dst}", test.log.info)
        ...
        src_path = dst_path
        dst_path = "/tmp/3"
        log_filename = f"scp-from-{dst}-to-{src}.log"
        error_context.context(f"Transfer file from {dst} to {src}", test.log.info)
        remote.scp_between_remotes(
            dst,
            src,
            port,
    ...
    for vm_name in vms:
        ...
        for index, nic in enumerate(vm.virtnet):
            ...
            if not (guest_ifname and guest_ip):
                err_log = f"vms {vm_name} get ip or ifname failed."
                err_log = f"ifname: {guest_ifname}, ip: {guest_ip}."
                test.fail(err_log)
