...
def run(test, params, env):
    ...
    try:
        for _username in username:
            ...
            if process.system(f"id {_username}", shell=True, ignore_status=True) == 0:
            ...
            if (
                process.system(
                    f"grep {_username} /etc/group", shell=True, ignore_status=True
                )
                == 0
            ):
                add_cmd = f"useradd -g {_username} {_username}"
            process.run(add_cmd)
        ...
        output = process.system_output(f"chmod -R 777 /home/{user_one}")
        error_context.context(output, test.log.info)
        # change user u2's supplementary group to u1
        output = process.system_output(f"usermod -G {user_one} {user_two}")
        error_context.context(output, test.log.info)
        ...
        error_context.context(
            f"Running daemon command {cmd_run_virtiofsd}", test.log.info
        )
        ...
        for _username in username[::-1]:
            ...
            if "is currently used by process" in output.stdout_text:
                ...
                pid = output.split(" ")[-1]
                process.run(f"kill -9 {pid}")
            process.run(f"rm -rf /home/{_username}")
