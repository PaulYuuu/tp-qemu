...
def run(test, params, env):
    ...
    def receive_data(test, session, serial_receive_cmd, data_file):
        ...
        if ori_data.strip() != output.strip():
            err = f"Data lost during transfer. Origin data is:\n{ori_data}"
            err += f"Guest receive data:\n{output}"
            test.fail(err)
    ...
    serial_send_cmd = (
        f"`command -v python python3 | head -1` {send_script} {host_file} {data_file}"
    )
    receive_script = params.get(
        "guest_receive_script", "VirtIoChannel_guest_recieve.py"
    )
    receive_script = f"{guest_path}{receive_script}"
