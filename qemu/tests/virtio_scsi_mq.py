...
def run(test, params, env):
    ...
    def create_data_images():
        ...
        for extra_image in range(images_num):
            image_tag = f"stg{extra_image}"
            params["images"] += f" {image_tag}"
            params[f"image_name_{image_tag}"] = f"images/{image_tag}"
            params[f"image_size_{image_tag}"] = extra_image_size
            params[f"force_create_image_{image_tag}"] = "yes"
    ...
    def check_interrupts2vcpus(irq_map):
        ...
        for irq_id in irq_map.keys():
            cmd = f"cat /proc/irq/{irq_id}/smp_affinity"
            cpu_selected = re.sub(
    ...
    def pin_interrupts2vcpus(irqs_id_reset, cpu_selects):
        ...
        for irq_id, cpu_select in zip(irqs_id_reset, cpu_selects):
            bind_cpu_cmd.append(f"echo {cpu_select} > /proc/irq/{irq_id}/smp_affinity")
    ...
    def load_io_data_disks():
        ...
        if len(cmds) != images_num:
            test.error(f"Disks are not all show up in system, only {len(cmds)} disks.")
        ...
        try:
            if not queue_num or queue_num[0] != num_queues:
                test.fail(
                    ...
                    f"queue_num from guest is {queue_num}, expected is {num_queues}"
                )
        finally:
            session.close()
    ...
    params[f"drive_format_{system_image}"] = system_image_drive_format
