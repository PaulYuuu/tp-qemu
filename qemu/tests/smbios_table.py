...
def run(test, params, env):
    ...
    if params.get("smbios_type_disable", "no") == "no":
        ...
        for sm_type in smbios_type.split():
            ...
            smbios += f" -smbios type={smbios_type_number}"  # pylint: disable=E0606
            ...
            for key in dmidecode_key:
                ...
                smbios += f",{key.lower()}='{smbios_key_para_set}'"
    ...
    if params.get("traversal_machine_emulated", "no") == "no":
        ...
        if ":" in machine_type:
            ...
            support_machine_types = [
                f"{prefix}:{m_type}" for m_type in support_machine_types
            ]
    ...
    for m_type in support_machine_types:
        ...
        error_context.context(
            f"Boot the vm using -M option:'-M {m_type}', smbios " f"para: '{smbios}'",
            test.log.info,
        )
        ...
        for sm_type in smbios_type.split():
            ...
            for key in dmidecode_key:
                ...
                if smbios_set_para not in smbios_get_para:
                    e_msg = f"{sm_type}.{key} mismatch, Set '{smbios_set_para}' but guest is : '{smbios_get_para}'"
    ...
    if failures:
        test.fail(
            "smbios table test reported {} failures:\n{}".format(
                len(failures), "\n".join(failures)
            )
        )
