...
def run(test, params, env):
    ...
    def run_pm_test(pm_test, plug_type):
        ...
        error_context.context(
            f"Run {pm_test} test after {plug_type} balloon device", test.log.info
        )
    ...
    for i in range(int(params.get("balloon_repeats", 3))):
        ...
        if out[1] is False:
            test.fail(f"Failed to hotplug balloon in iteration {i}, {out[0]}")
        ...
        if out[1] is False:
            test.fail(f"Failed to unplug balloon in iteration {i}, {out[0]}")
        time.sleep(2)
    ...
    if params.get("os_type") == "windows":
        out = vm.devices.simple_hotplug(new_dev, vm.monitor)
        if out[1] is False:
            test.fail("Failed to hotplug balloon at last, " f"output is {out[0]}")
