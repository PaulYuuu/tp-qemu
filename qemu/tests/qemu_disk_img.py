...
class QemuImgTest(qemu_storage.QemuImg):
    def __init__(self, test, params, env, tag):
        ...
        super().__init__(t_params, self.data_dir, tag)
    ...
    def start_vm(self, t_params=None):
        ...
        params["start_vm"] = "yes"
        try:
            del params[f"image_name_{base_image}"]
            del params[f"image_format_{base_image}"]
        except KeyError:
            pass
    ...
    def save_file(self, dst):
        ...
        error_context.context(f"save file('{dst}') md5sum in guest", LOG_JOB.info)
    ...
    def check_file(self, dst, md5):
        error_context.context(f"check file('{dst}') md5sum in guest", LOG_JOB.info)
        if md5 != self.__md5sum(dst):
            err = (
                "Md5 value does not match. "
                f"Expected value: {md5} Actual value: {self.__md5sum(dst)}"
            )
            LOG_JOB.error(err)
            return False
        return True
    ...
    def check_image(self, t_params=None):
        error_context.context(
            f"check image file ('{self.image_filename}')", LOG_JOB.info
        )
        t_params = t_params or {}
        return super().check_image(t_params, self.data_dir)

    @error_context.context_aware
    def get_info(self):
        error_context.context(f"get image file ('{self.image_filename}')")
        return super().info()
    ...
    def verify_info(self, params=None):
        ...
        for option in params.objects("option_verified"):
            ...
            if avalue is not None and avalue != evalue:
                msg = f"Get wrong {option} from image {image_filename}!"
                msg += f"Expect: {evalue}, actual: {avalue}"
                self.test.fail(msg)

    @error_context.context_aware
    def check_backingfile(self):
        error_context.context(
            f"check image('{self.image_filename}') backing file", LOG_JOB.info
        )
        out = self.get_info()
        try:
            ...
            if not self.base_tag or self.base_tag == "null":
                ...
                if base_image_repr != backingfile:
                    msg = f"Expected backing file: {self.base_image_filename}"
                    msg += f" Actual backing file: {backingfile}"
                    raise exceptions.TestFail(msg)
        except AttributeError:
            if self.base_tag and self.base_tag != "null":
                msg = f"Could not find backing file for image '{self.image_filename}'"
                raise exceptions.TestFail(msg)
...
def generate_base_snapshot_pair(image_chain):
    ...
    n = len(image_chain)
    if n < 2:
        raise ValueError("Image_chain should contain at" f"least 2 items, got {n}.")
    for i in range(1, n):
