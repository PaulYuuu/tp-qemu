...
def run(test, params, env):
    ...
    def check_interface_ip(timeout=600):
        ...
        try:
            if not utils_misc.wait_for(
                ...
                err_msg = f"Can't get valid ip in {timeout} seconds"
                test.fail(err_msg)
        finally:
            session.close()
    ...
    def set_link_test(
        ...
        error_context.context(
            "Check guest network connecting after reboot " f"by '{reboot_method}'",
            test.log.info,
        )
        guest_netwok_connecting_check(guest_ip, link_up, change_queues)
    ...
    error_context.context(
        f"Disable guest netdev link '{netdev_id}' by set_link", test.log.info
    )
    set_link_test(
    ...
    error_context.context(
        f"Re-enable guest netdev link '{netdev_id}' by set_link", test.log.info
    )
    set_link_test(netdev_id, True, expect_up_status, change_queues, operstate_always_up)

    error_context.context(
        f"Disable guest nic device '{device_id}' by set_link", test.log.info
    )
