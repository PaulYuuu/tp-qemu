...
def run(test, params, env):
    ...
    for serial_id in params.objects("serials"):
        if serial_id != "vs1" and serial_id != "vs9":
            ...
            kernel_params = f"console=hvc{hvc_id},115200"
        ...
        if backend == "unix_socket":
            ...
            hostfile = re.findall(
                f"{serial_id}: filename=pty:(/dev/pts/\\d)?", chardev_info
            )
            if not hostfile:
                test.fail(f"Can't find the corresponding pty backend: {chardev_info}")
