...
def run(test, params, env):
    ...
    def get_vhdx():
        ...
        test.log.info(f"Parameters: {download_url} {image_dir} {md5value} {vhdx_dest}")
        ...
        if status != 0:
            test.error(f"qemu-img convert failed, status: {status}, output: {output}")
    ...
    path_cmd = rf"powershell Remove-Item {nested_dest} -recurse -force -ErrorAction SilentlyContinue"
    ...
    status, output = session.cmd_status_output(
        rf"powershell {nested_dest}\hyperv_run.ps1", timeout=1800
    )
    if status != 0:
