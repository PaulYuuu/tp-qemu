...
def run(test, params, env):
    ...
    qemu_cmd = f"{qemu_bin} {test_cmd}"
    test.log.info(f"Send host command: {qemu_cmd}")
    process.run(cmd=qemu_cmd, verbose=True, ignore_status=True, shell=True)
    qemu_pid = process.getoutput(f"pgrep -f {src_test_binary}", shell=True)
    if not qemu_pid:
    ...
    if not is_file:
        test.fail(f"Can't find the log file {log_file}")
