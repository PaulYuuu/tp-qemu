...
def run(test, params, env):
    ...
    try:
        error_context.base_context(f"Saving VM to {snapshot_tag}", test.log.info)
        vm.monitor.human_monitor_cmd(f"savevm {snapshot_tag}")
        vm_snapshots = vm.monitor.info("snapshots")
        if snapshot_tag not in vm_snapshots:
            test.fail(f"Failed to save VM to {snapshot_tag}")
        error_context.context(f"Loading VM from {snapshot_tag}", test.log.info)
        vm.monitor.human_monitor_cmd(f"loadvm {snapshot_tag}")
        if os_type == "linux":
            vm.verify_kernel_crash()
            vm.verify_dmesg()
    finally:
        if snapshot_tag in vm.monitor.info("snapshots"):
            vm.monitor.human_monitor_cmd(f"delvm {snapshot_tag}")
