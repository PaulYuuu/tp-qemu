...
def run(test, params, env):
    ...
    def _stop_wireshark_session():
        ...
        if status:
            test.error(f"Failed to stop wireshark: status={status}, output={output}")
    ...
    def _parse_log_file(packet_filter):
        ...
        if status:
            test.error(
                "Failed to parse session log file," f" status={status}, output={output}"
            )
        return output
    ...
    if status:
        test.error(f"Failed to install pcap, status={status}, output={output}")
    ...
    if "tshark" not in check_result:
        ...
        if status:
            test.error(f"Failed to install wireshark, status={status}, output={output}")
    ...
    if "Len" not in output:
        test.fail(f"No packet length >= 1514, output={output}")
    session.close()
    ...
    if "Len" in output:
        test.fail(f"Some packet length > 1014, output={output}")
