...
def run(test, params, env):
    ...
    params[f"image_name_{images[0]}"] = params["image_name"]
    params[f"image_format_{images[0]}"] = params["image_format"]
    ...
    error_context.context(f"create snapshot {sn.tag}", test.log.info)
    sn.create(sn.params)

    error_context.context(f"boot vm from snapshot {sn.tag}", test.log.info)
    ...
    error_context.context(f"save random file {guest_file}", test.log.info)
    ...
    error_context.context(f"commit snapshot {sn.tag}", test.log.info)
    ...
    error_context.context(f"boot vm from base {base.tag}", test.log.info)
    ...
    vm.destroy()
    # remove snapshot
    params[f"remove_image_{sn.tag}"] = "yes"
