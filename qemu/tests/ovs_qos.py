...
def run(test, params, env):
    ...
    def set_ovs_port_attr(iface, attribute, value):
        ...
        """
        cmd = f"ovs-vsctl set interface {iface} {attribute}={value}"
        test.log.info("execute host command: %s", cmd)
        status = process.system(cmd, ignore_status=True)
        if status != 0:
            err_msg = f"set {attribute} to {value} for interface '{iface}' "
    ...
    def set_port_qos(vm, rate, burst):
        ...
        error_context.context(
            f"Set QoS for tap '{iface}' use by vm '{vm.name}'", test.log.info
        )
        attributes = zip(
    ...
    def get_throughout(
        ...
        """
        error_context.context(
            f"Set '{server_vm.name}' as netperf server", test.log.info
        )
        if not netperf_server.is_server_running():
            netperf_server.start()

        error_context.context(
            f"Set '{client_vm.name}' as netperf client", test.log.info
        )
    ...
    def report_test_results(datas):
        ...
        if fails:
            ...
            for tap, throughout, rate, burst in fails:
                msg += f"netperf throughout({throughout}) on '{tap}' "
                msg += f"should be near ingress_policing_rate({rate}), "
                msg += f"ingress_policing_burst is {burst};\n"
            test.fail(msg)

    def clear_qos_setting(iface):
        error_context.context(
            f"Clear qos setting for ovs port '{iface}'", test.log.info
        )
        clear_cmd = f"ovs-vsctl clear Port {iface} qos"
