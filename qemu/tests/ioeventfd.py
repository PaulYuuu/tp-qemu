...
def run(test, params, env):
        ...
        for node in qtree.get_nodes():
            if isinstance(node, qemu_qtree.QtreeDev) and (
                ...
                if node.qtree.get("ioeventfd", None) is None:
                    test.fail(f"The qtree device {dev_id} has no property ioeventfd.")
                    ...
                    test.fail(
                        f'The "{ioeventfd_opt}" mismatches with qtree device "{dev_id}"({ioevent_qtree_val}).'
                    )
        else:
            test.error(f'No such "{dev_id}" qtree device.')
    ...
    def _get_ioeventfds(ioeventfd_opt):
        ...
        cmd = f"ls -l /proc/$(pgrep qemu-kvm)/fd > /tmp/{dst_log}; cat /tmp/{dst_log}"
        test.log.debug("Running '%s'", cmd)
        s, o = process.getstatusoutput(cmd)
        test.log.debug(o)
        if s:
            test.error(f"Failed to get the number of event fd.\n{o}")
    ...
    def _compare_ioeventfds():
        ...
        test.log.debug(o)
        if s:
            test.error(f"Failed to compare the outputs.\n{s}")
    ...
    for ioeventfd in ioeventfds:
        ...
        if ioeventfd == "ioeventfd=off" and params.get("iothread_scheme"):
            error_context.context(f"Disable iothread under {ioeventfd}", test.log.info)
        ...
        error_context.context(f'Boot a guest with "{ioeventfd}".', test.log.info)
