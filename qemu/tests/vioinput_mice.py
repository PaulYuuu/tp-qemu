...
def mouse_btn_test(test, params, console, listener, wait_time):
    ...
    for btn in btns:
        error_context.context(f"Click mouse {btn} button", test.log.info)
...
def mouse_scroll_test(test, params, console, listener, wait_time, count=1):
    ...
    for scroll in scrolls:
        error_context.context(f"Scroll mouse {scroll}", test.log.info)
        if "up" in scroll:
...
def mouse_move_test(test, params, console, listener, wait_time, end_pos, absolute):
    ...
    error_context.context(f"Moving pointer from {start_pos} to {end_pos}")
...
def run(test, params, env):
    ...
    error_context.context(f"Check if {mice_name} device is working", test.log.info)
