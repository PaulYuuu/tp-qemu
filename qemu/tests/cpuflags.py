...
def run(test, params, env):
    ...
    def get_guest_host_cpuflags_legacy(cpumodel):
        ...
        if model is None:
            test.fail(f"Cannot find {cpumodel} cpu model.")
    ...
    class ParseCpuFlags:
        ...
        def parse_file(self, file_path):
            self.parser.ParseFile(open(file_path))
            return self.cpus
    ...
    get_guest_host_cpuflags = locals()[f"get_guest_host_cpuflags_{qcver}"]
    get_all_qemu_flags = locals()[f"get_all_qemu_flags_{qcver}"]
    get_cpu_models = locals()[f"get_cpu_models_{qcver}"]
    ...
    def disable_cpu(vm_session, cpu, disable=True):
        ...
        cpu_state = vm_session.cmd_output(f"cat {cpu_online}").strip()
        if disable and cpu_state == "1":
            vm_session.cmd(f"echo 0 > {cpu_online}")
            test.log.debug("Guest cpu %d is disabled.", cpu)
        elif cpu_state == "0":
            vm_session.cmd(f"echo 1 > {cpu_online}")
    ...
    def install_cpuflags_test_on_vm(vm, dst_dir):
        ...
        session.cmd("sync")
        session.cmd("cd {}; make EXTRA_FLAGS='';".format(os.path.join(dst_dir, "src")))
        session.cmd("sync")
        session.close()
        ...
        for f in flags:
            try:
                for tc in cpu.kvm_map_flags_to_test[f]:
                    session.cmd(
                        "{}/cpuflags-test --{}".format(os.path.join(path, "src"), tc)
                    )
                pass_Flags.append(f)
    ...
    class test_qemu_cpu_model(MiniSubtest):
        def test(self):
            if qcver == "legacy":
                ...
                if missing:
                    test.fail(
                        f"CPU models {missing} are not in output "
                        f"'{cmd}' of command \n{result.stdout}"
                    )
            else:
                test.cancel("New qemu does not support -cpu " f"?model. ({qcver})")
    ...
    class test_qemu_dump(MiniSubtest):
        def test(self):
            if qcver == "legacy":
                ...
                test.cancel("New qemu does not support -cpu " f"?dump. ({qcver})")
    ...
    class test_qemu_cpuid(MiniSubtest):
        def test(self):
            if qcver == "legacy":
                ...
                test.cancel("New qemu does not support -cpu " f"?cpuid. ({qcver})")
    ...
    class test_boot_cpu_model_and_additional_flags(Test_temp):
        def test(self):
            ...
            if Flags[1] & guest_flags:
                test.fail(f"Some flags do not work: {str(Flags[1])}")
    ...
    class test_boot_warn_with_host_unsupported_flags(MiniSubtest):
        def test(self):
            ...
            try:
                ...
                if fwarn_flags:
                    test.fail(
                        "Qemu did not warn the use of " f"flags {str(fwarn_flags)}"
                    )
    ...
    class test_migration_with_additional_flags(Test_temp):
        def test(self):
            ...
            test.assertTrue(flags[0], f"No cpuflags passed the check: {str(flags)}")
            test.assertFalse(flags[1], f"Some cpuflags failed the check: {str(flags)}")
            ...
            cmd = "nohup {}/cpuflags-test --stress  {}{} &".format(
                os.path.join(install_path, "src"),
                smp,
                cpu.kvm_flags_to_stresstests(flags[0]),
            )
                    ...
                    def check_worker(mig_data):
                        ...
                        if not_disabled:
                            test.fail(
                                ...
                                f"srchost:{disable_cpus}, Cpus not "
                                f"disabled on dsthost:{not_disabled}"
                            )
    ...
    if test_type in locals():
        ...
        if params.get("cpu_model"):
            ...
            if failed != []:
                test.fail(f"Test of cpu models {str(failed)} failed.")
    else:
        test.fail(f"Test group '{test_type}' is not defined in" " cpuflags test")
