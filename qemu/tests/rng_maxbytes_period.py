...
def run(test, params, env):
        ...
        if max_bytes != "0":
            ...
            try:
                ...
                if error_info not in e.output:
                    test.fail(
                        f"Expected error info '{error_info}' is not reported, "
                        f"output: {e.output}"
                    )
            return
    ...
    if max_bytes == "0":
        try:
            ...
            test.fail(f"Unexpected dd result, status: {s}, output: {o}")
    else:
        s, o = session.cmd_status_output(read_rng_cmd, timeout=read_rng_timeout)
        if s:
            test.error(o)
        ...
        if float(data_rate.group(1)) > expected_data_rate * 1.1:
            test.error(
                ...
                f"data rate: {data_rate.group(1)} kB/s, max-bytes: {max_bytes}, period: {period}"
            )
