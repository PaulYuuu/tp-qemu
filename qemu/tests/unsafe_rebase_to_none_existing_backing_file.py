...
def run(test, params, env):
    ...
    def _verify_unsafe_rebase(img):
        ...
        res = process.run(
            f"{img.image_cmd} check {img.image_filename}", ignore_status=True
        )
        expected = [
        ...
        for msg in expected:
            if msg not in res.stderr_text:
                test.fail(f"The {img.base_image_filename} should not exist.")
