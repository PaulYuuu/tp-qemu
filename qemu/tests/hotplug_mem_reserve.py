...
def run(test, params, env):
    ...
    size_target_mem = params[f"size_mem_{mem_name}"]
    hp_target = int(float(normalize_data_size(size_target_mem, "K")) / hp_size) + int(
        hp_total
    )
    process.system(f"echo {hp_target} > /proc/sys/vm/nr_hugepages", shell=True)
    ...
    if not wait_for(lambda: next(rsvd_is_stable), timeout, 5, 3):
        test.error(f"HugePages_Rsvd is not stable in {timeout}s")
    try:
        ...
        if params["reserve_mem"] == "yes":
            ...
            plug_size = params[f"size_mem_{mem_name}"]
            numa_size = params["size_mem_{}".format(params["mem_devs"])]
