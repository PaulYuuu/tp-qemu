...
def run(test, params, env):
    ...
    def fail_no_key(qmp_dict, key):
        ...
        if key not in qmp_dict:
            test.fail(f"'{key}' key doesn't exist in dict ('{str(qmp_dict)}')")
    ...
    def check_dict_key(qmp_dict, key, keytype):
        ...
        if not isinstance(qmp_dict[key], keytype):
            test.fail(
                f"'{key}' key is not of type '{keytype}', it's '{type(qmp_dict[key])}'"
            )
    ...
    def check_str_key(qmp_dict, keyname, value=None):
        ...
        if value and value != qmp_dict[keyname]:
            test.fail(
                f"'{keyname}' key value '{str(qmp_dict[keyname])}' should be '{str(value)}'"
            )
    ...
    def check_key_is_int(qmp_dict, key):
        ...
        try:
            ...
            test.fail(f"'{key}' key is not of type int, it's '{type(qmp_dict[key])}'")
    ...
    def check_success_resp(resp, empty=False):
        ...
        if empty and len(resp["return"]) > 0:
            test.fail(f"success response is not empty ('{str(resp)}')")
    ...
    def check_error_resp(resp, classname=None, datadict=None):
        ...
        if classname and resp["error"]["class"] != classname:
            test.fail(
                "got error class '{}' expected '{}'".format(
    ...
    def test_id_key(monitor):
        ...
        for id_key in (
            ...
            if resp["id"] != id_key:
                test.fail(
                    "expected id '{}' but got '{}'".format(str(id_key), str(resp["id"]))
                )
    ...
    def test_good_input_obj(monitor):
        ...
        resp = monitor.cmd_obj(
            {"id": id_key, "execute": "query-version", "arguments": {}}
        )
        check_success_resp(resp)
        check_str_key(resp, "id", id_key)
    ...
    def argument_checker_suite(monitor):
        ...
        for arg in ({}, [], True, "foo"):
            ...
            check_error_resp(
                resp, "InvalidParameterType", {"name": "val", "expected": "int"}
            )
        ...
        for arg in ({}, [], True, "foo"):
            ...
            check_error_resp(
                resp, "InvalidParameterType", {"name": "value", "expected": "number"}
            )
        ...
        check_error_resp(
            resp, "PropertyNotFound", {"device": "e1000", "property": "foo"}
        )
