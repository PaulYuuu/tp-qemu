...
def run(test, params, env):
    ...
    def dump_catch_data(session, dump_log, catch_reg):
        ...
        """
        dump_info = session.cmd_output(f"cat {dump_log}")
    ...
    def check_arp_info(session, entry, vm, match_mac=None):
        ...
        if not arp_entries:
            test.error(f"Can not find arp entry in {vm.name}: {arp_info}")

        if not re.findall(match_string, arp_entries[0], re.I):
            test.fail(
                "Can not find the mac address"
                f" {match_mac} of {vm.name} in arp"
                f" entry {arp_entries[0]}"
            )
    ...
    def file_transfer(sessions, addresses, timeout):
        ...
        scp_cmd = (
            ...
            rf"-P {port} /tmp/copy_file {username}@\[{addresses[1]}\]:/tmp/copy_file"
        )
        ...
        if new_md5 != ori_md5:
            test.fail(
                ...
                f"original is {ori_md5} and the new file"
                f" is: {new_md5}"
            )
    ...
    def nc_connect_test(
        ...
        try:
            ...
            if utils_misc.wait_for(
                ...
                0,
                2,
                text=f"Wait '{nc_protocol}' connect",
            ):
                nc_connect = True
            if nc_connect == drop_flow:
                err_msg = f"Error, '{nc_protocol}' "
            ...
            for session in sessions:
                ...
                session.cmd(f"{clean_cmd} {nc_log}", ignore_all_errors=True)
    ...
    def remove_plus_items(open_flow_rules):
        ...
        for plus_item in plus_items:
            open_flow_rules = re.sub(f"{plus_item}=.*?,", "", open_flow_rules)
    ...
    if not utils_net.ovs_br_exists(br_name):
        test.cancel(f"{br_name} isn't an openvswith bridge")
    ...
    f_base_options = f"{f_protocol},nw_src={addresses[0]},nw_dst={addresses[1]}"
    for session in sessions:
        session.cmd(
    ...
    try:
        for drop_flow in [True, False]:
            ...
            error_context.context(f"Do {f_command} {f_options} on {br_name}")
            ...
            if not acl_rules_check(acl_rules, f_options):
                test.fail("Can not find the rules from" f" ovs-ofctl: {acl_rules}")

            error_context.context(f"Run tcpdump in guest {vms[1].name}", test.log.info)
        ...
        if not removed_rule or not acl_rules_check(removed_rule[0], f_options):
            failed_msg.append(f"Failed to delete {f_options}")
        if bg_ping_session:
            ...
            if not bg_ping_ok[0]:
                failed_msg.append(
                    ...
                    f"background ping: {bg_ping_ok[1]}"
                )
