...
def run(test, params, env):
    ...
    mkdir_cmd = session.cmd(f"mkdir -p {guest_dir}")
    src_rpm = "kernel-" + kernel_v.rsplit(".", 1)[0] + ".src.rpm"
    linux_name = "linux-" + kernel_v.rsplit(".", 1)[0]
    download_rpm_cmd = f"cd {guest_dir} && " + params["download_rpm_cmd"] % src_rpm
    uncompress_cmd_src = f"cd {guest_dir} && " + params["uncompress_cmd_src"]
    uncompress_cmd = f"cd {guest_dir} && " + params["uncompress_cmd"]
    test_dir = guest_dir + linux_name + params["test_dir"]
    compile_cmd = f"cd {test_dir} && " + params["compile_cmd"]
    run_cmd = f"cd {test_dir} && " + params["run_cmd"]
