...
def run(test, params, env):
    ...
    if (qom_gap < 1) or (qom_gap > 2):
        test.fail(
            ...
            f"qom-get result before change guest's RTC time: {qom_st1}, "
            f"qom-get result after change guest's RTC time: {qom_st2}"
        )
    ...
    if (hwclock_st1 - hwclock_st2 - float(time_forward)) > float(drift_threshold):
        test.fail(
            "Unexpected hwclock drift, " f"hwclock: current guest time={hwclock_st2}s"
        )
