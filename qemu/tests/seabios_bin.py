...
def run(test, params, env):
    ...
    for value in bin_dict.values():
        if value not in output and value != bin_file_skip:
            test.fail(f"{value} is not available")
    ...
    for machine in machine_types:
        error_context.context(
            f"Check bin file with machine type: {machine}", test.log.info
        )
        for key in bin_dict:
            if key in machine:
            ...
            test.error(f"Uncertain which bin file in use for machine type: {machine}")
        ...
        if bin_file not in info_roms:
            test.fail(f"Checking bin file fails with {machine}, info roms: {info_roms}")
        vm.destroy()
