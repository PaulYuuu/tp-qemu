...
class UEFIShellTest:
    ...
    def setup(self, under_fs0):
        ...
        for cdrom in params.objects("cdroms"):
            boot_index = params.get(f"boot_index_{cdrom}")
            if boot_index is not None:
                params[f"boot_index_{cdrom}"] = int(boot_index) + 1
        for image in params.objects("images"):
            params[f"image_boot_{image}"] = "no"
        params["cdroms"] = "{} {}".format("uefishell", params["cdroms"])
    ...
    def copy_uefishell(self):
        ...
        uefishell_dst_path = f"images/{uefishell_filename}"
        ...
        if not os.path.exists(uefishell_dst_path):
            cp_command = f"cp -f {uefishell_src_path} {uefishell_dst_path}"
    ...
    def copy_secureboot_pk_kek(self, pk_kek_filename):
        ...
        pk_kek_dst_path = f"images/{pk_kek_filename}"
        pk_kek_dst_path = utils_misc.get_path(data_dir.get_data_dir(), pk_kek_dst_path)
        cp_command = f"cp -f {pk_kek_src_path} {pk_kek_dst_path}"
        process.system(cp_command)
    ...
    def send_command(self, command, check_result=None, interval=0.5):
        ...
        if not re.search(last_error, env_var, re.S):
            self.test.fail(
                f"Following errors appear {output} when running command {command}"
            )
        if check_result:
            value = []
            for result in check_result.split(", "):
                if not re.findall(result, output, re.S):
                    self.test.fail(
                        f"The command result is: {output}, which does not"
                        f" match the expectation: {result}"
                    )
                else:
...
def run(test, params, env):
    ...
    def handle_memory_map(output):
        ...
        for value in attribute_values:
            if params["attribute_value"] != value:
                test.fail(
                    "The last column should be '{}' for all "
                    "RT_Code entries. The actual value is '{}'".format(
        ...
        if re.search(params["adjacent_rt_code_lines"], output[0], re.M):
            test.fail(
                ...
                f" entry. The command output is {output[0]}"
            )
    ...
    def handle_smbiosview(output):
        ...
        if not smbios_version:
            test.fail(
                "Failed to find smbios version. " f"The command output is {output[0]}"
            )
        bios_version = re.findall(params["bios_version"], output[0], re.S)
        if not bios_version:
            test.fail(
                "Failed to find bios version. " f"The command output is {output[0]}"
            )
        ...
        if not bios_release_date:
            test.fail(
                ...
                f"The command output is {output[0]}"
            )
        ...
        if (
            ...
            test.fail(
                ...
                f"bios_version is {bios_version[0]}, the date from bios_release_date "
                f"is {bios_release_date[0]}."
            )
    ...
    for scenario in test_scenarios.split():
        command = params[f"command_{scenario}"]
        if params.get("command_{}_{}".format(scenario, "args")):
            func_name = params["command_{}_{}".format(scenario, "args")]
            command += eval(func_name)
        check_result = params.get(f"check_result_{scenario}")
        output = uefishell_test.send_command(command, check_result, time_interval)
        if params.get(f"{scenario}_output_handler"):
            func_name = params[f"{scenario}_output_handler"]
