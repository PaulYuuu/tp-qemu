...
def run(test, params, env):
    ...
    if params["nettype"] != "user":
        for nic in vm.virtnet:
            process.system(f"ifconfig {nic.ifname} down")
    ...
    if not result:
        test.fail(
            ...
            f"'{bootorder}', actual result: '{output}'"
        )
