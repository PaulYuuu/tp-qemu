...
def run(test, params, env):
    ...
    def is_dev_used_by_qemu(dev_file, vm_pid):
        ...
        """
        lsof_cmd = f"lsof {dev_file}"
        output = process.system_output(lsof_cmd, ignore_status=True).decode()
        return re.search(rf"\s+{vm_pid}\s+", output, re.M)
    ...
    if os_type == "windows":
        ...
        if not system.file_exists(session, rng_dst):
            ...
            session.cmd(f"copy {rng_src} {rng_dst} /y")
    else:
        ...
        try:
            ...
            err = f"{verify_cmd} timeout, pls check if it's a product bug"
            test.fail(err)

        if not re.search(rf"{driver_name}", output, re.M):
            msg = "Verify device driver failed, "
            msg += f"guest report driver is {output}, "
            msg += f"expect is '{driver_name}'"
            test.fail(msg)
    ...
    if params.get("test_duration"):
        ...
        while (time.time() - start_time) < float(params.get("test_duration")):
            ...
            if len(re.findall(rng_data_rex, output, re.M)) < 2:
                test.fail(f"Unable to read random numbers from guest: {output}")
    else:
        output = session.cmd_output(read_rng_cmd, timeout=read_rng_timeout)
        if len(re.findall(rng_data_rex, output, re.M)) < 2:
            test.fail(f"Unable to read random numbers from guest: {output}")
