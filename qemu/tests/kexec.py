...
def run(test, params, env):
    ...
    def install_new_kernel():
        ...
        try:
            ...
            test.error(f"Failed to install a new kernel in guest: {detail}")
    ...
    for kernel in kernels:
        ...
        if cur_kernel_version not in kernel:
            new_kernel = kernel[7:]
    if not new_kernel:
        test.error("Could not find new kernel, " f"command line output: {output}")
    msg = f"Reboot to kernel {new_kernel} through kexec"
    ...
    if kernel.strip() != new_kernel.strip():
        test.fail(f"Fail to boot to kernel {new_kernel}, current kernel is {kernel}")
    if "yes" in check_x2apic:
        check_x2apic_flag()
    session.close()
