...
def run(test, params, env):
    ...
    def _compare_images(img1, img2, strict=False):
        ...
        if strict:
            ...
            if res.exit_status != 0:
                test.fail(f"qemu-img compare error: {res.stderr_text}.")
            if "Images are identical" not in res.stdout_text:
                test.fail(
                    f"{img1.image_filename} and {img2.image_filename} are not identical."
                )
    ...
    if not c_qit.check_file(file, md5):
        test.fail(
            f"The file {file}'s md5 on initial image and" " target file are different."
        )
    c_qit.destroy_vm()
