...
def run(test, params, env):
    ...
    error_context.context(f"numa info in monitor: {numa_monitor!r}", test.log.info)
    ...
    if os_type == "linux":
        ...
        error_context.context(f"numa info in guest: {numa_guest!r}", test.log.info)
        ...
        MemTotal = float(normalize_data_size(f"{MemTotal} KB"))
        error_context.context(f"MemTotal in guest os is {MemTotal} MB", test.log.info)
    ...
    for nodenr, node in enumerate(numa_guest):
        ...
        if mdev:
            mdev = mdev.split("-")[1]
            size = float(normalize_data_size(params.get(f"size_{mdev}")))
        else:
