...
def run(test, params, env):
    ...
    def unpack_stress_pkg():
        ...
        process.system(f"tar -xzvf {archive_path} -C {stress_inst_dir}")
    ...
    def copy_base_vm_image():
        ...
        for vm_name in vms_list:
            dst_filename = os.path.join(dst_dir, f"{vm_name}.{src_format}")
    ...
    def configure_images_copied():
        ...
        for vm_name in vms_list:
            params[f"images_{vm_name}"] = vm_name
            image_name = f"image_name_{vm_name}_{vm_name}"
            params[image_name] = f"images/{vm_name}"
            params[f"remove_image_{vm_name}"] = "yes"
    ...
    def wait_for_shutdown_all_vms(vms, sessions):
        ...
        for vm, session in zip(vms, sessions):
            ...
            if not vm.wait_for_shutdown():
                test.fail(f"Failed to shutdown {vm.name}.")
