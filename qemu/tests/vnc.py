    ...
    def hand_shake(self, password=None):
        ...
        if self.rfb_version != "3.3":
            ...
            if auth == 0:
                ...
                (auth_type,) = struct.unpack(f"!{auth}B", rec)
...
def run(test, params, env):
    ...
    for rfb_version in rfb_version_list:
        error_context.base_context(f"Test with guest RFB version {rfb_version}")
        ...
        error_context.context(
            "Connect to VNC server after setting password" f" to '{password}'"
        )
