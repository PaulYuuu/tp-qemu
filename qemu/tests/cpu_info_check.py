...
def run(test, params, env):
    ...
    query_cmd = f"{qemu_binary} -cpu ? | awk '{{print $2}}'"
    qemu_binary_output = (
    ...
    if model_name != vm.cpuinfo.model:
        test.fail(
            "Command query-cpu-model-expansion return" f" wrong model: {model_name}"
        )
    ...
    for flag in cpu.CPU_TYPES_RE.get(model_name).split(","):
        ...
        for f in flags:
            ...
            test.fail(f"Check cpu model props failed, {flag} is not True")
