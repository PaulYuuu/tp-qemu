...
def run(test, params, env):
    ...
    def find_disk(session, cmd):
        ...
        if params.get("os_type") == "linux":
            ...
            test.cancel("Unsupported OS type '{}'".format(params.get("os_type")))
    ...
    def verify_deleted_event(device_list, timeout=120):
        ...
        for dev in device_list:
            ...
            if not utils_misc.wait_for(
                ...
                test.fail(
                    f"Failed to get deleted event of {dev_qid} "
                    f"during {timeout} sec."
                )
    ...
    def verify_unplug_devices_by_qtree(device_list, timeout=30):
        ...
        for dev in device_list:
            if not utils_misc.wait_for(
                ...
                test.error(f"The {dev} is still in qtree after unplugging.")
    ...
    def rw_disk_in_guest(session, plug_disks, iteration):
        ...
        for disk in plug_disks:
            ...
            if status:
                test.fail("Check for block device rw failed." f"Output: {output}")
