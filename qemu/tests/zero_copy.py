...
def run(test, params, env):
    ...
    def zerocp_enable_status():
        ...
        cmd_status = process.system(f"grep 1 {para_path}", ignore_status=True)
        if cmd_status:
            return False
        else:
            return True
    ...
    if status:
        err_msg = f"Run ping {guest_ip} failed, after set zero copy"
        test.error(err_msg)
    elif utils_test.get_loss_ratio(output) == 100:
        err_msg = f"All packets lost during ping guest {guest_ip}."
        test.fail(err_msg)
