...
def ping_result_check(test, loss_ratio, same_vlan):
    ...
    if same_vlan and loss_ratio > 0:
        ...
        msg += f"Loss ratio is {loss_ratio}"
        test.fail(msg)
...
def ping(test, os_type, match_error, dest, count, session, same_vlan):
    ...
    if os_type == "linux":
        status, output = utils_test.ping(
            dest, count, timeout=int(count) * 1.50, session=session
        )
...
def run(test, params, env):
    ...
    try:
        ...
        test.fail(f"Fail to create ovs bridge {bridge_name}")
    ...
    try:
        ...
        for vm_name in vms:
            ...
            create_port_cmd = f"ovs-vsctl set Port {ifname} tag={vlan}"
            try:
                ...
                err += f"on bridge {bridge_name}."
                err += f" Command: {create_port_cmd}, "
                err += f"output: {output}."
                test.fail(err)

            session_ctl = vm.wait_for_serial_login(timeout=login_timeout)
            if os_type == "linux":
                txt = f"Stop NetworkManager service in guest {vm_name}."
                test.log.info(txt)
                session_ctl.cmd(params["stop_network_manager"], timeout=120)

            mac = vm.get_mac_address()
            txt = f"Set guest {vm_name} mac {mac} IP to {guest_ip}"
            error_context.context(txt, test.log.info)
        ...
        txt = f"Ping between two guests in same vlan. {vms[0]} -> {vms[1]}"
        error_context.context(txt, test.log.info)
        ping(
            test,
            os_type,
        ...
        txt = f"Run netserver in VM {vms[0]}"
        error_context.context(txt, test.log.info)
        ...
        if status != 0:
            ...
            err += f" Command output {output}"
            test.error(err)

        txt = f"Run netperf client in VM {vms[1]}"
        error_context.context(txt, test.log.info)
        ...
        if params.get("file_transfer_test", "yes") == "yes":
            ...
            txt = f"Create {filesize} MB file {file_path} in {vms[0]}"
            error_context.context(txt, test.log.info)
            ...
            txt = f"Transfer file {file_path} between guests in same "
            txt += f"vlan. {vms[0]} -> {vms[1]}"
            error_context.context(txt, test.log.info)
            ...
            f"file_transfer_from_{ips[0]}_to_{ips[1]}.log"
