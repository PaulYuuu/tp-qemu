...
def run(test, params, env):
    ...
    if params.get("prewhql_install") == "yes":
        ...
        process.system(f"mkisofs -J -o {virtio_iso} /tmp/virtio_win")
    ...
    for driver in drivers_install:
        ...
        if params.get("check_info") == "yes":
            ...
            process.system(f"mount -o loop {iso_path} {mount_point}")
            ...
            inf.close()
            process.system(f"umount {mount_point}")
            ...
            for i in params_driver.get("check_cmd").split(";"):
                ...
                cmd_c = re.sub(
                    f"DRIVER_PATTERN_{cmd_n}",
                    params_driver.get(f"info_pattern_{cmd_n}", ""),
                    cmd_c,
                )
    ...
    for driver in drivers_install:
        error_context.context(f"Install drivers {driver}", test.log.info)
    ...
    error_log = open(f"{test.resultsdir}/error_log", "w")
    fail_flag = False
    ...
    for driver in drivers_install:
        error_log.write(f"For driver {driver}:\n")
        if isinstance(check_str[driver], dict):
            for i in check_str[driver]:
                ...
                if not re.findall(check_str[driver][i], output, re.I):
                    fail_flag = True
                    fail_log += f" {driver}"
                    fail_log += f"({i}) is not right; "
                    error_log.write(f"inf:\t{check_str[driver][i]}\n")
            ...
            if not re.findall(check_str[driver], output, re.I):
                fail_flag = True
                fail_log += f" {driver}"
                error_log.write(f"Check command output: {output}\n")
