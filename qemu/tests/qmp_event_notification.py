...
def run(test, params, env):
    ...
    pre_event_cmd_options = eval(
        "dict({})".format(params.get("pre_event_cmd_options", ""))
    )
    event_cmd = params.get("event_cmd")
    event_cmd_options = eval("dict({})".format(params.get("event_cmd_options", "")))
    ...
    post_event_cmd_options = eval(
        "dict({})".format(params.get("post_event_cmd_options", ""))
    )
    ...
    if qmp_num > 0:
        test.fail(f"Did not receive qmp {event_check} event notification")
