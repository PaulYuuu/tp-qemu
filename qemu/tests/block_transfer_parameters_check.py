...
def run(test, params, env):
    ...
    def _verify_transfer_info(host_info, guest_info):
        errmsg = f"The guest get unexpected transfer {guest_info}"
    ...
    def _get_target_devices(trans_type, size=None):
        ...
        if size:
            cond += f' && $3=="{size}"'
        cmd = f"lsblk -Spo 'NAME,TRAN,SIZE' |awk '{{if({cond}) print $1}}'"
        logger.debug(cmd)
        status, output = process.getstatusoutput(cmd)
        devs_str = output.strip().replace("\n", " ")
        if devs_str:
            cmd = f"lsblk -Jpo 'NAME,HCTL,SERIAL,TRAN,FSTYPE,WWN' {devs_str}"
            status, output = process.getstatusoutput(cmd)
    ...
    try:
        ...
        if tran_type == "iscsi":
            ...
            time.sleep(2)
            logger.debug(_run_cmd(f"lsblk -JO {dev_name}"))

        target_devs = _get_target_devices(tran_type, params["image_size"])
        if not len(target_devs):
            if tran_type == "fc":
                test.cancel("No FC device:{}".format(params["image_size"]))
            else:
                test.error("No ISCSI device:{}".format(params["image_size"]))
