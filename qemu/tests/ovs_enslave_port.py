...
def run(test, params, env):
    ...
    error_context.context(f"OVS bridge {new_br_name} created.", test.log.info)
    ...
    try:
        ...
        if "already exists on bridge" not in e.result.stderr_text:
            test.fail(
                f"Port {ports[0]} should not be enslaved to another bridge."
                f" Output: {e.result.stderr_text}"
            )
    else:
        test.fail(
