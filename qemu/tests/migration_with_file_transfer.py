...
def run(test, params, env):
    ...
    host_path = f"/tmp/file-{utils_misc.generate_random_string(6)}"
    host_path_returned = f"{host_path}-returned"
    ...
    if mig_protocol == "exec":
        mig_file = os.path.join(
            test.tmpdir, f"tmp-{utils_misc.generate_random_string(8)}"
        )

    try:
        process.run(f"dd if=/dev/urandom of={host_path} bs=1M count={file_size}")
        ...
        if orig_hash != returned_hash:
            test.fail(
                f"Returned file hash ({returned_hash}) differs from "
                f"original one ({orig_hash})"
            )
        error_context.context()
