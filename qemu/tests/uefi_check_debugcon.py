...
def run(test, params, env):
    ...
    if not utils_misc.wait_for(lambda: bg.is_alive, timeout):
        test.fail(f"Failed to start command: '{trace_record_cmd}'")
    try:
        vm.create()
        vm.verify_alive()
        vm.destroy()
        ...
        if not utils_misc.wait_for(lambda: not check_trace_process(), timeout, 30, 3):
            test.fail(
                f"Failed to stop command: '{stop_trace_record}' after {timeout} seconds."
            )
        pio_read_counts = int(
            process.run(check_pio_read, shell=True).stdout.decode().strip()
        )
        err_str = "pio_read counts should be greater than 0. "
        err_str += f"But the actual counts are {pio_read_counts}."
        ...
        if params.get("ovmf_log"):
            ...
            err_str += f"But the actual counts are {pio_write_counts}."
