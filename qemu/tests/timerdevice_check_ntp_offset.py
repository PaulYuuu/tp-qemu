...
def run(test, params, env):
    ...
    def clean_tmp_file():
        if not session.cmd_status(f"dir {ntp_dst_path}"):
            session.cmd(f"rd /s /q {ntp_dst_path}")
        ntp_install_path = params["ntp_install_path"]
        ntp_uninstall_cmd = params["ntp_uninstall_cmd"]
        if not session.cmd_status(f"dir {ntp_install_path}"):
    ...
    try:
        for _ in range(params.get_numeric("nums")):
            ...
            if ntp_offset > 100:
                test.fail(f"The ntp offset {ntp_offset} is larger than 100ms")
    finally:
        clean_tmp_file()
