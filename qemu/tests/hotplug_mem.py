...
class MemoryHotplugSimple(MemoryHotplugTest):
    def run_sub_test(self):
        ...
        if self.params.get("sub_type"):
            step = "Run sub test '{}' {} {} memory device".format(
    ...
    def restore_memory(self, pre_vm, post_vm):
        ...
        func = getattr(self, f"{operation}_memory")  # pylint: disable=E0606
    ...
    def start_test(self):
        ...
        func = getattr(self, f"{operation}_memory")
        if not callable(func):
            self.test.error(f"Unsupported memory operation '{operation}'")
