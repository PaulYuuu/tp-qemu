...
class BlockdevIncBackupPullModeDiff(blockdev_base.BlockdevBaseTest):
    ...
    def _init_arguments_by_params(self, tag):
        ...
        self.fleecing_full_backups.append(f"drive_{bk_tags[0]}")
        self.fleecing_inc_backups.append(f"drive_{bk_tags[1]}")

        # bitmaps
        self.full_backup_bitmaps.append(f"full_bitmap_{tag}")
        self.before_2nd_inc_bitmaps.append(f"before_2nd_inc_bitmap_{tag}")
        self.merged_bitmaps.append(f"merged_bitmap_{tag}")
        self.inc_backup_bitmaps.append(f"inc_bitmap_{tag}")
        self.params[f"nbd_export_bitmaps_{bk_tags[1]}"] = self.merged_bitmaps[-1]
        ...
        fullbk = self.params[f"client_image_{bk_tags[0]}"]
        ...
        incbk = self.params[f"client_image_{bk_tags[1]}"]
        ...
        self.params[f"image_backup_chain_{tag}"] = bk_tags[0]
    ...
    def hotplug_inc_backup_images(self):
        for idx, tag in enumerate(self.src_img_tags):
            self.params[f"image_backup_chain_{tag}"] = self.inc_backup_tags[idx]
    ...
    def rebase_inc_onto_full(self):
        ...
        for i, tag in enumerate(self.inc_backup_tags):
            incbk = self.params[f"client_image_{tag}"]
            fullbk = self.params[f"client_image_{self.full_backup_tags[i]}"]
            image_params = self.params.object_params(incbk)
            image_params["image_chain"] = f"{fullbk} {incbk}"
