...
class QSDBackupPullMode(blockdev_base.BlockdevBaseTest):
    ...
    def _init_arguments_by_params(self, tag):
        ...
        self.full_backups.append(f"drive_{bk_tags[0]}")
        self.full_backup_bitmaps.append(f"full_bitmap_{tag}")

        # fleecing image used for inc backup, to be exported by nbd
        self.inc_backups.append(f"drive_{bk_tags[1]}")
        self.inc_backup_bitmaps.append(f"inc_bitmap_{tag}")

        # nbd export image used full backup
        nbd_image = self.params[f"nbd_image_{bk_tags[0]}"]
        ...
        nbd_image = self.params[f"nbd_image_{bk_tags[1]}"]
        disk = qemu_storage.QemuImg(
            self.params.object_params(nbd_image), None, nbd_image
        )
        ...
        client_image = self.params[f"client_image_{bk_tags[0]}"]
        ...
        client_image = self.params[f"client_image_{bk_tags[1]}"]
        disk = self.source_disk_define_by_params(
            self.params.object_params(client_image), client_image
        )
        disk.create(self.params)
        ...
        self.disabled_bitmaps.append(f"full_bitmap_{tag}")
    ...
    def init_nbd_exports(self):
        def _init_nbd_exports(tag):
            ...
            self.full_backup_nbd_objs.append(
                InternalNBDExportImage(self.qsd, self.params, bk_tags[0])
            )

            self.params[f"nbd_export_bitmaps_{bk_tags[1]}"] = f"full_bitmap_{tag}"
            self.inc_backup_nbd_objs.append(
                InternalNBDExportImage(self.qsd, self.params, bk_tags[1])
            )
    ...
    def cancel_backup_jobs(self):
        for job_id in self.backup_jobs:
            arguments = {"id": job_id}
            self.qsd.monitor.cmd("job-cancel", arguments)
    ...
    def do_full_backup(self):
        ...
        self.backup_jobs = [job["id"] for job in job_utils.query_jobs(self.qsd)]
    ...
    def restart_vm_with_backup_images(self):
        ...
        self.params["qsd_images_qsd1"] = " ".join(
            [i.tag for i in self.inc_backup_client_images]
        )
        for addr, inc in enumerate(self.inc_backup_client_images, start=15):
            add_vubp_into_boot(inc.tag, self.params, addr)
        self.start_qsd()
    ...
    def verify_data_files(self):
        ...
        try:
            for tag, info in disks_info.items():
                ...
                self.test.log.info("Check %s should not exist", file_path)
                s, o = session.cmd_status_output(cat_cmd)
                if s == 0:
                    self.test.fail(f"File ({non_existed_files[tag]}) exists")
                elif "No such file" not in o.strip():
                    self.test.fail(f"Unknown error: {o}")
        finally:
            if session:
                session.close()
    ...
    def prepare_test(self):
        self.start_qsd()
        add_vubp_into_boot(self.params["source_images"], self.params)
        super().prepare_test()
...
def run(test, params, env):
    ...
    test steps:
        1. export data disk with qsd, boot VM with the exoorted disk
        ...
        17. destroy vm, destroy qsd, restart qsd with back2 as exported image,
        then restart vm with the exported image.
    ...
    """
    inc_test = QSDBackupPullMode(test, params, env)
    inc_test.run_test()
