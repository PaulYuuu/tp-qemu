...
def run(test, params, env):
    ...
    error_context.context(f"Format disk {disk_index_list[0]} in guest.", test.log.info)
    ...
    params[f"image_readonly_{data_img}"] = "yes"
    params[f"force_create_image_{data_img}"] = "no"
    ...
    if not o.find("write protect"):
        test.fail(f"Write in readonly disk should failed\n. {o}")
