...
def run(test, params, env):
    ...
    if real_difference_ > tolerance:
        test.error(
            f"Unexpected timedrift between host and guest, host time: {host_epoch_time_},"
            f"guest time: {guest_epoch_time_}"
        )

    error_context.context(f"Set host system time back {seconds_to_back} s")
    ...
    try:
        ...
        if abs(real_difference - time_difference) >= tolerance:
            test.fail(
                f"Unexpected timedrift between host and guest, host time: {host_epoch_time},"
                f"guest time: {guest_epoch_time}"
            )
    finally:
        time.sleep(10)
        error_context.context(
