...
def _stop_service(test, params, session, service):
    ...
    if s:
        test.error("Failed to query service list, " f"status={s}, output={o}")
    service_item = re.search(rf"SERVICE_NAME:\s+{service}", o, re.I | re.M)
    if not service_item:
        return
    ...
    if s:
        test.error(
            f"Failed to get status for service: {service}, " f"status={s}, output={o}"
        )
...
def run(test, params, env):
    ...
    for thread_id in vcpu_ids:
        ...
        if host_cpu_usage > thread_cpu_level:
            test.fail(
                f"The cpu usage of thread {thread_id} is {host_cpu_usage}"
                f" > {thread_cpu_level}"
            )
