...
def convert_params(params, args):
    ...
    params[f"image_name_{name}"] = args.pop("filename")
    params[f"image_size_{name}"] = params["stg_image_size"]
    params[f"remove_image_{name}"] = "yes"
    params[f"boot_drive_{name}"] = "no"
    if params.get(f"image_format_{name}"):
        params[f"image_format_{name}"] = params.get(f"image_format_{name}")
    else:
        params[f"image_format_{name}"] = params.get("image_format")
    if params.get(f"image_iothread_{name}"):
        params[f"image_iothread_{name}"] = params.get(f"image_iothread_{name}")
    else:
        params[f"image_iothread_{name}"] = params.get("image_iothread")
    for key, value in args.items():
        params[f"{convert.get(key, key)}_{name}"] = value
    return params
...
def run(test, params, env):
    ...
    def verify_qtree(params, info_qtree, info_block, qdev):
        ...
        if err:
            ...
            test.fail(f"{err} errors occurred while verifying" " qtree vs. params")

    def _create_params_matrix():
        matrix = {}
        stg_image_name = params["stg_image_name"]
        if not stg_image_name[0] == "/":
            stg_image_name = f"{data_dir.get_data_dir()}/{stg_image_name}"
        ...
        for i in range(len(stg_params)):
            if not stg_params[i].strip():
                continue
            if stg_params[i][-1] == "\\":
                stg_params[i] = f"{stg_params[i][:-1]} {stg_params.pop(i + 1)}"
            if not stg_params[i].strip():
                continue
            ...
            for j in range(len(parm)):
                if parm[j][-1] == "\\":
                    parm[j] = f"{parm[j][:-1]},{parm.pop(j + 1)}"
            matrix[cmd] = parm
        return matrix
    ...
    try:
        ...
        if pid:
            logdir = test.logdir
            process.getoutput(f"gstack {pid} > {logdir}/gstack.log")
            process.getoutput(
                f"timeout 20 strace -tt -T -v -f -s 32 -p {pid} -o {logdir}/strace.log"
            )
        else:
