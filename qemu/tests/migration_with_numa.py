...
def run(test, params, env):
    ...
    def get_nodes_size(size_type="MemTotal", session=None):
        ...
        for node in numa_nodes:
            ...
            nodes_size[node] = float(normalize_data_size(f"{node_size} KB"))
    ...
    for mem_dev in mem_devs:
        ...
        if host_nodes_size[0][1] >= size_mem:
            params[f"host-nodes_mem_{mem_dev}"] = str(host_nodes_size[0][0])
    ...
    if os_type == "linux":
        ...
        mem_total = float(normalize_data_size(f"{mem_total} KB"))
        error_context.context(f"MemTotal in guest os is {mem_total} MB", test.log.info)
        threshold = float(params.get_numeric("mem") - mem_total) + 30
        error_context.context(
            f"The acceptable threshold is: {threshold}", test.log.info
        )
        ...
        for nodenr, node in enumerate(params.objects("guest_numa_nodes")):
            ...
            if mdev:
                mdev = mdev.split("-")[1]
                size = float(normalize_data_size(params.get(f"size_mem_{mdev}")))
    ...
    for mem_dev in mem_devs:
        ...
        if not match:
            test.error(f"Failed to get the mem start address in smaps: {smaps}")
        ...
        if not node_match:
            test.fail(f"Failed to get the bind node in numa_maps: {numa_maps}")
        bind_node = node_match.groups()[0]
        expected_node = memdev_params.get("host-nodes_mem")
        if bind_node != expected_node:
            test.fail(
                f"Host node for memdev {mem_dev} in numa_maps is {bind_node}, while the "
                f"expected is:{expected_node}"
            )
