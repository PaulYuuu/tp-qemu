...
def run(test, params, env):
    ...
    patt = re.compile(rf"{fw_filter}", re.M)
    list_fw = patt.findall(str(o))

    patt = re.compile(rf"{addr_filter}", re.M)
    ...
    if ret:
        test.fail(
            f"ROM '{ret}' is intended to be loaded by the firmware, "
            "but is was also loaded by QEMU itself."
        )
