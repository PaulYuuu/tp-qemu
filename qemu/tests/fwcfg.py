...
def run(test, params, env):
    ...
    try:
        ...
        vm.copy_files_to(dump_zip_file, f"{disk_letter}:\\Memory.dmp.zip")
        ...
        if status:
            test.error(f"unzip dump file failed as:\n{output}")
        ...
        process.system(f"rm {dump_file} {dump_zip_file}", shell=True)
        session.cmd("del {}".format(params["dump_analyze_file"]))
