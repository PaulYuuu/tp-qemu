...
def run(test, params, env):
    ...
    def verify_vlan_table(expect_vlan=None):
        ...
        query_cmd = f"query-rx-filter name={vm.virtnet[0].device_id}"
        ...
        if not expect_vlan:
            ...
            if (
                ...
                test.fail(
                    f"Guest vlan table is not correct, expect: {expect_vlan},"
                    f" actual: {vlan_table}"
                )
        elif vlan_table and vlan_table[0] != int(expect_vlan):
            test.fail(
                f"Guest vlan table is not correct, expect: {expect_vlan}, actual: {vlan_table[0]}"
            )
