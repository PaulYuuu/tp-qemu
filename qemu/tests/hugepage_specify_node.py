...
def run(test, params, env):
    ...
    mem_size = int(normalize_data_size("{}M".format(params["mem"]), "K"))
    idle_node_mem = int(normalize_data_size("{}M".format(params["idle_node_mem"]), "K"))
    ...
    for node_id in node_list:
        error_context.base_context(
            "Check preprocess HugePages Free on host " f"numa node {node_id}.",
            test.log.info,
        )
    ...
    for node_id in node_list:
        error_context.base_context(
            "Specify qemu process only allocate " f"HugePages from node{node_id}.",
            test.log.info,
        )
        params["target_nodes"] = f"{node_id}"
        huge_pages_num = math.ceil(mem_size / hugepage_size)
        params[f"target_num_node{node_id}"] = huge_pages_num
        error_context.context(
            f"Setup huge pages for specify node{node_id}.", test.log.info
        )
        check_list = [_ for _ in idle_node_list if _ != node_id]
        for idle_node in check_list:
            params["target_nodes"] += f" {idle_node}"
            params[f"target_num_node{idle_node}"] = math.ceil(
                idle_node_mem / hugepage_size
            )
            error_context.context(
                f"Setup huge pages for idle node{idle_node}.", test.log.info
            )
        hp_config = test_setup.HugePageConfig(params)
        try:
            hp_config.setup()
            params["qemu_command_prefix"] = f"numactl --membind={node_id}"
            params["start_vm"] = "yes"
            ...
            try:
                ...
                for index in check_list:
                    error_context.base_context(
                        "Check process HugePages Free on host " f"numa node {index}.",
                        test.log.info,
                    )
                    ...
                    if int(node_meminfo[index]["HugePages_Free"]) > hugepages_free:
                        test.fail(
                            ...
                            f"Expect: node{node_id}, used: node{index}."
                        )
            finally:
                vm.destroy()
            ...
            details = str(details)
            if re.findall(rf"Cannot set {huge_pages_num} hugepages", details):
                test.cancel(
