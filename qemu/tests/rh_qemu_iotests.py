...
def run(test, params, env):
    ...
    def retry_command(cmd):
        ...
        raise exceptions.TestError(
            f"{msg} after {max_retry} times retry: {detail}"  # noqa: F821
        )
    ...
    def install_test(build_root):
        ...
        get_rpm_name_cmd = (
            f"rpm -qf {utils_misc.get_qemu_binary(params)} --queryformat={query_format}"
        )
    ...
    def run_test(qemu_src_dir):
        ...
        error_context.context(
            f"running qemu-iotests for image format {image_format}", test.log.info
        )
        ...
        if match:
            iotests_log_file = f"qemu_iotests_{image_format}.log"
    ...
    cmd = f"{utils_misc.get_qemu_binary(params)} -version"
