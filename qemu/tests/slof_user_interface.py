...
def run(test, params, env):
    ...
    def _check_menu_info(menu_info):
        ...
        for i in range(1, int(params["boot_dev_num"]) + 1):
            ...
            pattern = re.compile(
                rf"(\d+)\)\s+{dev_type}(\d+)?\s+:\s+/{hba_type}(\S+)?/{child_bus}@{addr}",
                re.M,
            )
            searched = pattern.search(menu_info)
            if not searched:
                test.fail(
                    "No such item({}) in boot menu list.".format(
                        "->".join(option.split())
                    )
                )
    ...
    def _enter_user_interface(mode):
        ...
        if not o:
            test.fail(f"Failed to enter user interface in {ack_timeout} sec.")
        return o
    ...
    def _f12_user_interface_test():
        ...
        if actual_num != int(dev_num):
            test.fail(f"The number of boot devices is not {dev_num} in menu list.")
        if not utils_misc.wait_for(
            lambda: _send_key(_check_menu_info(menu_list), False), ack_timeout, step=0.0
        ):
            test.fail(
                "Failed to load after selecting boot device " f"in {ack_timeout} sec."
            )
    ...
    def _load_user_interface_test():
        ...
        if not utils_misc.wait_for(
            ...
            test.fail(f"Failed to load after '{keys}' in {ack_timeout} sec.")
    ...
    def _check_serial_log_status():
        ...
        if not utils_misc.wait_for(
            ...
            test.error(f"No found serial log during {file_timeout} sec.")
    ...
    error_context.context(f"Try to log into guest '{vm.name}'.", test.log.info)
    ...
    session.cmd(f"ping {extra_host_ip} -c 5")
