...
def run(test, params, env):
    ...
    def data_mon(session, cmd, timeout):
        try:
            ...
            if re.findall(catch_date % (addresses[1], addresses[0]), str(e)):
                test.fail(f"God! Capture the transfet data:'{str(e)}'")
    ...
    src_file = tmp_dir + f"src-{utils_misc.generate_random_string(8)}"
    ...
    try:
        ...
        if params.get("os_type") == "linux":
            ...
            if status:
                test.error(
                    f"Failed to install wireshark, status={status}, output={output}"
                )
        ...
        sessions[0].cmd(f" {clean_cmd} {src_file} ")
        sessions[1].cmd(f" {clean_cmd} {src_file} ")
        if mon_session:
            mon_session.close()
