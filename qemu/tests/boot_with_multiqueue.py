...
def run(test, params, env):
    ...
    if params["os_type"] == "linux":
        ...
        set_queue_cmd = f"ethtool -L {ifname} combined {queues}"
        ...
        for device in devices.split("\n"):
            ...
            if not find_result:
                test.fail(f"No MSI info in output: {output}")
            msis = int(find_result.group(1))
            if msis != 2 * queues + 2:
                test.fail(f"MSI not correct with output: {output}")
    else:
        # verify driver
        ...
        if cur_queues != queues or msis != 2 * queues + 2:
            test.fail(f"queues not correct with {cur_queues}, expect {queues}")
