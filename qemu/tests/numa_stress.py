...
def run(test, params, env):
    ...
    try:
        ...
        guest_stress_args = f"-a -p -l {int(test_mem)}M"
        ...
        for test_round in range(test_count):
            ...
            error_context.context(
                f"Executing stress test round: {test_round}", test.log.info
            )
            try:
                error_context.context(
                ...
                error_context.context(
                    "Run mem_mapping on host node " f"{numa_node_malloc}.",
                    test.log.info,
                )
                ...
                if node_after == most_used_node and memory_after >= memory_used:
                    ...
                    error_context.context(
                        ...
                        f"{numa_node_malloc} to node {idle_nodes[0]}.",
                        test.log.info,
                    )
                    ...
                    if node_after == most_used_node and memory_after >= memory_used:
                        test.fail(f"Memory still stick in node {numa_node_malloc}")
            finally:
