...
def run(test, params, env):
    ...
    for i in range(1, num_chardev):
        ...
        params["extra_serials"] = "{} {}".format(
            params.get("extra_serials", ""),
            serial_name,
        )
        params[f"serial_type_{serial_name}"] = "virtserialport"
    ...
    try:
        ...
        if not re.search(f"Duplicate (device |)ID '{serial_devices[0]}'", str(e.data)):
    ...
    try:
        ...
        if not (
            f"duplicate property '{char_devices[0]}'" in str(e.data)
            or f"'{char_devices[0]}' already exists" in str(e.data)
        ):
            msg = (
                f"Should fail to hotplug device {char_devices[0]} with error Duplicate"
            )
            test.fail(msg)
    else:
