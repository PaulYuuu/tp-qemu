...
def run(test, params, env):
    ...
    vm.copy_files_to(host_path, f"{guest_path}{source_file}")
    ...
    s, o = session.cmd_status_output(f"{guest_path}{test_bin}")
    session.cmd(params["delete_cmd"])
    if s != 0:
        test.fail(f"rdrand failed with status {s}")
    if params["os_type"] == "linux":
        try:
            if int(float(o)) not in range(-101, 101):
                test.fail(f"rdrand output is {o}, which is not expected")
        except ValueError as e:
            test.fail(f"rdrand should output a float: {str(e)}")
