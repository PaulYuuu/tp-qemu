...
def run(test, params, env):
    ...
    if host_basic_page not in basic_page_list:
        test.fail(f"Host basic page size is {host_basic_page} not as expected.")

    test.log.info("Check the basic page size mapping to the hugepage size on host.")
    host_basic_page = normalize_data_size(f"{host_basic_page}B", "K")
    hugepage_list = params.objects(f"mapping_pgsize_{host_basic_page}k")
    host_local_pglist = os.listdir(local_pglist)
    if sorted(host_local_pglist) != sorted(hugepage_list):
        test.fail(f"Host huge page size is {host_local_pglist} not as expected.")
    ...
    if guest_basic_page not in basic_page_list:
        test.fail(f"Guest page size is {guest_basic_page} not as expected.")

    test.log.info("Check the basic page size mapping to the hugepage size on guest.")
    guest_basic_page = normalize_data_size(f"{guest_basic_page}B", "K")
    hugepage_list = params.objects(f"mapping_pgsize_{guest_basic_page}k")
    guest_local_pglist = session.cmd(f"ls {local_pglist}").strip().split()
    if sorted(guest_local_pglist) != sorted(hugepage_list):
        test.fail(f"Guest huge page size is {guest_local_pglist} not as expected.")
