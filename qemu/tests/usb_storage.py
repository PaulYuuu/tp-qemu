...
def run(test, params, env):
    ...
    def _verify_string(regex_str, string, expect_result, search_opt=0):
        ...
        def _compare_str(act, exp, ignore_case):
            ...
            if str_func(act) != str_func(exp):
                return f"Expected: '{str_func(exp)}', Actual: '{str_func(act)}'"
            return ""
        ...
        error_context.context(
            "Finding matched sub-string with regex" f" pattern '{regex_str}'",
            test.log.info,
        )
    ...
    def _check_serial_option(serial, regex_str, expect_str):
        error_context.context(f"Set serial option to '{serial}'", test.log.info)
    ...
    def _check_removable_option(removable, expect_str):
        error_context.context(f"Set removable option to '{removable}'", test.log.info)
        ...
        session = _login()
        cmd = f"dmesg | grep {_get_usb_disk_name_in_guest(session)}"
    ...
    def _check_io_size_option(min_io_size="512", opt_io_size="0"):
        error_context.context(
            f"Set min_io_size to {min_io_size}, opt_io_size to {opt_io_size}",
            test.log.info,
        )
        opt = {}
        ...
        cmd = f"cat /sys/block/{d}/queue/{{minimum,optimal}}_io_size"
    ...
    for rt in range(1, repeat_times + 1):
        disk_hotplugged = []
        if hotplug_unplug:
            error_context.context(f"Hotplug the {rt} times.", test.log.info)
            ...
            for dev in devices:
                ...
                if ret[1] is False:
                    test.fail(f"Failed to hotplug device '{dev}'. Output:\n{ret[0]}")
        ...
        if hotplug_unplug:
            error_context.context(f"Hotunplug the {rt} times.", test.log.info)
