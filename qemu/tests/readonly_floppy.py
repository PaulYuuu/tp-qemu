    ...
    try:
        ...
        for floppy_index in range(floppy_count):
            error_context.context(
                f"Format the {floppy_index} floppy disk", test.log.info
            )
            ...
            if s == 0:
                test.error(
                    f"Floppy disk {floppy_index} is not readonly and"
                    " it's formatted successfully"
                )
            error_context.context(
                f"Check the {floppy_index} floppy is readonly", test.log.info
            )
            found = re.search("(Read-only)|(protected)", o)
            test.log.debug("Output of format command: %s", o)
            if not found:
                test.error(
                    f"Floppy disk {floppy_index} cannot be formatted"
                    " for reasons other than readonly"
                )
            else:
                test.log.info(
