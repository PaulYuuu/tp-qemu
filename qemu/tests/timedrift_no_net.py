...
def subw_guest_pause_resume(test, params, vm, session):
    ...
    if not vm.monitor.verify_status("paused"):
        test.error(f"VM is not paused Current status: {vm.monitor.get_status()}")
    time.sleep(float(params.get("wait_timeout", "1800")))
    vm.monitor.cmd("cont")
    if not vm.monitor.verify_status("running"):
        test.error(f"VM is not running. Current status: {vm.monitor.get_status()}")
...
def run(test, params, env):
    ...
    if guest_clock_source not in out:
        test.fail(
            f"Clock source {guest_clock_source} missing in guest clock "
            f"sources {out}."
        )
    ...
    if sub_work in globals():  # Try to find sub work function.
        ...
        test.cancel(f"Unable to found subwork {sub_work} in {__file__} test file.")
    ...
    if test_type == "guest_suspend":
        ...
        if date_diff > tolerance and hwclock_diff > tolerance:
            test.fail(
                f"hwclock {hwclock_diff}s and date {date_diff}s difference is "
                "'guest_diff_time != host_diff_time'"
                f" out of tolerance {tolerance}s"
            )
        elif date_diff > tolerance:
            test.fail(
                f"date {date_diff}s difference is "
                "'guest_diff_time != host_diff_time'"
                f" out of tolerance {tolerance}s"
            )
        elif hwclock_diff > tolerance:
            test.fail(
                f"hwclock {hwclock_diff}s difference is "
                "'guest_diff_time != host_diff_time'"
                f" out of tolerance {tolerance}s"
            )
