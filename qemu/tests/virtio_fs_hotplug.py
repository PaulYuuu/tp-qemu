...
def run(test, params, env):
    ...
    def start_vfs_service(session):
        ...
        test.log.info(
            f"Check if virtiofs service is registered with {viofs_sc_query_cmd}."
        )
        ...
        if "RUNNING" not in output:
            ...
            if sc_start_s != 0:
                test.fail(f"Failed to start virtiofs service, output is {sc_start_o}")
    ...
    def get_win_dst_dir(session):
        ...
        return volume_letter, f"{volume_letter}:"

    def run_io_test(session, volume_letter, fs_dest):
        """
        Run io test on the shared dir.
        """
        error_context.context(f"Run io test on the {fs_dest}.", test.log.info)
        guest_file = os.path.join(fs_dest, test_file)
        error_context.context(
            f"Creating file under {fs_dest} inside " "guest.", test.log.info
        )
