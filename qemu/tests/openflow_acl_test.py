...
def run(test, params, env):
    ...
    def access_service(access_sys, access_targets, disabled, host_ip, ref=False):
        ...
        for asys in access_sys:
            for atgt in access_targets:
                ...
                atgt_disabled = access_params[f"disabled_{atgt}"]
                if asys in vms_tags:
                    vm = env.get_vm(asys)
                ...
                if access_cmd in ["ssh", "telnet"]:
                    ...
                    try:
                        ...
                        stat = 1
                        out_err = f"Failed to login {atgt} "
                        out_err += f"from {asys}, err: {err.output}"
                    if "TelnetServer" in params.get("setup_cmd_windows", ""):
                        try:
                            ...
                            stat += 1
                            out_err += f"Failed to login {asys} "
                            out_err += f"from {atgt}, err: {err.output}"
                    if out_err:
                        out = out_err
                else:
                    try:
    ...
    def get_acl_cmd(protocol, in_port, action, extra_options):
        ...
        acl_cmd += f",in_port={in_port.strip()}"
        if extra_options.strip():
            acl_cmd += ",{}".format(",".join(extra_options.strip().split()))
        if action.strip():
            acl_cmd += f",action={action.strip()}"
        return acl_cmd
    ...
    def remote_login(client, host, src, params_login, host_ip):
        ...
        if client == "ssh":
            ...
            cmd = (
                ...
                f"-o PreferredAuthentications=password -p {port} {username}@{host}"
            )
        elif client == "telnet":
        ...
        if src == "localhost":
            ...
            if params_login.get("os_type") == "windows":
                if client == "telnet":
                    cmd = f"C:\\telnet.py {host} {username} "
                    cmd += f'{password} "{prompt}" && '
                    cmd += "C:\\wait_for_quit.py"
                cmd = f"{cmd} || ping 127.0.0.1 -n 5 -w 1000 > nul"
            else:
                cmd += " || sleep 5"
    ...
    def setup_service(setup_target):
        ...
        error_context.context(
            "Set up {} service in {}".format(setup_params.get("service"), setup_target),
            test.log.info,
        )
        ...
        if "ftp" in access_param.get("access_cmd") and os_type == "linux":
            setup_func(
                "sed -i 's/anonymous_enable=NO/anonymous_enable=YES/g' {}".format(
                    params["vsftpd_conf"]
                )
            )
        if prepare_cmd:
    ...
    def stop_service(setup_target):
        ...
        error_context.context(
            "Stop {} service in {}".format(setup_params.get("service"), setup_target),
            test.log.info,
        )
        if stop_cmd:
    ...
    for vm in env.get_all_vms():
        ...
        if params.get("copy_curl") and params.get("os_type") == "windows":
            ...
            session.cmd(f"dir {curl_win_path} || mkdir {curl_win_path}")
    ...
    for target in all_target:
        if target not in access_targets:
            ...
            for tgt in access_targets:
                ...
                access_sys[target][f"disabled_{tgt}"] = acl_disabled
    ...
    if_port = re.findall(rf"(\d+)\({if_name}\)", br_infos)
    if not if_port:
        test.cancel(f"Can not find {if_name} in bridge {br_name}")
    if_port = if_port[0]
