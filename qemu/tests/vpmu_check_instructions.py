...
def run(test, params, env):
    ...
    src_file = os.path.join(src_dir, f"million-{host_arch}.s")
    dst_file = os.path.join(tmp_dir, f"million-{host_arch}.s")
    ...
    if not utils_package.package_install("perf", session):
        test.error("Install dependency packages failed")
    src_file = os.path.join(src_dir, f"million-{vm_arch}.s")
    error_context.context(
        f"transfer '{src_file}' to guest('{dst_file}')", test.log.info
    )
