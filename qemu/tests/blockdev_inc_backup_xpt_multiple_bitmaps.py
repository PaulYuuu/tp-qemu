...
class BlockdevIncbkXptMutBitmaps(BlockdevLiveBackupBaseTest):
    ...
    def check_nbd_export_info(self):
        if self._block_export_uid is not None:
            ...
            if (
                ...
                self.test.fail(f"Failed to get the correct export information: {info}")
    ...
    def check_bitmaps_from_export(self):
        ...
        for bm in self._bitmaps:
            opts[self.params["dirty_bitmap_opt"]] = f"qemu:dirty-bitmap:{bm}"
            args = f"'json:{json.dumps(opts)}'"
            map_cmd = f"{qemu_img} map --output=human {args}"
