...
def run(test, params, env):
    ...
    def sub_hotunplug():
        error_context.context(
            f"Hotunplug vcpu devices after vcpu {hotpluggable_test}", test.log.info
        )
        for plugged_dev in pluggable_vcpu_dev[::-1]:
            try:
                ...
                vm.params[f"vcpu_enable_{plugged_dev}"] = "no"

    def sub_reboot():
        error_context.context(
            f"Reboot guest after vcpu {hotpluggable_test}", test.log.info
        )
        vm.reboot(
    ...
    def sub_shutdown():
        error_context.context(
            f"Shutdown guest after vcpu {hotpluggable_test}", test.log.info
        )
        ...
        if not vm.wait_for_shutdown(360):
            test.fail(f"Guest refuses to go down after vcpu {hotpluggable_test}")

    def sub_migrate():
        error_context.context(
            f"Migrate guest after vcpu {hotpluggable_test}", test.log.info
        )
        vm.migrate()
        vm.verify_alive()
        ...
        while sub_test_after_migrate:
            ...
            error_context.context(f"{sub_test} after migration completed")
            eval(f"sub_migrate_{sub_test}")()

    def sub_online_offline():
        error_context.context(
            f"Offline then online guest CPUs after vcpu {hotpluggable_test}",
            test.log.info,
        )
    ...
    if params.get("pause_vm_before_hotplug", "no") == "yes":
        error_context.context(f"Pause guest before {hotpluggable_test}", test.log.info)
        vm.pause()

    error_context.context(f"{hotpluggable_test} all vcpu devices", test.log.info)
    for vcpu_dev in pluggable_vcpu_dev:
        getattr(vm, f"{hotpluggable_test}_vcpu_device")(vcpu_dev)
    if vm.is_paused():
        error_context.context(f"Resume guest after {hotpluggable_test}", test.log.info)
        vm.resume()
