...
def _setup_hugepage(params):
    ...
    error_context.context(f"Assign {size}MB hugepages in host.", LOG_JOB.info)
    ...
    error_context.context(f"mount hugepages to {huge_page.hugepage_path}", LOG_JOB.info)
...
def run(test, params, env):
    ...
    if not utils_misc.wait_for(
        ...
        test.fail(
            "Guest memory size is not increased {} in {} sec.".format(
