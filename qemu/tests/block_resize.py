...
def run(test, params, env):
    ...
    def check_shrink_completion(vol_id, size):
        ...
        def _check_event_cmd(cmd):
            ...
            test.log.debug(f"Get event: {output}")
            test.log.debug(f"Expect contain msg: {msg}")  # pylint: disable=E0606
        ...
        for ratio in params.objects("disk_change_ratio"):
            ...
            if float(ratio) < 1.0:
                error_context.context(
                    f"Shrink disk size to {block_size} in guest", test.log.info
                )
                if os_type == "windows":
            ...
            error_context.context(
                f"Change disk size to {block_size} in monitor", test.log.info
            )
            ...
            if os_type == "linux" and labeltype == "gpt":
                session.cmd(f"sgdisk -e /dev/{disk}")
            ...
            if float(ratio) > 1.0:
                error_context.context(
                    f"Extend disk to {block_size} in guest", test.log.info
                )
                if os_type == "windows":
            ...
            if params.get("md5_test") == "yes":
                ...
                test.assertTrue(new_md5 == md5, f"Unmatched md5: {new_md5}")
