...
def wait_timeout(timeout=10):
    """
    time.sleep(timeout) + LOG_JOB.debug(timeout)

    :param timeout=10
    """
    LOG_JOB.debug("Waiting (timeout=%ss)", timeout)
    time.sleep(timeout)
...
def clear_cb(session, params):
    ...
    clear_cmd = f"{interpreter} {script_call} {script_clear_params}"

    try:
        LOG_JOB.info("Clearing the clipboard")
        session.cmd(clear_cmd)
    except aexpect.ShellCmdError:
        LOG_JOB.debug("Clearing the clipboard, Test will continue")
...
def place_img_in_clipboard(
    ...
    """
    cmd = f"{interpreter} {script_call} {script_params} {dst_image_path}"
    ...
    try:
        ...
        if "The image has been placed into the clipboard." in output:
            ...
            test.fail(f"Copying to the clipboard failed. {output}")
    except aexpect.ShellCmdError:
...
def verify_img_paste(
    ...
    :param test_timeout: timeout time for the cmd
    """
    cmd = f"{interpreter} {script_call} {script_params} {final_image_path}"
...
def verify_txt_paste_success(
    ...
    :param test_timeout: timeout time for the cmd
    """
    cmd = f"{interpreter} {script_call} {script_params} {final_text_path}"
    try:
        LOG_JOB.debug("------------ Script output ------------")
...
def place_text_in_clipboard(
    ...
    :param test_timeout: timeout time for the cmd
    """
    cmd = f"{interpreter} {script_call} {script_params} {testing_text}"
