...
def run(test, params, env):
    ...
    def generate_snapshot_chain(snapshot_chain, snapshot_num):
        for i in range(snapshot_num):
            snapshot_tag = f"sn{i}"
            snapshot_chain += f" {snapshot_tag}"
            params[f"image_name_{snapshot_tag}"] = f"images/{snapshot_tag}"
        if snapshot_num > 0:
            params[f"check_base_image_{snapshot_tag}"] = "yes"
            ...
            try:
                ...
                if index != 0:
                    ...
                    if not os.path.exists(image.image_filename):
                        errs.append(
                            f"Image {image.image_filename} was not created during test."
                        )
                    image.remove()
            except Exception as details:
                errs.append(f"Fail to remove image {image.image_filename}: {details}")
        return errs
    ...
    try:
        for index, image in enumerate(snapshot_chain):
            ...
            if status != 0:
                test.error(f"Execute '{sync_cmd}' with failures('{output}') ")
                ...
                if image_params.get("file_create"):
                    for file in md5_value[image]:
                        ...
                        if md5 != md5_value[image][file]:
                            error_message = f"File {file} in image {image} changed "
                            error_message += (
                                f"from '{md5_value[image][file]}' to '{md5}'(md5)"
                            )
                            test.fail(error_message)
                    files_check = session.cmd(file_check_cmd % file_dir)
                    if files_check != files_in_guest[image]:
                        error_message = f"Files in image {image} is not as expect:"
                        error_message += f"Before shut down: {files_in_guest[image]}"
                        error_message += f"Now: {files_check}"
                        test.fail(error_message)
        ...
        error_context.context(
            f"Force-cleaning after exception: {details}", test.log.error
        )
        if vm.is_alive():
            vm.destroy()
