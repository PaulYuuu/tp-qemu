...
class NetPingError(utils_net.NetError):
    ...
    def __str__(self):
        e_msg = f"Can't ping from {self.src} to {self.dst}"
        if self.details is not None:
            e_msg += f" : {self.details}"
        return e_msg
...
def run(test, params, env):
    ...
    def add_vlan(interface, v_id, session=None):
        ...
        """
        vlan_if = f"{interface}.{v_id}"
        add_cmd = params["add_vlan_cmd"] % (interface, vlan_if, v_id)
        error_context.context(
            f"Create vlan interface '{vlan_if}' on {interface}", test.log.info
        )
        if session:
    ...
    def set_ip_vlan(vlan_if, vlan_ip, session=None):
        ...
        """
        error_context.context(
            f"Assign IP '{vlan_ip}' to vlan interface '{vlan_if}'", test.log.info
        )
        if session:
            ...
            session.cmd_output_safe(f"ifconfig {vlan_if} 0.0.0.0")
            session.cmd_output_safe(f"ifconfig {vlan_if} down")
            session.cmd_output_safe(f"ifconfig {vlan_if} {vlan_ip} up")
        else:
            process.system(f"ifconfig {vlan_if} {vlan_ip} up")
    ...
    def set_mac_vlan(vlan_if, mac_str, session):
        ...
        """
        mac_cmd = f"ip link set {vlan_if} add {mac_str} up"
        error_context.context(
            f"Give a new mac address '{mac_str}' for vlan interface " f"'{vlan_if}'",
            test.log.info,
        )
    ...
    def ping_vlan(vm, dest, vlan_if, session):
        ...
        """
        error_context.context(
            f"Test ping from '{vlan_if}' to '{dest}' on guest '{vm.name}'"
        )
    ...
    host_vlan_if = f"{host_br}.{host_vlan_id}"
    ...
    for vm_index, vm in enumerate(vms):
        error_context.context(f"Prepare test env on {vm.name}")
        session = vm.wait_for_serial_login()
        if not session:
            err_msg = f"Could not log into guest {vm.name}"
            test.error(err_msg)

        interface = utils_net.get_linux_ifname(session, vm.get_mac_address())

        error_context.context(f"Load 8021q module in guest {vm.name}", test.log.info)
        session.cmd_output_safe("modprobe 8021q")

        error_context.context(
            f"Setup vlan environment in guest {vm.name}", test.log.info
        )
        inter_ip = "%s.%s.%d" % (subnet, host_vlan_id, vm_index + 1)
        set_ip_vlan(interface, inter_ip, session=session)
        set_arp_ignore(session)
        params["vlan_nic"] = f"{interface}.{host_vlan_id}"
        error_context.context(
            f"Test ping from guest '{vm.name}' to host with "
        ...
        try:
            ...
            test.fail("Guest ping to host should fail with interface" f" '{interface}'")
    ...
    error_context.context(
        f"Test ping between guests with interface {ifname[0]}", test.log.info
    )
    ...
    for vm_index, vm in enumerate(vms):
        ...
        error_context.context(f"Add vlan interface on guest '{vm.name}'")
        session.cmd_output(f"ifconfig {ifname[vm_index]} 0.0.0.0", safe=True)
        vlan_if = add_vlan(
        ...
        error_context.context(
            f"Test ping from interface '{vm_vlan_if[vm_index]}' on guest "
            f"'{vm.name}' to host.",
            test.log.info,
        )
