...
def deploy_test_form(test, guest_vm, params):
    ...
    guest_vm.copy_files_to(
        script_path, "/tmp/{}".format(params.get("guest_script")), timeout=60
    )
...
def run_test_form(guest_session, params):
    ...
    cmd = "python /tmp/{} &> /dev/null &".format(params.get("guest_script"))
    guest_session.cmd(cmd)
    cmd = "disown -ar"
    guest_session.cmd(cmd)
...
def test_leds_migration(client_vm, guest_vm, guest_session, params):
    ...
    LOG_JOB.info(f"RHEL version: #{rhel_ver}#")
...
def run(test, params, env):
    ...
    if result is not None:
        test.fail("Testing of sending keys failed:" f"  Expected keycode = {result}")
