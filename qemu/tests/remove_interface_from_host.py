...
def run(test, params, env):
    ...
    error_context.context(f"Get the guest ip {guest_ip}", test.log.info)
    ...
    error_context.context(f"Get interface name: {host_ifname_name}. ", test.log.info)
    host_ifname = utils_net.Interface(host_ifname_name)

    # Step 3,4, disable interface and ping should fail
    error_context.context(f"Set interface {host_ifname_name} down.", test.log.info)
    host_ifname.down()
    ...
    status, output = utils_test.ping(guest_ip, 30, timeout=20)
    if status == 0:
        test.fail(f"Ping should fail, status: {status}, output: {output}")

    # Step 5, enable interface, ping should work
    error_context.context(f"Set interface {host_ifname_name} up.", test.log.info)
    host_ifname.up()
    time.sleep(secs_after_iplink_action)
    ...
    status, output = utils_test.ping(guest_ip, 30, timeout=20)
    if status != 0:
        test.fail(f"Ping should work, status: {status}, output: {output}")

    # Step 6, delete the interface, qemu should not crash,
    # ping should fail
    error_context.context(f"Delete the interface {host_ifname_name}.", test.log.info)
    host_ifname.dellink()
    time.sleep(secs_after_iplink_action)
