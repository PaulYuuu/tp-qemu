...
def run(test, params, env):
    ...
    def _shutdown_vm(session):
        ...
        error_context.context(
            f'Shutting down VM by "{shutdown_command}".', test.log.info
        )
    ...
    def _check_cdrom_info_by_qmp(items):
        ...
        error_context.context(
            f'Check if the info "{str(items)}" are match with the output of query-block.',
            test.log.info,
        )
    ...
    def _check_cdrom_info_by_guest():
        ...
        for index in range(len(cdroms)):
            ...
            test.fail(f'No such the file "{iso_name}" in cdrom.')
    ...
    for cdrom in cdroms:
        ...
        getattr(plug, f"{action}_devs_serial")(cdrom)
        if action == HOTPLUG:
            _check_cdrom_info(items_checked)
