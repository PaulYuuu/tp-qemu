...
def run(test, params, env):
    ...
    def _send_file_from_host_to_l1(obj, msg):
        ...
        test.log.info(f"vm.address: {vm.get_address(timeout=120)}")

        vsock_port = params.get_numeric("vsock_port", 2345)
        dd_cmd = f"dd if=/dev/urandom of={tmp_file} count=10240 bs=1024"
        process.system(dd_cmd, shell=True)
        md5_origin = process.system_output(f"md5sum {tmp_file}").split()[0]

        cmd_transfer = None
        if vsock_test_tool == "ncat":
            tool_bin = path.find_command("ncat")
            cmd_transfer = (
                f"{tool_bin} --vsock --send-only -l {vsock_port} < {tmp_file} &"
            )
        if vsock_test_tool == "nc_vsock":
            tool_bin = vsock_test.compile_nc_vsock(test, vm, session)
            cmd_transfer = f"{tool_bin} -l {vsock_port} < {tmp_file} &"
        if cmd_transfer is None:
            raise ValueError(f"unsupported test tool: {vsock_test_tool}")

        test.log.info(f"cmd_transfer: {cmd_transfer}")
    ...
    tmp_file = f"/var/tmp/vsock_file_{utils_misc.generate_random_string(6)}"
    ...
    process.run(f"git clone {playbook_repo} {playbook_dir}", verbose=False)
    ...
    test.log.info(f"host:{host} port:{mq_port}")
    client = message_queuing.MQClient(host, mq_port)
    time.sleep(2)

    test.log.info(f"vm.address: {vm.get_address()}")
    ...
    try:
        ...
        try:
            ...
            process.system(f"rm -f {tmp_file}")
