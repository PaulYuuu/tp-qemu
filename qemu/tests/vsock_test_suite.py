...
def copy_compile_testsuite(test, vm, session):
    ...
    compile_cmd = "cd {} && make vsock_test".format(
...
def run(test, params, env):
    ...
    try:
        ...
        if status != 0:
            test.fail(f"Test fail {status} {output}")
        test.log.info(f"command output: {output}")
