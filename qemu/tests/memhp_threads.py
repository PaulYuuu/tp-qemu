...
def run(test, params, env):
    ...
    def get_qemu_threads(cmd, timeout=60):
        ...
        while time.time() < end_time:
            ...
            if cur_threads != threads:
                ...
                return threads
        test.error(f"Can't get stable qemu threads number in {timeout}s.")
    ...
    for target_mem in target_mems:
        ...
        test.log.info(f"QEMU boot threads number is {pre_threads}")
        ...
        dev.set_param("id", "{}-{}".format("mem", target_mem))
        ...
        if post_threads - pre_threads != threads_num:
            test.fail(
                f"QEMU threads number is not right, pre is {pre_threads}, post is {post_threads}"
            )
        bg.join()
