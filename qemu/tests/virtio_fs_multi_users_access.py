...
def run(test, params, env):
    ...
    def get_eth_name(session):
        ...
        match = f"Connection-specific DNS Suffix  . : {domain_dns}"
    ...
    def switch_dynamic_ip_to_static_ip(session, eth_name):
        ...
        :param session: the session of guest
        """
        error_context.context(f"Get the config of {eth_name}.", test.log.info)
        net = session.cmd_output(f'netsh interface ip show config name="{eth_name}"')
        ...
        error_context.context(
            f"The config will be set to ipaddress:{ipaddr}, "
            f"gateway:{gateway}, subnet mask:{subnet_mask}.",
            test.log.info,
        )

        ip_cmd = (
            f'netsh interface ip set address name="{eth_name}" source=static '
            f"addr={ipaddr} mask={subnet_mask} gateway={gateway}"
        )
        session.cmd(ip_cmd)

        dns_cmd = (
            f'netsh interface ip set dnsservers "{eth_name}" static '
            "192.168.0.1 primary"
        )
        session.cmd(dns_cmd)
    ...
    def switch_ip_to_dynamic(session, eth_name):
        if eth_name:
            restore_ip_cmd = (
                f'netsh interface ip set address name="{eth_name}" ' "source=dhcp"
            )
            session.cmd(restore_ip_cmd)
            restore_dns_cmd = (
                f'netsh interface ip set dnsservers name="{eth_name}" ' "source=dhcp"
            )
    ...
    try:
        ...
        if os_type == "windows":
            ...
            session.cmd(f"chmod -R 777 {fs_dest}")

        for _username, _pwd in zip(username, pwd):
            try:
                if not domain_dns:
                    error_context.context(f"Login the user: {_username}", test.log.info)
