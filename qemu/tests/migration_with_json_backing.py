...
def run(test, params, env):
    ...
    def _create_image_with_backing(image):
        ...
        if image.base_tag in secids:
            ...
            backing_file = (
                f"'{get_image_json(image.base_tag, base_params, get_data_dir())}'"
            )
            ...
            msg = f"Create image by command: {qemu_img_cmd}"
            LOG_JOB.info(msg)
    ...
    def blockdev_add_image(tag):
        ...
        for dev in devices:
            ...
            if not ret[1]:
                test.fail(f"Failed to hotplug '{dev}': {ret[0]}.")

    def create_snapshot():
        options = ["node", "overlay"]
        cmd = "blockdev-snapshot"
        arguments = params.copy_from_keys(options)
        arguments.setdefault("node", "drive_{}".format(params["base_tag"]))
        arguments.setdefault("overlay", "drive_{}".format(params["snapshot_tag"]))
    ...
    def verify_backing_chain(info):
        ...
        for image, img_info in zip(images, reversed(info)):
            ...
            if base_image != backing_info:
                test.fail(
                    f"backing chain check for image {image.image_filename} failed, backing"
                    f" file from info is {backing_info}, which should be {base_image}."
                )
    ...
    params[f"image_name_{images_tag[0]}"] = params["image_name"]
