...
def run(test, params, env):
    ...
    if "0" not in output:
        test.fail(f"Failed to check vsyscall. Output: '{output}'")
    ...
    if not fail_cnt:
        test.error(f"Could not get correct test output. Output: '{output}'")

    tsc_cnt, tod_cnt, clk_cnt = (int(_) for _ in fail_cnt[-1])
    if tsc_cnt or tod_cnt or clk_cnt:
        msg = output.splitlines()[-5:]
        test.fail(
            "Get error when running time-warp-test." f" Output (last 5 lines): '{msg}'"
        )
    ...
    try:
        ...
        fail_cnt = re.findall(re_str, output)
        if not fail_cnt:
            test.error("Could not get correct test output." f" Output: '{output}'")
