...
class BlockdevIncbkInconsistentBitmap(BlockdevLiveBackupBaseTest):
    ...
    def check_bitmap_field(self, **args):
        ...
        if bitmap is None:
            self.test.fail(f"Failed to get bitmap {self._bitmaps[0]}")
        else:
            for key, value in args.items():
                if value != bitmap[key]:
                    self.test.fail(
                        f"bitmap field {key} is not correct: "
                        f"expected {value}, got {bitmap[key]}"
                    )
    ...
    def handle_bitmap_with_qmp_cmd(self):
        ...
        for action in forbidden_actions:
            try:
                ...
                self.test.fail(f"{action} completed unexpectedly")
    ...
    def remove_bitmap_with_qmp_cmd(self):
        ...
        if bitmap is not None:
            self.test.fail(f"Failed to remove bitmap {self._bitmaps[0]}")
