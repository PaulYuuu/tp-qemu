...
def run(test, params, env):
    ...
    def _start_allocator(vm, session, timeout):
        ...
        try:
            ...
            test.fail(f"Command guest script on vm '{vm.name}' failed: {str(exc)}")
    ...
    def _execute_allocator(command, vm, session, timeout):
        ...
        try:
            ...
            e_str = (
                f"Failed to execute command '{command}' on guest script, "
                f"vm '{vm.name}': {str(exc)}"
            )
            test.fail(e_str)
        return (match, data)
    ...
    mem_fill = f"mem = MemFill({shared_mem}, 0, {seed})"
    ...
    if split == "yes":
        if test_type == "negative":
            cmd = f"mem.static_random_fill({random_bits})"
        else:
    ...
    if fail_type != 0:
        ...
        while fail_type > 0:
            ...
            turns += 1
        test.fail(
            f"KSM test failed: {sharing_page_0} {sharing_page_1} {sharing_page_2}"
        )
    session.close()
