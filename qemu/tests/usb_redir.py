...
def run(test, params, env):
    ...
    def _start_usbredir_server(port):
        ...
        usbredirserver_args = usbredir_server + f" -p {port} "
        usbredirserver_args += f" {vendorid}:{productid}"
    ...
    def create_repo():
        ...
        create_cmd = rf'echo -e "{content}" > /etc/yum.repos.d/temp.repo'
    ...
    def _host_config_check():
        ...
        if option == "with_negative_config":
            ...
            if s:
                status = False
                err_msg = f"Fail to unconfig the USB device, output: {o}"
                return (status, err_msg)
        ...
        if backend == "spicevmc":
            ...
            if not obj or gui_group not in obj.group(1):
                ...
                if s:
                    status = False
                    err_msg = f"Fail to install '{gui_group}' on host, "
                    err_msg += f"output: {o}"
                    return (status, err_msg)
    ...
    def _usbredir_preprocess():
        def _generate_usb_redir_cmdline():
            ...
            chardev_id = usbredir_params.get(
                "chardev_id", f"chardev_{usbredirdev_name}"
            )
            ...
            usbredir_dev.set_param("id", f"usb-{usbredirdev_name}")
    ...
    def _start_spice_redirection():
        ...
        rv_args += f"'-1,0x{vendorid},0x{productid},-1,1'"
    ...
    def _kill_rv_proc():
        s, o = process.getstatusoutput(f"pidof {rv_binary}")
        if not s:
            process.getoutput(f"killall {rv_binary}")
    ...
    def _get_usb_mount_point():
        ...
        if s:
            s, o = session.cmd_status_output(f"mount /dev/{dev} /mnt")
            if s:
                test.error(f"Fail to mount /dev/{dev}, output: {o}")
            mp = "/mnt"
        else:
            mp = re.findall(rf"/dev/{dev}\d*\s+(\S+)\s+", o)[0]
        return mp
    ...
    if backend not in ("spicevmc", "tcp_socket"):
        test.error(f"Unsupported char device backend type: {backend}")
    ...
    if option == "with_bootindex":
        ...
        bootindex = int(params[f"usbdev_option_bootindex_{usbredirdev_name}"])
