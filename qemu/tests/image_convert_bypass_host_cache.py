...
def run(test, params, env):
    ...
    strace_output_file = os.path.join(
        test.debugdir, f"convert_to_{convert_target1}.log"
    )
    ...
    fail_msg = f"'O_DIRECT' is presented in system calls {strace_events}"
    ...
    strace_output_file = os.path.join(
        test.debugdir, f"convert_to_{convert_target2}.log"
    )
    ...
    fail_msg = f"'O_DIRECT' is not presented in system calls {strace_events}"
