...
def set_hpt(session, params, test, hpt_size):
    ...
    if st:
        ...
        # by kernel so dealing with it particularly
            ...
            test.fail(f"Set hpt test failed,please check:'{o}'")
    ...
    if params.get("sub_type") != "negative":
        ...
        if s:
            ...
            if int(get_hpt_value) != hpt_size:
                test.fail(f"HPT order not match! '{get_hpt_value}' vs '{hpt_size}'")
...
def run(test, params, env):
    ...
    if params.get("sub_type") == "mem":
        ...
        if not utils_misc.wait_for(
            ...
            test.fail(
                f"Guest memory size is not increased {increase_mem} in {plug_timeout} sec."
            )
