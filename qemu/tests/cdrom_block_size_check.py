...
def run(test, params, env):
    ...
    def create_iso_image(params, name, prepare=True, file_size=None):
        ...
        if prepare:
            ...
            process.run(f"mkisofs -o {cdrom_path} {name}")
    ...
    def check_cdrom_size(session):
        ...
        if not output:
            ...
            msg += f" Command: {check_cdrom_size_cmd}\nOutput: {output}"
            test.error(msg)
    ...
    def mount_cdrom(session, guest_cdrom, mount_point, show_mount_cmd, mount_cmd):
        ...
        if mount_point not in mounted:
            ...
            if status:
                msg = f"Unable to mount cdrom. command: {mount_cmd}\n"
                msg += f" Output: {output}"
                test.error(msg)
    ...
    if orig_size == empty_size:
        err = f"Get same block size '{orig_size}' after new cdrom attached"
        test.fail(err)
    ...
    if umount_cmd:
        ...
        if status:
            msg = f"Unable to umount cdrom. command: {umount_cmd}\n"
            msg += f"Output: {output}"
            test.error(msg)
    ...
    if final_size == empty_size or final_size == orig_size:
        err = f"Get same block size '{final_size}' after new cdrom attached"
        test.fail(err)
