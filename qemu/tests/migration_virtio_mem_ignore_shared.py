...
def run(test, params, env):
    ...
    params["stress_args"] = (
        f"--cpu 4 --io 4 --vm 2 --vm-bytes {float(test_mem * 0.8):f}M"
    )
    clone = None
    try:
        ...
        try:
            ...
            test.log.debug(f"Total memory migrated: {total_mem_migrated:f}")
            ...
            test.log.debug(f"Stress tool running status: {stress_test.app_running()}")
        ...
        if clone:
            ...
            env.unregister_vm(f"{vm.name}_clone")
