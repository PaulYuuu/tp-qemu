...
def run(test, params, env):
    ...
    try:
        ...
        error_context.context(
            (f"Run subtest {bg_stress_test} between host and guest."), test.log.info
        )
        ...
        if not utils_misc.wait_for(
            ...
            0,
            1,
            f"Wait {bg_stress_test} test start",
        ):
        ...
        txt = f"Ping {host_ip} from {dsthost} during netperf testing"
        ...
        txt = f"{package_lost}% packeage lost when ping {host_ip} from {dsthost}."
