...
def run(test, params, env):
    ...
    try:
        ...
        if "False" in output:
            test.fail(f"Secure boot is not enabled. The actual output is {output}")
        ...
        if check_enable_info not in output:
            test.fail(f"Device Guard enable failed. The actual output is {output}")
        ...
        if check_ready_info not in output:
            test.fail(f"Device Guard running failed. The actual output is {output}")
