...
class QemuGuestAgentTest(BaseVirtTest):
    ...
    def _check_ga_pkg(self, session, cmd_check_pkg):
        ...
        if s == 0 and self.params.get("os_variant", "") == "rhel8":
            ...
            version_list = []
            full_qga_rpm_info = session.cmd(f"readlink {self.qga_pkg_path}")
            ...
            if len(version_list) < 2:
                self.test.error(
                    f"Does not find proper qga package at {self.qga_pkg_path}, it"
                    "is recommended to install a specific version"
                    "of qga rpm in advance"
                )
    ...
    def gagent_install(self, session, vm):
        ...
        if self.params.get("os_variant", "") == "rhel8":
            ...
            if s_check == 0:
                ...
                session.cmd(f"rpm -e {o_check.strip()}")
            self.gagent_install_cmd = f"rpm -ivh {self.qga_pkg_path}"
        ...
        if s_inst != 0:
            self.test.fail(
                "qemu-guest-agent install failed," f" the detailed info:\n{o_inst}."
            )
    ...
    def gagent_start(self, session, vm):
        ...
        if s and "already been started" not in o:
            self.test.fail(
                f"Could not start qemu-ga service in VM '{vm.name}'," f"detail: '{o}'"
            )
    ...
    def gagent_stop(self, session, vm):
        ...
        if s and "is not started" not in o:
            self.test.fail(
                f"Could not stop qemu-ga service in VM '{vm.name}', " f"detail: '{o}'"
            )
    ...
    def gagent_verify(self, params, vm):
        ...
        if not self.gagent:
            self.test.error("Could not find guest agent object " f"for VM '{vm.name}'")
    ...
    def gagent_setsebool_value(self, value, params, vm):
        ...
        error_context.context(
            f"Turn {value} virt_qemu_ga_read_nonsecurity_files.", LOG_JOB.info
        )
    ...
    def setup(self, test, params, env):
        ...
        if self.start_vm == "yes":
            ...
            if self.params.get("os_variant", "") == "rhel8":
                ...
                if not s:
                    output = session.cmd_output(f"cat {o}")
                    test.fail(f"Failed to get qga, details: {output}")
                else:
            ...
            if self._check_ga_service(session, params.get("gagent_status_cmd")):
                ...
                if output and "qemu-guest-agent" in output:
                    test.error(
                        ...
                        f"check more details from {output}"
                    )
                else:
...
class QemuGuestAgentBasicCheck(QemuGuestAgentTest):
    ...
    def gagent_check_install_uninstall(self, test, params, env):
        ...
        for i in range(repeats):
            error_context.context(f"Repeat: {i + 1}/{repeats}", LOG_JOB.info)
            if self._check_ga_pkg(session, params.get("gagent_pkg_check_cmd")):
    ...
    def gagent_check_guest_info(self, test, params, env):
        ...
        if qga_ver_qga != qga_ver_guest:
            test.fail(
                f"The qga version {qga_ver_qga} from qga is different with {qga_ver_guest} "
                "from guest."
            )
        ...
        if params.get("os_type") == "linux":
            ...
            try:
                if "core-dump" in output:
                    test.fail(
                        "Guest-agent aborts after guest-shutdown" f" detail: '{output}'"
                    )
            finally:
    ...
    def gagent_check_reboot(self, test, params, env):
        ...
        error_context.context(f"Verify serial output has '{pattern}'")
        ...
        try:
            ...
            test.fail("Could not login to guest" f" detail: '{detail}'")
    ...
    def gagent_check_set_mem_blocks(self, test, params, env):
        ...
        if mem_blocks[mem_list_index]["online"] is not False:
            test.fail(
                f"{mem_phys_index} phys-index memory block is still online"
                " via agent."
            )
        ...
        if delta != mem_unit_size:
            test.fail(
                f"Memory block info is not correct\nit's {mem_unit_size} via agent\n"
                f"it's {delta} via guest."
            )
        ...
        if mem_blocks_list is not None:
            ...
            if float(mem_size_aft_offline_qga) >= float(mem_size_aft_offline_guest):
                test.fail(
                    f"Memory isn't decreased\nsize before is {mem_size_aft_offline_guest}\n"
                    f"size after is {mem_size_aft_offline_qga}"
                )
        else:
            LOG_JOB.info(
    ...
    def gagent_check_time_sync(self, test, params, env):
        ...
        error_context.context(f"Sleep {pause_time} seconds.", LOG_JOB.info)
        ...
        if time_diff_before < (pause_time - 5):
            test.error(f"Time is not paused about {pause_time} seconds.")
    ...
    def _get_mem_used(self, session, cmd):
        ...
        try:
            ...
            raise exceptions.TestError(
                "Get invalid memory usage by " f"cmd '{cmd}' ({output})"
            )
    ...
    def gagent_check_memory_leak(self, test, params, env):
        ...
        session = self.vm.wait_for_login(timeout=timeout)
        error_context.context(
            f"get the memory usage of qemu-ga before run '{test_command}'",
            LOG_JOB.info,
        )
        ...
        for i in range(repeats):
            error_context.context(
                f"execute '{test_command}' {i + 1} times", LOG_JOB.info
            )
            return_msg = self.gagent.guest_info()
            LOG_JOB.info(str(return_msg))
        self.vm.verify_alive()
        error_context.context(
            f"get the memory usage of qemu-ga after run '{test_command}'",
            LOG_JOB.info,
        )
        session = self.vm.wait_for_login(timeout=timeout)
        ...
        if memory_usage_after - memory_usage_before > 500:
            test.fail(
                ...
                f"before run command is {memory_usage_before}kb and "
                f"after run command is {memory_usage_after}kb"
            )
    ...
    def gagent_check_fstrim(self, test, params, env):
        ...
        params["start_vm"] = "yes"
        params[f"image_name_{test_image}"] = disk_name
        params[f"image_format_{test_image}"] = "raw"
        params[f"image_raw_device_{test_image}"] = "yes"
        params[f"force_create_image_{test_image}"] = "no"
        params[f"drive_format_{test_image}"] = "scsi-block"
        params[f"drv_extra_params_{test_image}"] = "discard=on"
        params["images"] = " ".join([params["images"], test_image])

        error_context.context(f"boot guest with disk '{disk_name}'", LOG_JOB.info)
        ...
        error_context.context(f"format disk '{device_name}' in guest", LOG_JOB.info)
        ...
        error_context.context(
            f"mount disk with discard options '{device_name}'", LOG_JOB.info
        )
        ...
        error_context.context(
            f"the bitmap_before_trim is {bitmap_before_trim}", LOG_JOB.info
        )
        ...
        if not re.match(r"\d+-\d+", bitmap_after_trim):
            test.fail("didn't get the bitmap of the target disk")
        error_context.context(
            f"the bitmap_after_trim is {bitmap_after_trim}", LOG_JOB.info
        )
    ...
    def gagent_check_get_disks(self, test, params, env):
        ...
        for disk_info_guest in disks_info_guest:
            ...
            for disk_info_qga in disks_info_qga:
                if diskname == disk_info_qga["name"]:
                    ...
                    if disk_info_guest["type"] == "disk":
                        ...
                        if not dependencies or (
                            ...
                            test.fail(
                                f"Disk {diskname} dependencies is different "
                                "between guest and qga."
                            )

                    error_context.context(
                        f"Check partition of disk {diskname}", LOG_JOB.info
                    )
                    partition = False if disk_info_guest["type"] != "part" else True
                    if disk_info_qga["partition"] != partition:
                        test.fail(
                            f"Disk {diskname} partition is different "
                            "between guest and qga."
                        )

                    if disk_info_guest["type"] == "lvm":
                        error_context.context(
                            f"Check alias of disk {diskname}", LOG_JOB.info
                        )
                ...
                test.fail(f"Failed to get disk {diskname} with qga.")
    ...
    def gagent_check_ssh_public_key_injection(self, test, params, env):
        ...
        def ssh_key_test(operation, guest_name, *keys, **kwargs):
            ...
            op_func = getattr(self.gagent, f"ssh_{operation}_authorized_keys")
        ...
        def _value_compared_ga_guest(return_value_ga, return_value_guest, status):
            ...
            for keys_ga in keys_ga_list:
                if keys_ga not in keys_guest:
                    test.fail(
                        "Key {} is not same with guest, " "{} ssh keys failed.".format(
                            *keys_ga
                        ),
                        status,
                    )
        ...
        for key in [host_key1, host_key3]:
            if key in keys_guest.replace("\n", ","):
                test.fail(
                    f"Key {key} is still in guest," "Can not remove key in guest."
                )
        _login_guest_test(guest_ip_ipv4)
        ...
        for cs in cs_info_qga:
            ...
            for key in cs.keys():
                if (
                    ...
                    improper_list.append({(f"cpu{cpu_num_qga}"): key})
            cpu_num_qga += 1
        if improper_list:
            test.fail(f"Cpustats info is not totally correct: {improper_list}")
    ...
    def gagent_check_get_diskstats(self, test, params, env):
        ...
        for ds in ds_info_qga:
            ds_name = ds["name"]
            error_context.context(
                f"Check {ds_name} 'major' and 'minor' value.", LOG_JOB.info
            )
        ...
        if improper_list:
            test.fail(f"Diskstats info is not totally correct: {improper_list}")
        ...
        if disk_num_qga != int(disk_num_guest):
            test.fail(
                f"Number of disks is not correct, disk_num_qga is {disk_num_qga};"
                f"disk_num_guest is {disk_num_guest}"
            )
    ...
    def gagent_check_get_interfaces(self, test, params, env):
        ...
        def ip_addr_check(session, mac_addr, ret_list, if_index, if_name):
            ...
            for ip in ip_lists:
                if ip["ip-address-type"] == "ipv4":
                    ...
                    test.fail(
                        "The ip address type is {}, but it should be"
                        " ipv4 or ipv6.".format(ip["ip-address-type"])
                    )
            if (
                ...
                test.fail(
                    f"Get the wrong ip address for {if_name} interface:\n"
        ...
        if not if_name:
            test.fail(
                "Did not get the expected interface," f" the network info is \n{ret}."
            )

        error_context.context(
            f"Check the available interface name {if_name}" " via qga.", LOG_JOB.info
        )
        if os_type == "linux":
        ...
        if if_name != if_name_guest:
            test.fail(
                f"Get the wrong interface name, value from qga is: {if_name}; "
                f"the expected is: {if_name_guest}"
            )
        ...
        if os_type == "linux":
            ...
            for ip in ip_lists:
                if ip["ip-address-type"] == "ipv4":
                    ip_addr_qga_ipv4 = ip["ip-address"]
                    break
            session_serial.cmd(f"ip addr del {ip_addr_qga_ipv4} dev {if_name}")
            ...
            for mode in (gagent.SHUTDOWN_MODE_POWERDOWN, gagent.SHUTDOWN_MODE_REBOOT):
                try:
                    ...
                    if not re.search("guest-shutdown has been disabled", str(detail)):
                        test.fail(
                            f"This is not the desired information: ('{str(detail)}')"
                        )
                else:
                    test.fail(
            ...
            for black_cmd in cmd_black_list.split():
                ...
                if (
                    ...
                    self.test.fail(
                        ...
                        f"white list, the output is {output}"
                    )
    ...
    def _read_check(self, ret_handle, content, count=None):
        ...
        if not content_read.strip() == content.strip():
            self.test.fail(
                f"The read content is '{content_read}'; the real content is '{content}'."
            )
    ...
    def _guest_file_prepare(self):
        ...
        guest_file = "{}{}".format(self.params["file_path"], file_name)
    ...
    def gagent_check_file_seek(self, test, params, env):
        ...
        cmd_del_file = "{} {}".format(params["cmd_del"], tmp_file)
    ...
    def gagent_check_file_write(self, test, params, env):
        ...
        for cnt in range(1, 10, 2):
            error_context.context(f"Write {cnt} bytes to guest file.", LOG_JOB.info)
    ...
    def gagent_check_file_read(self, test, params, env):
        ...
        cmd_create_file = f"echo helloworld > {tmp_file}"
        ...
        try:
            ...
            if res_windows not in str(detail) and res_linux not in str(detail):
                test.fail("This is not the desired information: " f"('{str(detail)}')")
        else:
    ...
    def gagent_check_with_fsfreeze(self, test, params, env):
        ...
        try:
            ...
            if not re.search("guest-file-open has been disabled", str(detail)):
                self.test.fail(
                    "This is not the desired information: " f"('{str(detail)}')"
                )
        else:
            self.test.fail(
    ...
    def gagent_check_with_selinux(self, test, params, env):
        ...
        def result_check_enforcing():
            ...
            def check(guest_file, open_mode):
                error_context.context(
                    f"Try to open {guest_file} with {open_mode} mode via"
                ...
                if "/tmp" in guest_file and open_mode == "a+":
                    ...
                    try:
                        ...
                        test.fail(
                            ...
                            f" agent should not open {guest_file} with {open_mode} mode."
                        )
        ...
        session.cmd(f"echo 'hello world' > {guest_temp_file}")
        session.cmd(f"echo 'hello world' > {guest_file}")
    ...
    def gagent_check_guest_exec(self, test, params, env):
        ...
        try:
            ...
            if not re.search("Failed to execute child process", str(detail)):
                test.fail(f"This is not the desired information: ('{str(detail)}')")
        else:
    ...
    def _fsfreeze(self, fsfreeze_list=False, mountpoints=None, check_mountpoints=None):
        ...
        error_context.context(
            ...
            f" {fsfreeze_list} and mountpoints is {mountpoints}.",
            LOG_JOB.info,
        )
        ...
        try:
            ...
            try:
                ...
                LOG_JOB.warn("Finally failed to thaw guest fs," " detail: '%s'", detail)
            raise
        # check after fsthaw
    ...
    def gagent_check_fsfreeze_list(self, test, params, env):
        ...
        if params.get("os_type") == "linux":
            ...
            if disk_index:
                ...
                mount_points = ["C:\\", f"{mnt_point_data}:\\"]
            else:
    ...
    def gagent_check_thaw_unfrozen(self, test, params, env):
        ...
        if ret != 0:
            test.fail(
                f"The return value of thawing an unfrozen fs is {ret},"
                "it should be zero"
            )
            ...
            if self.gagent.get_fsfreeze_status() == self.gagent.FSFREEZE_STATUS_FROZEN:
                try:
                    ...
                    if not re.search(
                        ...
                        test.error(
                            "guest-fsfreeze-thaw cmd failed with:" f"('{str(detail)}')"
                        )
            self.vm.verify_alive()
            if params.get("os_type") == "linux":
    ...
    def gagent_check_path_fsfreeze_hook(self, test, params, env):
        ...
        cmd_get_hook_path = rf"cat {qga_man_file} |grep /fsfreeze-hook"
        ...
        if (
            ...
            msg += f"it's in help cmd is {help_cmd_hook_path}\n"
            msg += f"it's in man page is {man_cmd_hook_path}\n"
            test.fail(msg)
        session.cmd(f"rm -rf {qga_man_file}")
    ...
    def gagent_check_fsfreeze_hook_script(self, test, params, env):
        ...
        def log_check(action):
            msg = f"testing {user_script_path}:{action}"
            hook_log = session.cmd_output(f"cat {log_path}")
            if msg not in hook_log.strip().splitlines()[-2]:
                test.fail(
                    "Fsfreeze hook test failed\nthe fsfreeze"
                    f" hook log is {hook_log}."
                )
        ...
        if params["os_variant"] == "rhel6":
            ...
            if not session.cmd_output(cmd_get_hook):
                cmd_enable_hook = (
                    ...
                    f"FSFREEZE_HOOK_ENABLE=1/g' {config_file}"
                )
                ...
                if "org.qemu.guest_agent.0" in chardev_dict["filename"]:
                    ...
                    if ret is expected:
                        ...
                        test.fail(
                            ...
                            f"is {ret}, it should be {expected}"
                        )
    ...
    def gagent_check_frozen_io(self, test, params, env):
        ...
        try:
            ...
            if not re.search(
                ...
                test.fail("guest-fsfreeze-freeze cmd failed with:" f"('{str(detail)}')")
    ...
    def gagent_check_vss_status(self, test, params, env):
        ...
        def check_vss_info(cmd_type, key, expect_value):
            cmd_vss = (
                f'sc {cmd_type} "QEMU Guest Agent VSS Provider" | findstr /i {key}'
            )
            ...
            if vss_result != expect_value:
                test.fail(f"The output is {vss_result} which is not expected.")
        ...
        try:
            ...
            if not re.search("fsfreeze is limited up to 10 seconds", str(detail)):
                test.error("guest-fsfreeze-thaw cmd failed with:" f"('{str(detail)}')")
    ...
    def gagent_check_fsinfo(self, test, params, env):
        ...
        def check_usage_qga_guest(mount_point):
            ...
            if diff_total_qgaguest != 0:
                test.fail(f"File System {mount_point} Total bytes doesn't match.")
            if diff_used_qgaguest != 0:
                if mount_point != "C:" and mount_point != "/":
                    test.fail(f"File system {mount_point} used bytes doesn't match.")
                else:
        ...
        for fs in fs_info_qga:
            ...
            error_context.context(
                f"Check file system '{mount_pt}' usage statistics.", LOG_JOB.info
            )
            ...
            error_context.context(
                f"Check file system type of '{mount_pt}' mount point.", LOG_JOB.info
            )
            fs_type_qga = fs["type"]
            ...
            if params["os_type"] == "linux":
                if not re.findall(r"^/\w*/\w*$", disk_name_guest):
                    disk_name_guest = session.cmd(f"readlink {disk_name_guest}").strip()
            ...
            if not re.findall(serial_num, serial_qga):
                test.fail(
                    ...
                    f"but it should include {serial_num}."
                )
            else:
    ...
    def gagent_check_nonexistent_cmd(self, test, params, env):
        ...
        try:
            ...
            pattern = f"command {cmd_wrong} has not been found"
            if not re.search(pattern, str(detail), re.I):
                test.fail(
                    "The error info is not correct, the return is" f" {str(detail)}."
                )
        else:
    ...
    def gagent_check_log(self, test, params, env):
        ...
        def log_check(qga_cmd):
            ...
            """
            error_context.context(f"Check {qga_cmd} cmd in agent log.", LOG_JOB.info)
            log_str = session.cmd_output(get_log_cmd).strip().split("\n")[-1]
            pattern = rf"{qga_cmd}"
            if not re.findall(pattern, log_str, re.M | re.I):
                test.fail(f"The {qga_cmd} command is not recorded in agent" " log.")
    ...
    def gagent_check_umount_frozen(self, test, params, env):
        ...
        if params.get("os_type") == "linux":
            if params["os_variant"] == "rhel6":
                try:
                    session.cmd(f"umount {mnt_point[0]}")
            ...
            if re.search("Status.*Online", details, re.I | re.M):
                ...
                if status != 0:
                    test.fail(f"Can not offline disk: {output} with" " fsfreeze.")
        ...
        try:
            ...
            if not re.search("fsfreeze is limited up to 10 seconds", str(detail)):
                test.error(f"guest-fsfreeze-thaw cmd failed with: ('{str(detail)}')")
    ...
    def gagent_check_blacklist(self, test, params, env):
        ...
        def bl_check(qga_cmd):
            ...
            try:
                ...
                if re.search(f"{qga_cmd} has been disabled", str(detail)):
                    LOG_JOB.info("%s cmd is disabled.", qga_cmd)
                else:
                    test.fail(f"{qga_cmd} cmd failed with:" f"('{str(detail)}')")
            else:
                test.fail(f"{qga_cmd} cmd is not in blacklist," " pls have a check.")
        ...
        if black_list_spec == "allow-rpcs":
            black_list_change_cmd = f"sed -i 's/{black_list_spec}.*/{black_list_spec_replace}=guest-info\"/g' /etc/sysconfig/qemu-ga"  # pylint: disable=E0606
        else:
            black_list_change_cmd = f"sed -i 's/{black_list_spec}.*/{black_list_spec}=guest-info/g' /etc/sysconfig/qemu-ga"
        try:
    ...
    def gagent_check_virtio_device(self, test, params, env):
        ...
        for device in devs_list:
            ...
            error_context.context(f"Check {driver_name} info.", LOG_JOB.info)
    ...
    def gagent_check_os_basic_info(self, test, params, env):
        ...
        if params["os_type"] == "windows":
            ...
            if timezone_name_ga not in timezone_name_list:
                msg = "The result is different between qga and guest\n"
                msg += f"from qga: {timezone_name_ga}\n"
        ...
        if user_num_qga != user_num_guest:
            ...
            msg += f"from qga: {len(user_num_qga)}\n"
        ...
        for user_qga in user_qga_list:
            ...
            error_context.context(f"Check {user_name_qga} user info.", LOG_JOB.info)
            ...
            if delta > 60:
                msg = f"{user_name_qga} login time are different between"
                msg += " qga and guest\n"
                msg += f"from qga: {login_time_qga}\n"
    ...
    def run_once(self, test, params, env):
        ...
        chk_type = f"gagent_check_{gagent_check_type}"
...
class QemuGuestAgentBasicCheckWin(QemuGuestAgentBasicCheck):
    ...
    def _check_serial_driver(self, test, params, env):
        ...
        if check_serial:
            ...
            if "Running" not in check_serial_result:
                ...
                s, o = session.cmd_status_output(f"dir {devcon_path}", timeout=120)
                if s:
                    test.error(f"Not found devcon.exe, details: {o}")
                vm_infos = {
                    "drive_letter": "",
                    "product_dirname": "",
                    "arch_dirname": "",
                ...
                for chk_point in vm_infos.keys():
                    try:
                        get_content_func = getattr(
                            virtio_win, f"{chk_point}_{media_type}"
                        )
                    ...
                    if not vm_infos[chk_point]:
                        test.error(f"Could not get {chk_point} of guest")
                inf_middle_path = (
                ...
                for hwid in device_hwid.split():
                    ...
                    if re.search("No matching devices found", output, re.I):
                        continue
                    inst_cmd = f"{devcon_path} updateni {inf_path} {hwid}"
                    status, output = session.cmd_status_output(inst_cmd, 360)
                    # acceptable status: OK(0), REBOOT(1)
                    if status > 1:
                        test.error(
                            f"Failed to install driver '{driver_name}', "
                            f"details:\n{output}"
                        )
                    installed_any |= True
                if not installed_any:
                    test.error(
                        "Failed to find target devices " f"by hwids: '{device_hwid}'"
                    )
    ...
    def get_qga_pkg_path(self, qemu_ga_pkg, test, session, params, vm):
        ...
        """
        error_context.context(f"Get {qemu_ga_pkg} path where it locates.", LOG_JOB.info)
        qemu_ga_pkg_path = ""
        if self.gagent_src_type == "url":
            ...
            if rpm_install:
                ...
                gagent_download_cmd = (
                    f"wget -qP {gagent_host_path} {gagent_download_url}"
                )
                gagent_host_path += src_qgarpm_path
            else:
                gagent_host_path += qga_msi
                gagent_download_cmd = f"wget {gagent_host_path} {gagent_download_url}"
            ...
            s, gagent_host_path = process.getstatusoutput(f"ls {gagent_host_path}")
            if s != 0:
                test.error(
                    "qemu-ga package is not exist, maybe it is not "
                    "successfully downloaded "
                )
            s, o = session.cmd_status_output(f"mkdir {self.gagent_guest_dir}")
            if s and "already exists" not in o:
                test.error(
                    "Could not create qemu-ga directory in "
                    f"VM '{vm.name}', detail: '{o}'"
                )
            if rpm_install:
                ...
                process.system(f"cp -r {qgamsi_path[0]} {qgamsi_path[1]} {tmp_dir}")
            ...
            qemu_ga_pkg_path = rf"{self.gagent_guest_dir}\{qemu_ga_pkg}"
            ...
            qemu_ga_pkg_path = r"{}:\{}\{}".format(
                vol_virtio, "guest-agent", qemu_ga_pkg
            )
        else:
            test.error(
    ...
    def gagent_check_get_disks(self, test, params, env):
        ...
        for disk_info in disks_info_qga:
            ...
            if diskname.upper() != disk_info_guest[0].replace("\\", r"\\"):
                test.fail(
                    f"Disk {diskname} name is different " "between guest and qga."
                )
    ...
    def gagent_check_fsfreeze_vss_test(self, test, params, env):
        ...
        def result_check(flag, write_timeout, session):
            ...
            if s:
                self.test.error(
                    f"Command '{k_cmd}' failed, status: {s}," f" output: {o}"
                )

            error_context.context("Check guest FS status.", LOG_JOB.info)
            # init fs status to 'thaw'
            fs_status = "thaw"
            file_name = f"/tmp/fsfreeze_{flag}.txt"
            process.system(f"rm -rf {file_name}")
            ...
            if not fs_status == flag:
                self.test.fail(f"FS is not {flag}, it's {fs_status}.")
        ...
        try:
            error_context.context(
                f"Waiting {write_timeout}, then finish writing the time "
                "stamp in guest file."
            )
    ...
    def gagent_check_fstrim(self, test, params, env):
        ...
        def get_blocks():
            ...
            """
            blocks = process.system_output(f"stat -t {image_filename_stg}")
        ...
        guest_dir = rf"{mnt_point[0]}:"
        data_file = os.path.join(
            guest_dir, f"qga_fstrim{utils_misc.generate_random_string(5)}"
        )
        for i in range(2):
            count = 1000 * (i + 1)
        ...
        if int(blocks_after_fstrim) >= int(blocks_before_fstrim):
            msg = "Fstrim failed\n"
            msg += f"the blocks before fstrim is {blocks_before_fstrim}\n"
            msg += f"the blocks after fstrim is {blocks_after_fstrim}."
            test.fail(msg)
            ...
            if _is_increase(peak_l, idx_p) or _is_increase(trough_l, idx_t):
                if _is_increase(check_list, idx_cl, ignore_stable_end=False):
                    test.fail(
                        ...
                        f"anyway. {check_type} is {check_list[-1]}"
                    )
    ...
    def gagent_check_debugview_VSS_DLL(self, test, params, env):
        ...
        if s:
            test.error(
                "Debugviewconsole.exe run failed, " f"Please check the output is: {o}"
            )
        gagent.fsfreeze()
        gagent.fsthaw()
