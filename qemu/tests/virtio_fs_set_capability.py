...
def run(test, params, env):
    ...
    cmd_run_virtiofsd += f" -o source={fs_source}"
    ...
    try:
        ...
        try:
            # No extended attributes (file streams) in virtio-fs for windows
            if not is_windows:
                if cmd_set_trusted:
                    error_context.context(
                        "Trusted attribute test without "
                        f"{capability} for linux guest",
                        test.log.info,
                    )
                    ...
                    output = str(process.run(f"getfattr {fs_source}").stdout.strip())
                    test.log.info("The host file trusted is %s.", output)
                    if host_attributes not in output:
                        test.fail(
                            "Failed to check the trusted attribute from "
                            f"host, the output is {output}."
                        )
