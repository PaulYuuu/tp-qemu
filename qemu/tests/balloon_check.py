...
class BallooningTest(MemoryBaseTest):
        ...
        if self.pre_mem + ballooned_mem <= 100:
            ...
            try:
                if self.get_win_mon_free_mem(session) > gcompare_threshold:
                    self.test.fail(f"Balloon_min test failed {step}")
            finally:
                session.close()
        else:
            ...
            if mmem - self.pre_mem != ballooned_mem or (
                ...
                raise exceptions.TestFail(f"Balloon test failed {step}")
        return mmem, gmem
    ...
    def _memory_stats_compare(self, keyname, memory_stat_qmp):
        ...
        error_context.context(
            "Get memory from guest aligned" f" with {keyname}.", self.test.log.info
        )
        ...
        memory_stat_qmp = f"{memory_stat_qmp}B"
        memory_stat_qmp = int(
        ...
        if mem_diff > guest_mem * check_mem_ratio and mem_diff > check_mem_diff:
            self.test.fail(
                f"{keyname} of guest {guest_mem} is not equal to {memory_stat_qmp}"
                " in qmp,the acceptable ratio/diff"
                f" is {check_mem_ratio}/{check_mem_diff}"
            )
    ...
    def balloon_memory(self, new_mem):
        ...
        try:
            ...
            if (
                ...
                raise exceptions.TestFail(
                    "Balloon memory fail with error" f" message: {e}"
                )
        ...
        if status is None:
            raise exceptions.TestFail(
                "Failed to balloon memory to expect" f" value during {balloon_timeout}s"
            )
    ...
    def run_ballooning_test(self, expect_mem, tag):
        ...
        self.memory_check(f"after {tag} memory", ballooned_memory)
        if params_tag.get(
...
class BallooningTestWin(BallooningTest):
    ...
    def get_win_mon_free_mem(self, session):
        ...
        if status == 0:
            free = "{}".format(re.findall(r"\d+\.\d+", output)[2])
    ...
    def operate_balloon_service(self, session, operation):
        ...
        """
        error_context.context(
            f"{operation} Balloon Service in guest.", self.test.log.info
        )
        drive_letter = self.get_disk_vol(session)
        try:
            operate_cmd = self.params[f"{operation}_balloon_service"] % drive_letter
            if operation == "status":
                output = session.cmd_output(operate_cmd)
            else:
                output = session.cmd(operate_cmd)
        except Exception as err:
            self.test.error(f"{operation} balloon service failed! Error msg is:\n{err}")
        return output
...
class BallooningTestLinux(BallooningTest):
    ...
    def error_report(self, step, expect_value, monitor_value, guest_value):
        ...
        """
        self.test.log.error("Memory size mismatch %s:\n", step)
        error_msg = f"Assigner to VM: {expect_value}\n"
        if monitor_value:
            error_msg += f"Reported by monitor: {monitor_value}\n"
        if guest_value:
            error_msg += f"Reported by guest OS: {guest_value}\n"
        self.test.log.error(error_msg)
    ...
    for tag in params.objects("test_tags"):
        ...
        if params.get("balloon_opt_deflate_on_oom") == "yes":
            ...
            if guest_ori_mem != guest_curr_mem:
                ...
                test.fail(f"Balloon test failed {tag}")
        if quit_after_test:
            return
    try:
