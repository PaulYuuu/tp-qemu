...
def run(test, params, env):
    ...
    def check_network_config(session_serial):
        ...
        if static_ip_address != params["static_ip"]:
            test.fail(f"Failed to setup static ip,current ip is {static_ip_address}")
    ...
    for driver_name in test_drivers:
        test_name = params.get(f"driver_test_name_{driver_name}")
        test_func = f"win_driver_installer_test.{test_name}_test"
        driver_test_params = params.get(f"driver_test_params_{driver_name}", "{}")
        ...
        try:
            eval(f"{test_func}(test, params, vm, **driver_test_params)")
        except Exception as e:
            fail_tests.append(f"{test_name}:\n{str(e)}")
    if fail_tests:
        test.fail(f"Function test failed list is {fail_tests} after update")
