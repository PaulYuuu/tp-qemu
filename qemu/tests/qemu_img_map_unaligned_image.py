...
def run(test, params, env):
    ...
    def _verify_qemu_img_map(output, str_len):
        ...
        if res != expected:
            test.fail(
                ...
                f"image '{img.image_filename}' is not correct."
            )
    ...
    process.run(f"rm -f {img.image_filename}")
    process.run(f"truncate -s 1G {img.image_filename} ")

    random_str, str_len = _generate_random_string()
    test.log.info("Write '%s' into the file %s.", random_str, img.image_filename)
    process.run(f"echo -n '{random_str}' > {img.image_filename}", shell=True)
    res = img.map(output="json")
    if res.exit_status != 0:
        test.fail(f"qemu-img map error: {res.stderr_text}.")
