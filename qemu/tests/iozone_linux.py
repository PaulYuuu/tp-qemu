...
def run(test, params, env):
    ...
    def _get_mounted_points(did, disks, mount_info):
        ...
        points = []
        for id in re.finditer(rf"({did}\d+)", " ".join(disks)):
            s = re.search(rf"/dev/{id.group(1)}\s+(\S+)\s+", mount_info, re.M)
            if s:
    ...
    def _wait_for_procs_done(timeout=1800):
        ...
        if not utils_misc.wait_for(
            ...
            test.error(f"Not all iozone processes done in {timeout} sec.")
    ...
    try:
        ...
        if dids:
            ...
            for did, image_name in dids.items():
                size = params.get(f"image_size_{image_name}")
                start = params.get(f"image_start_{image_name}", "0M")
