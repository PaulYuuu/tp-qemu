...
def run(test, params, env):
    ...
    def install_qemu(session):
        ...
        """
        cmd = f"{pm_tool} install -y {qemu_package}"
        s, o = session.cmd_status_output(cmd, timeout=OPERATION_TIMEOUT)
        if s != 0:
            test.error(f"Installation of '{qemu_package}' failed with: {o}")
        test.log.info(f"Installation of '{qemu_package}' succeeded")
    ...
    def update_guest_qemu(session, install_id):
        ...
        try:
            ...
            test.log.info(f"Creation of repo '{install_id}' succeeded")
        except Exception as e:
            test.error(f"Creation of repo '{install_id}' failed with: {e}")
        ...
        if "+" in install_id:
            ...
            if s_disable != 0:
                test.fail(f"Disable of module virt failed with: {o_disable}")
            else:
            ...
            if s_enable != 0:
                test.fail(f"Enable of module virt:{stream} failed with: {o_enable}")
            else:
                test.log.info(f"Enable of module virt:{stream} succeeded")
        # Run upgrade to newer qemu-kvm version
        if "+" in install_id:
            cmd_upgrade = f"{pm_tool} module update -y virt:{stream}"
        else:
            cmd_upgrade = f"{pm_tool} upgrade -y {qemu_package}"
        s_upgrade, o_upgrade = session.cmd_status_output(
            cmd_upgrade, timeout=INSTALL_TIMEOUT
        )
        if s_upgrade != 0:
            test.fail(f"Upgrade of '{qemu_package}' failed with: {o_upgrade}")
        test.log.info(f"Upgrade of '{qemu_package}' succeeded")
    ...
    def verify_installed_qemu(host_qemu, guest_qemu):
        ...
        if installed_nvr == expected_nvr:
            test.log.info(f"NVR of installed pkg '{installed_nvr}' is correct")
        else:
            test.fail(
                f"NVR of installed pkg '{installed_nvr}' differs from expected '{expected_nvr}'"
            )
