...
class EnospcConfig:
    ...
    def setup(self):
        ...
        try:
            process.run(
                f"{self.qemu_img_binary} create -f raw {self.raw_file_path} 10G"
            )
            ...
            process.run(f"losetup -f {self.raw_file_path}")
            self.loopback = l_result.stdout.decode().strip()
            # Add the loopback device configured to the list of pvs
            # recognized by LVM
            process.run(f"pvcreate {self.loopback}")
            process.run(f"vgcreate {self.vgtest_name} {self.loopback}")
            # Create an lv inside the vg with starting size of 200M
            process.run(f"lvcreate -L 200M -n {self.lvtest_name} {self.vgtest_name}")
            # Create a 10GB qcow2 image in the logical volume
            process.run(
                f"{self.qemu_img_binary} create -f qcow2 {self.lvtest_device} 10G"
            )
    ...
    def cleanup(self):
        ...
        if os.path.islink(self.lvtest_device):
            process.run(f"fuser -k {self.lvtest_device}", ignore_status=True)
            time.sleep(2)
...
def run(test, params, env):
    ...
    logical_volume = f"/dev/{vgtest_name}/{lvtest_name}"
    ...
    while i < iterations:
        if vm.monitor.verify_status("paused"):
            ...
            error_context.context(
                f"Checking all images in use by {vm.name}", test.log.info
            )
            ...
            try:
                process.run(f"lvextend -L +200M {logical_volume}")
            ...
            test.error(f"Unexpected guest status: {status}")
        time.sleep(10)
        i += 1
