...
def run(test, params, env):
    ...
    def check_attribute_in_qtree(dev_id, name, excepted_val):
        ...
        error_context.context(f"Check if {name} attribute works.", test.log.info)
        ...
        for node in qtree.get_nodes():
            if isinstance(node, qemu_qtree.QtreeDev) and node.qtree.get("id") == dev_id:
                ...
                if _node.qtree.get("drive").endswith(f'_{dev_id}"'):
                    if _node.qtree.get(name) is None:
                        test.fail(f"The qtree device {dev_id} has no property {name}.")
                        ...
                        test.fail(
                            f'The "{name}" mismatches with qtree device "{dev_id}"'
                            f"({excepted_val})."
                        )
        else:
    ...
    if params["start_vm"] == "no":
        ...
        try:
            ...
            if error_msg not in str(e):
                test.fail(f'No found "{error_msg}" from the output of qemu:{str(e)}.')
        return
