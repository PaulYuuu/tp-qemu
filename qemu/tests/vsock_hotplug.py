...
def run(test, params, env):
    ...
    device_str = re.findall(rf"{addr_pattern}\s{device_pattern}", lspci_output)

    if params.get("dmesg_check") == "yes":
        if not device_str:
            test.fail(f'check_vsock_cmd failed, no device "{device_pattern}"')
        else:
            address = re.findall(addr_pattern, device_str[0])[0]
            chk_dmesg_cmd = "dmesg"
            output = re.findall(address, session.cmd_output(chk_dmesg_cmd))
            if not output:
                test.fail(f"dmesg failed, no info related to {address}")
            else:
                error_msg = ""
                for o in output:
                    if re.search(r"fail|error", o, re.I):
                        ...
                        break
                if error_msg:
                    test.fail(f"dmesg check failed: {error_msg}")
    ...
    try:
        ...
        session.cmd_output(f"rm -f {tmp_file}")
        session.close()
    vm.reboot()
