...
def check_query_memdev(test, params, vm):
    ...
    for dev in query_list:
        ...
        if dev["policy"] != policy:
            test.fail(
                "memdev = {}: 'policy' is '{}', but not '{}'!".format(
        ...
        if host_node != memdev_params["host-nodes"]:
            test.fail(
                "memdev = {}: 'host-nodes' is '{}', but not '{}'!".format(
...
def check_memory_in_procfs(test, params, vm):
    ...
    for mem_dev in params["mem_devs"].split():
        ...
        if mem_path and (mem_path not in smaps):
            test.fail(
                f"memdev = {mem_dev}: mem-path '{mem_path}' is not in smaps '{smaps}'!"
            )
        ...
        if mem_path and (mem_path not in numa_maps):
            test.fail(
                f"memdev = {mem_dev}: mem-path '{mem_path}' is not in numa_maps '{numa_maps}'!"
            )
        ...
        if policy != policy_numa[0]:
            test.fail(
                f"memdev = {mem_dev}:"
                f" 'policy' in numa_maps is '{policy_numa[0]}', but not '{policy}'!"
            )
        elif policy != "default":
            ...
            if policy_numa[1] != host_node:
                test.fail(
                    f"memdev = {mem_dev}:"
                    f" 'host-nodes' in numa_maps is '{policy_numa[1]}', but not '{host_node}'!"
                )
