...
def run(test, params, env):
        ...
        def wrapper(*args, **kwargs):
            if not utils_disk.is_mount(mapper_dev, mount_path, session=session):
                ...
                if not utils_disk.mount(mapper_dev, mount_path, session=session):
                    test.error(f"Cannot mount {mapper_dev} to {mount_path}")
    ...
    def compare_md5sum():
        ...
        if md5_current != md5_original:
            test.fail(
                f"File {dd_file} changed, the current md5({md5_current}) is mismatch of the"
    ...
    def auto_boot_unlocking():
        ...
        """
        disk_uuid = session.cmd_output(f"blkid -s UUID -o value {extra_disk}").strip()
        session.cmd(
            f'echo "{mapper_name} UUID={disk_uuid} none tpm2-device=auto" >> /etc/crypttab'
        )
        session.cmd(f'echo "{mapper_dev} {mount_path} xfs defaults 0 0" >> /etc/fstab')
        session.cmd("restorecon -Rv " + mount_path)
        s, o = session.cmd_status_output("mount -av")
        if s != 0:
            test.fail(f"Mount format is incorrect:\n{o}")
    ...
    error_context.base_context(
        f"Bind {extra_disk} using the TPM2 policy", test.log.info
    )
    session.cmd(clevis_bind_cmd % extra_disk)
    clevis_list = session.cmd_output(clevis_list_cmd + extra_disk)
    if not re.search(rf"tpm2 \S+{pcr_policy}", clevis_list, re.M):
