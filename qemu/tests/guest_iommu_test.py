...
def check_data_disks(test, params, env, vm, session):
    ...
    def _get_mount_points():
        ...
        if os_type == "linux":
            ...
            for img in image_list:
                size = params[f"image_size_{img}"]
                img_param = params[f"blk_extra_params_{img}"].split("=")[1]
                drive_path = utils_misc.get_linux_drive_path(session, img_param)
                if not drive_path:
                    test.error(f"Failed to get drive path of '{img}'")
                did = drive_path[5:]
                for mp in re.finditer(rf"/dev/{did}\d+\s+(\S+)\s+", mounts):
            ...
            for img in image_list:
                ...
                if not disks:
                    test.fail(f"Fail to list image {img}")
    ...
    for image in image_list:
        drive = f"drive_{image}"
        if drive not in blocks:
            test.fail(f"{drive} is missing: {blocks}")
...
def verify_eim_status(test, params, session):
    ...
    if variant_name == "eim_off" or variant_name == "eim_on":
        for key_words in params["check_key_words"].split(";"):
            output = session.cmd_output(f'journalctl -k | grep -i "{key_words}"')
        if not output:
            test.fail(
                f'journalctl -k | grep -i "{key_words}"' "from the systemd journal log."
            )
