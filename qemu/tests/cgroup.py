...
class SparseRange(list):
    ...
    def __init__(self, value):
        ...
        try:
            ...
            raise ValueError(f"Can't parse SparseRange from {value}: {details}")
...
def run(test, params, env):
    ...
    def get_dd_cmd(direction, count=None, blocksize=None):
        ...
        if blocksize:
            params += f" bs={blocksize}"
        if count:
            params += f" count={count}"
        return (
            f"export FILE=$(ls /dev/disk/by-id/*{RANDOM_DISK_NAME} | tail -n 1); touch /tmp/cgroup_lock"
            f" ; while [ -e /tmp/cgroup_lock ]; do dd {params} ; done"
        )
    ...
    def get_maj_min(dev):
        ...
        try:
            ...
            raise exceptions.TestFail(f"get_maj_min({dev}) failed: {details}")
        return ret
    ...
    def param_add_scsi_disks():
        ...
        for name in params["vms"].split(" "):
            ...
            process.system(
                "echo 1 > /sys/block/{}/queue/rotational".format(dev.split("/")[-1]),
                shell=True,
            )
            vm_disks = params.get(f"images_{name}", params.get("images", "image1"))
            params[f"images_{name}"] = f"{vm_disks} {disk_name}"
            params[f"image_name_{disk_name}"] = dev
            params[f"image_snapshot_{disk_name}"] = "no"
            params[f"image_format_{disk_name}"] = "raw"
            params[f"remove_image_{disk_name}"] = "no"
            params[f"image_raw_device_{disk_name}"] = "yes"
            params[f"drive_cache_{disk_name}"] = params.get("drive_cache", "none")
            params[f"drive_serial_{disk_name}"] = RANDOM_DISK_NAME
    ...
    def param_add_file_disks(size, prefix="hd2-"):
        ...
        for name in params["vms"].split(" "):
            ...
            params[f"images_{name}"] = f"{vm_disks} {disk_name}"
            params[f"image_size_{disk_name}"] = size
            params[f"image_name_{disk_name}"] = disk_name
            params[f"image_snapshot_{disk_name}"] = "no"
            params[f"force_create_image_{disk_name}"] = "yes"
            params[f"image_format_{disk_name}"] = "raw"
            params[f"create_with_dd_{disk_name}"] = "yes"
            params[f"remove_image_{disk_name}"] = "yes"
            params[f"drive_cache_{disk_name}"] = params.get("drive_cache", "none")
            params[f"drive_serial_{disk_name}"] = RANDOM_DISK_NAME
    ...
    def blkio_bandwidth():
        ...
        def _test(direction):
            ...
            err = ""
            limit = float(
                params.get(f"cgroup_limit_{direction}", params.get("cgroup_limit", 0.1))
            )
            ...
            if err:
                err = f"blkio_bandwidth_{direction}: limits [{err[:-2]}] were broken"
                test.log.debug(err)
                return err + "\n"
            return ""
    ...
    def blkio_throttle_multi(speeds=None):
        ...
        def _test(direction, blkio):
            ...
            if err:
                err = f"blkio_throttle_{direction}: limits [{err[:-2]}] were broken"
                test.log.debug(err)
                return err + "\n"
            return ""
        ...
        if speeds:  # blkio_throttle
            ...
            try:
                ...
                if not isinstance(speeds, list):
                    raise TypeError
                if not isinstance(speeds[0], list):
        ...
        for i in range(no_vms):
            ...
            dev = get_maj_min(params[f"image_name_scsi-debug-{vms[i].name}"])
            for j in range(no_speeds):
                speed = speeds[i][j]
                blkio.mk_cgroup()
    ...
    def cpu_cfs_util():
        ...
        try:
            ...
            time.sleep(1)
            stats.append(open("/proc/stat").readline())
            time.sleep(1)
            stats.append(open("/proc/stat").readline())
            time.sleep(9)
            stats.append(open("/proc/stat").readline())
        ...
        test.log.info("Results")
        if err:
            err = f"The host vs. guest CPU time ratio is over {limit} in {err} cases"
            raise exceptions.TestFail(err)
        else:
            return f"Guest times are over {limit}%: {stats[1:]}"
    ...
    def cpu_share():
        ...
        try:
            ...
            # Time 0
            for vm in vms:
                f_stats.append(open("/proc/%d/stat" % vm.get_pid()))
            ...
            if err:
                err = f"[{err[:-2]}] parts broke their limits"
                test.log.error(err)
            else:
    ...
    def cpuset_cpus():
        ...
        try:
            ...
            if not (isinstance(cpusets, list) or (cpusets is None)):
                raise Exception
        except Exception:
            raise exceptions.TestError(
        ...
        if cpusets:
            if len(cpus) < (len(cpusets[0]) - 1):
                err = (
                    ...
                    f"cpusets (cpus={len(cpus)}, cpusets={cpusets})"
                )
                test.log.error(err)
        ...
        if len(vcpus) != vm_cpus:
            raise exceptions.TestFail(
                f"Incorrect number of vcpu PIDs; smp={vm_cpus} vcpus=" f"{vcpus}"
            )
    ...
    def cpuset_cpus_switching():
        ...
        if len(cpus) < 2:
            raise exceptions.TestFail(
                "This test needs at least 2 CPUs on " f"host, cpuset={cpus}"
            )
        ...
        try:
            ...
            try:
                ...
                err += f"VM died (no_switches={i}): {exc_details}\n"
    ...
    def cpuset_mems_switching():
        ...
        if len(mems) < 2:
            raise exceptions.TestSkipError(
                "This test needs at least 2 memory nodes, " f"detected mems {mems}"
            )
        # Create cgroups
        ...
        try:
            ...
            try:
                ...
                err = f"dd didn't produce expected output: {out}"
    ...
    def devices_access():
        ...
        def _add_drive(monitor, monitor_type, disk, name, readonly=False):
            ...
            if monitor_type == "HUMAN PCI_ADD":
                out = monitor.cmd(
                    f"pci_add auto storage file={disk},readonly={readonly_str},"
                    f"if=virtio,id={name}"
                )
                ...
                monitor.cmd(
                    f"drive_add auto file={disk},readonly={readonly_str},if=none,id={name}"
                )
                if name not in monitor.cmd("info block"):
                    return False
            elif monitor_type == "HUMAN RH":
                monitor.cmd(
                    f"__com.redhat_drive_add id={name},file={disk},readonly={readonly_str}"
                )
                if name not in monitor.cmd("info block"):
                    return False
        ...
        try:
            ...
            if err:
                err = f"Some restrictions weren't enforced:\n{err[:-2]}"
                test.log.error(err)
            else:
    ...
    def freezer():
        ...
        def _get_stat(pid):
            ...
            for i in range(10):
                try:
                    out = process.getoutput(f"cat /proc/{pid}/task/*/stat", shell=True)
        ...
        try:
            ...
            for tsttime in [0.5, 3, test_time]:
                ...
                if stat != stat_:
                    err = (
                        f"Process was running in FROZEN state; stat={stat}, "
                        f"stat_={stat_}, diff={stat - stat_}"
                    )
                    break
                ...
                if (stat - stat_) < (90 * tsttime):
                    err = (
                        f"Process was not active in FROZEN state; stat={stat}, "
                        f"stat_={stat_}, diff={stat - stat_}"
                    )
                    break
    ...
    def memory_limit(memsw=False):
        ...
        try:
            ...
            raise exceptions.TestFail(
                f"init: Failed to recreate the VM: {failure_detail}"
            )
        assign_vm_into_cgroup(vm, cgroup, 0)
        timeout = int(params.get("login_timeout", 360))
        ...
        fstats = open(f"/proc/{vm.get_pid()}/status")
            ...
            * Checking every 0.1s
            ...
            for _ in range(int(mem / 1024)):
                ...
                try:
                    ...
                    if memsw:
                        err = (
                            f"dd command died (VM should die instead): {detail}\n"
                            f"Output:{out}\n"
                        )
                    else:
                        err = (
                            f"dd command died (should pass): {detail}\nOutput:"
                            f"\n{out}"
                        )
                    break
            ...
            if err:
                ...
                if max_rssswap > mem_limit:
                    err = f"The limit was broken: max_rssswap={max_rssswap}, limit={mem_limit}"
                elif vm.process.get_status() != 137:  # err: Limit exceeded
                    err = f"VM exit code is {vm.process.get_status()} (should be {137})"
                else:
                    out = (
                ...
                if max_rss > mem_limit * 1.05:
                    ...
                    err = f"The limit was broken: max_rss={max_rss}, limit={mem_limit} (+5%)"
                elif exit_nr != "0":
                    err = f"dd command failed({exit_nr}) output: {out}"
    ...
    def cfs_bandwidth():
        ...
        while count < 6:
            ...
            o = process.system_output(
                f"top -b -p {vm.get_pid()} -n 3 | tail -1", shell=True
            ).decode()
            if float(o.split()[-4]) <= 51:
                ...
                test.fail(
                    f"CPU utilization of guest is: {float(o.split()[-4]):.2f}%, it should be about "
                    "50"
                )
        stress_t.join()
    ...
    try:
        ...
        raise exceptions.TestSkipError(
            f"Test {_fce} doesn't exist. Check 'cgroup_test' " "variable in subtest.cfg"
        )
    else:
        return fce()
