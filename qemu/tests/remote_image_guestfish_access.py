...
def run(test, params, env):
    ...
    if result.exit_status != 0:
        test.fail(f"Failed to write a file, error message: {result.stderr.decode()}")

    test.log.info("Read file '%s'", testfile)
    result = process.run(read_cmd, ignore_status=True, shell=True)
    if result.exit_status != 0:
        test.fail(f"Failed to read a file, error message: {result.stderr.decode()}")
    elif result.stdout.decode().strip() != msg:
        test.fail(f"Message '{msg}' mismatched with '{result.stdout.decode()}'")
