...
def run(test, params, env):
    ...
    if os_type == "linux":
        ...
        if utils_package.package_install("sysstat", session):
            ...
            mpstat_cmd = "mpstat 1 5 -P {} | cat".format(
                ",".join(map(str, plugged_cpu_ids))
            )
            ...
            for cpu_id in plugged_cpu_ids:
                ...
                if cpu_usage_rate < 50:
                    test.error(
                        f"Stress test on vCPU({cpu_id}) failed, usage rate: "
                        f"{cpu_usage_rate:.2f}%"
                    )
                test.log.info(
        ...
        if not vm_arch_name.startswith("s390"):
            for vcpu_dev in vcpu_devices:
                error_context.context(
                    f"Hotunplug vcpu device: {vcpu_dev}", test.log.info
                )
