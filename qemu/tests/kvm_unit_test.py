...
def run(test, params, env):
    ...
    if not os.path.isdir(unittest_dir):
        test.cancel(
            f"No unittest dir {unittest_dir} available (did you run the "
            "build test first?)"
        )
    ...
    if not test_list:
        test.error(f"No tests listed on config file {unittest_cfg}")
    ...
    for t in test_list:
        ...
        if parser.has_option(t, "extra_params"):
            ...
            if not params.get("extra_params"):
                params["extra_params"] = ""
            params["extra_params"] += f" {extra_params}"

        vm_name = params["main_vm"]
        params["kernel"] = os.path.join(unittest_dir, flat_file)

        testlog_path = os.path.join(test.debugdir, f"{t}.log")
        ...
        try:
            try:
                ...
                msg = f"Waiting for unittest '{t}' to complete, timeout {timeout}"
                if os.path.isfile(testlog):
                    msg += f", output in {testlog}"
                else:
                    testlog = None
                test.log.info(msg)

                if not utils_misc.wait_for(vm.is_dead, timeout):
                    test.fail(f"Timeout elapsed ({timeout}s)")
    ...
    if nfail != 0:
        test.fail("Unit tests failed: {}".format(" ".join(tests_failed)))
