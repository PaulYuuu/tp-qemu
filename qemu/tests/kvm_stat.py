...
def run(test, params, env):
    ...
    def check_log():
        ...
        if check_log_num == rotate_time:
            test.log.info(f"Get the expected log file num {check_log_num}")
        else:
            test.fail(f"Except {rotate_time} log file, but get {check_log_num}")

    def restore_env():
        test.log.info("Restore the host environment")
        tweak_file(
            r"-s\s([\.\d]+)\s",
            f"-s {initial_kvm_stat_interval} ",
            kvm_stat_service_path,
        )
        tweak_file(
            r"size \s(.*)\s",
            f"size {initial_rotate_size}",
    ...
    with open(kvm_stat_service_path) as fd:
        content = fd.read()
    ...
    with open(logrotate_config_file_path) as fd:
        content = fd.read()
    ...
    try:
        ...
        tweak_file(
            rf"-s {initial_kvm_stat_interval}",
            f"-s {kvm_stat_interval}",
            kvm_stat_service_path,
        )
        ...
        tweak_file(
            rf"size {initial_rotate_size}",
            f"size {rotate_size_limit}",
            logrotate_config_file_path,
        )
