...
def run(test, params, env):
    ...
    def verify_backing_file(image):
        ...
        if backing_file != backing_file_info:
            err_msg = f"backing file mismatch, got {backing_file_info}, expected {backing_file}."
    ...
    params[f"image_size_{rebase_target}"] = images[0].size
    ...
    params[f"remove_image_{snapshot.tag}"] = "yes"
    params["images"] += f" {rebase_target.tag}"
