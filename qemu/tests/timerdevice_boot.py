...
def run(test, params, env):
    ...
    def verify_timedrift(session, is_hardware=False):
        ...
        if drift > timerdevice_drift_threshold:
            test.fail(
                f"The guest's {time_type} time is different with"
                f" host's system time. Host time: '{host_time}', guest time:"
                f" '{guest_time}'"
            )
    ...
    def update_clksrc(session, clksrc):
        ...
        if clksrc in avail_clksrc:
            clksrc_cmd = f"echo {clksrc} > /sys/devices/system/clocksource/"
            clksrc_cmd += "clocksource0/current_clocksource"
            status, output = session.cmd_status_output(clksrc_cmd, safe=True)
            if status:
                test.fail(
                    "fail to update guest's clocksource to {}," "details: {}".format(
                        *clksrc
                    ),
                    output,
                )
            ...
            test.error(
                ...
                f"available clocksources: {avail_clksrc}"
            )
    ...
    if params.get("timerdevice_reboot_test") == "yes":
        ...
        if sleep_time:
            error_context.context(
                f"Sleep '{sleep_time}' secs before reboot", test.log.info
            )
