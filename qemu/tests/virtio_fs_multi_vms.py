    ...
    for vm, vm_obj, session in zip(params.objects("vms"), vms, sessions):
        ...
        if os_type == "windows":
            ...
            error_context.context(
                f"{vm}: Install winfsp for windows guest.", test.log.info
            )
            ...
            if installed:
                ...
                if not utils_misc.wait_for(
                    ...
                    test.error(f"{vm}: Winfsp tool is not installed.")

            error_context.context(
                f"{vm}: Start virtiofs service in guest.", test.log.info
            )
            ...
            if "not exist as an installed service" in output:
                ...
                if sc_create_s != 0:
                    test.fail(
                        f"Failed to register virtiofs service, output is {sc_create_o}"
                    )
            ...
            if "RUNNING" not in output:
                ...
                if sc_start_s != 0:
                    test.fail(
                        f"Failed to start virtiofs service, output is {sc_start_o}"
                    )
            else:
            ...
            if viofs_debug_enable_cmd and viofs_log_enable_cmd:
                ...
                if "debugflags" not in ret.lower() or "debuglogfile" not in ret.lower():
                    ...
                    for reg_cmd in (viofs_debug_enable_cmd, viofs_log_enable_cmd):
                        error_context.context(f"Set {reg_cmd} ", test.log.info)
                        s, o = session.cmd_status_output(reg_cmd)
                        if s:
                            test.fail(f"Fail command: {reg_cmd}. Output: {o}")
        ...
        for fs in vm_params.objects("filesystems"):
            ...
            if os_type == "linux":
                error_context.context(
                    f"{vm}: Create a destination directory {fs_dest} inside guest.",
                    test.log.info,
                )
                utils_misc.make_dirs(fs_dest, session)

                error_context.context(
                    f"{vm}: Mount the virtiofs target {fs_target} to {fs_dest} inside guest.",
                    test.log.info,
                )
                ...
                error_context.context(
                    f"{vm}: Get Volume letter of virtio fs"
                    f" target, the disk label is {virtio_fs_disk_label}.",
                    test.log.info,
                )
            ...
            if os_type == "linux":
                error_context.context(
                    f"{vm}: Umount the viriofs target {fs_target}.", test.log.info
                )
    ...
    if shared_fs_source_dir:
        ...
        for vm, info in mapping.items():
            ...
            for fs in info["filesystems"]:
                ...
                error_context.context(
                    f"{vm}: Get the md5 of {shared_data}.", test.log.info
                )
                if os_type == "linux":
                ...
                if os_type == "linux":
                    error_context.context(
                        "{}: Umount the viriofs target {}.".format(vm, fs["fs_target"]),
                        test.log.info,
                    )
                    utils_disk.umount(
