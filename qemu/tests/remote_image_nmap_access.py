...
def run(test, params, env):
    ...
    params["nbd_server_{}".format(params["nbd_image_tag"])] = h if h else "localhost"
    nmap_cmd = params["nmap_cmd"].format(
        localhost=params["nbd_server_{}".format(params["nbd_image_tag"])]
    )
    try:
        result = process.run(nmap_cmd, ignore_status=True, shell=True)
        if result.exit_status != 0:
            test.fail(f"Failed to execute nmap, error: {result.stderr.decode()}")
