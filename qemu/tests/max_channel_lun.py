...
def run(test, params, env):
    ...
    for i in range(stg_image_num):
        ...
        params[f"blk_extra_params_{name}"] = channel
        params[f"drive_port_{name}"] = i
        params[f"scsi_hba_{name}"] = "spapr-vscsi"
    if params["luns"] == "lun_33":
        ...
        params["extra_params"] = f"-blockdev node-name=file_stg32,\
driver=file,auto-read-only=on,discard=unmap,aio=threads,filename={image.image_filename},\
...
drive=drive_stg32,write-cache=on,channel=0"
    ...
    try:
        ...
        if error_info not in e.output:
            test.fail(f"{error_info} is not reported by QEMU")
    ...
    if params["luns"] == "lun_32":
        ...
        cmd_w = f"dd if=/dev/zero of=/dev/{disk} bs=1M count=8"
        cmd_r = f"dd if=/dev/{disk} of=/dev/null bs=1M count=8"
