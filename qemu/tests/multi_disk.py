...
def run(test, params, env):
    ...
    def _get_data_disks():
        if ostype == "windows":
            ...
            if len(data_disks) < stg_image_num:
                test.fail(
                    "Fail to list all the volumes" f", {err_msg}" % len(data_disks)
                )
    ...
    for i in range(stg_image_num):
        ...
        if not has_name:
            params[f"image_name_{name}"] = stg_image_name % name
            param_table[-1].append(params.get(f"image_name_{name}"))
        for parm in param_matrix.items():
            params[f"{parm[0]}_{name}"] = str(parm[1][i % len(parm[1])])
            param_table[-1].append(params.get(f"{parm[0]}_{name}"))
    ...
    try:
        for i in range(n_repeat):
            ...
            for n, disk in enumerate(disks):
                error_context.context(f"Format disk in guest: '{disk}'", test.log.info)
                ...
                for partition in partitions:
                    ...
                    error_context.context(
                        "Copy file into / out of partition:" f" {partition}...",
                        test.log.info,
                    )
                    ...
                    if dd_test != "no":
                        error_context.context(
                            f"dd test on partition: {partition}...", test.log.info
                        )
                        status, output = session.cmd_status_output(
                            dd_test % (partition, partition), timeout=cmd_timeout
                        )
                        if status != 0:
                            test.fail(f"dd test fail: {output}")
                    ...
                    if ostype == "linux" and "scsi_debug add_host" in pre_command:
                        status, output = session.cmd_status_output(
                            f"umount /dev/{partition}", timeout=cmd_timeout
                        )
                        if status != 0:
                            test.fail(
                                f"Failed to umount partition '{partition}': {output}"
                            )
