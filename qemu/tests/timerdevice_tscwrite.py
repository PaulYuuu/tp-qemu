...
def run(test, params, env):
    ...
    session.cmd(f"cd {msr_dir} && tar -zxvf {os.path.basename(tarball)}")
    session.cmd(f"cd {msr_name} && {compile_cmd}")
    ...
    if "0" not in output:
        test.fail(f"Test failed before run msr tools. Output: '{output}'")
    ...
    output = session.cmd(date_cmd)
    if "1" not in output:
        test.fail(f"Test failed after run msr tools. Output: '{output}'")
