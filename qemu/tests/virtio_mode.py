...
def run(test, params, env):
    ...
    def verify_virtio_mode_qtree():
        ...
        if (
            ...
            test.fail(
                f"virtio mode in qtree is not correct, details are {disable_modern} {disable_legacy}"
            )
    ...
    def win_memory_range(session, devcon_folder, hwid, virtio_mode):
        ...
        :param virtio_mode: VirtIO mode for the device
        """
        mem_check_cmd = f'{devcon_folder}devcon.exe resources @"{hwid}" | find "MEM"'
    ...
    def verify_virtio_mode_guest_linux(session):
        ...
        cmd = f"grep . /sys/bus/pci/devices/0000:{pci_n}/virtio*/features"
