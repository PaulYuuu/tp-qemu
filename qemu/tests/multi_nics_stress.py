...
def run(test, params, env):
    ...
    if client in vms:
        ...
        if params.get("dhcp_cmd"):
            ...
            if status:
                test.log.warn("Failed to execute dhcp-command, output:\n %s", output)
    ...
    if params.get("os_type") == "linux":
        ...
        for ip in server_ips:
            ...
            client_ctl.cmd(f"route add  -host {ip} {nics_list[index]}")
    ...
    try:
        ...
        for protocol in test_protocols.split():
            ...
            for size in sizes_test:
                for sess in sessions_test:
                    ...
                    test_option += f" -t {protocol} -l {test_duration} "
                    test_option += f" -- -m {size}"
        ...
        for s_ip in server_ips:
            ...
            if package_lost != 0:
                test.fail(f"{package_lost} packeage lost when ping server ip {server} ")
    finally:
        for n_server in netperf_servers:
            n_server.stop()
