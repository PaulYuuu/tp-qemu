...
def run(test, params, env):
    ...
    def generate_image(dev_slots, plug, params, qdev, image_size, pcie, test):
        ...
        for slot in dev_slots:
            scsi_bus = 1
            parent_bus = f"pcie_extra_root_port_{slot}" if pcie else "pci.0"
            images = []
            for i in range(1, 9):
                stg = f"stg{slot}{i}"
                images.append(stg)
                params["images"] += f" {stg}"
                params[f"image_name_{stg}"] = f"images/{stg}"
                params[f"image_size_{stg}"] = image_size
                params[f"remove_image_{stg}"] = "yes"
                params[f"force_create_image_{stg}"] = "no"
                params[f"create_image_{stg}"] = "yes"
                params[f"boot_drive_{stg}"] = "no"
