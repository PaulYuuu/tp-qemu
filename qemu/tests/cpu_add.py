...
def run(test, params, env):
    ...
    def cpu_online_offline(session, cpu_id, online=""):
        ...
        if online == "online":
            ...
            online = 0
        online_file = f"/sys/devices/system/cpu/cpu{cpu_id}/online"
        if session.cmd_status(f"test -f {online_file}"):
            test.log.info(
                "online file %s not exist, just pass the cpu%s", online_file, cpu_id
            )
            return
        session.cmd(f"echo {online} > {online_file} ")
    ...
    for i in range(vcpu_need_hotplug):
        hotplug_vcpu_params = params.object_params(f"hotplug_vcpu{i}")
        ...
        if status:
            ...
            if qmp_check_info and re.findall(qmp_check_info, output, re.I):
                ...
                err_msg += f"the hotplug cpu_id is: '{plug_cpu_id}', "
                err_msg += f"the maxcpus allowed is: '{maxcpus}', "
                err_msg += "qemu cpu list is:'{}'".format(vm.monitor.info("cpus"))
    ...
    for i in range(repeat_time):
        for offline_cpu in offline_list:
            ...
            if ntp_query_cmd:
                ...
                if current_offset > acceptable_offset:
                    test.fail(
                        f"time drift({current_offset}s)"
                        + f"after online cpu({offline_cpu})"
                    )
        for online_cpu in online_list:
            ...
            if ntp_query_cmd:
                ...
                if current_offset > acceptable_offset:
                    test.fail(
                        f"time drift({current_offset})"
                        + f"after offline cpu({online_cpu})"
                    )
    ...
    if params.get("run_sub_test", "no") == "yes" and "sub_test_name" in params:
        ...
        error_context.context(
            f"Run subtest {sub_test} after cpu hotplug", test.log.info
        )
