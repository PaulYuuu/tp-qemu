...
def run(test, params, env):
    ...
    def get_procs():
        ...
        for x in range(0, int(nums_cpu)):
            pipefile = f"/tmp/virtio-trace/trace-path-cpu{x}.out"
            proc = aexpect.run_bg(f"cat {pipefile}")
            procs.append(proc)
        return procs
    ...
    try:
        ...
        if not os.path.isdir(v_path):
            process.run(f"mkdir {v_path}")
        for t in ["in", "out"]:
            process.run(f"mkfifo {v_path}agent-ctl-path.{t}")
            for x in range(int(nums_cpu)):
                process.run(f"mkfifo {v_path}trace-path-cpu{x}.{t}")

        enable_cmd = "echo 1 > /tmp/virtio-trace/agent-ctl-path.in"
        disable_cmd = "echo 0 > /tmp/virtio-trace/agent-ctl-path.in"
        for x in range(int(nums_cpu)):
            serials += f" vs{x} "
            params[f"serial_type_vs{x}"] = "virtserialport"
            params[f"chardev_backend_vs{x}"] = "pipe"
            params[f"serial_name_vs{x}"] = f"trace-path-cpu{x}"
            params[f"chardev_path_vs{x}"] = f"{v_path}trace-path-cpu{x}"
        ...
        if status != 0:
            test.error(f"Enable ftrace in the guest failed as {output}")
        ...
        for index, proc in enumerate(procs):
            if not proc.get_output():
                test.fail(f"cpu {index} do not have output while it is enabled in host")
            proc.close()
        ...
        for index, proc in enumerate(procs):
            if proc.get_output():
                test.fail(f"cpu {index} still have output after disabled in host")
            proc.close()
    finally:
        process.run(f"rm -rf {v_path}")
