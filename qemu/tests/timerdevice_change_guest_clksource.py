...
def run(test, params, env):
    ...
    def verify_guest_clock_source(session, expected):
        if expected not in session.cmd(cur_clk):
            test.fail(f"Guest didn't use '{expected}' clocksource")
    ...
    try:
        ...
        test.fail("Couldn't get guest available clock source." f" Detail: '{detail}'")
    try:
        for avl_clksrc in available_clksrc_list:
            if avl_clksrc == "kvm-clock":
                continue
            error_context.context(
                f"Update guest kernel cli to '{avl_clksrc}'", test.log.info
            )
