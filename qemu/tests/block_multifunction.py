...
def set_addr(image_name, slot, function, params, multifunction="on"):
    ...
    if params["drive_format"].startswith("scsi"):
        param_name = f"bus_extra_params_{image_name}"
    else:
        param_name = f"blk_extra_params_{image_name}"
    if function % 8 == 0:
        LOG_JOB.info("Set multifunction=on for %s", image_name)
        params[param_name] = f"multifunction={multifunction}"
        if function == 0:
            return
    addr_pattern = f"addr={hex(slot)}.{hex(function % 8)}"
...
def run(test, params, env):
    ...
    for i in range(1, 9):
        stg = f"stg{i}"
        vm.params["images"] += f" {stg}"
        vm.params[f"image_name_{stg}"] = f"images/{stg}"
        vm.params[f"image_size_{stg}"] = image_size
        vm.params[f"remove_image_{stg}"] = "yes"
        vm.params[f"force_create_image_{stg}"] = "yes"
        vm.params[f"boot_drive_{stg}"] = "no"
