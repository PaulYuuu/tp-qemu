    ...
    try:
        if bg_stress_test:
            error_context.context(
                f"Run test {bg_stress_test} background", test.log.info
            )
            stress_thread = ""
        ...
        error_context.context(
            f"Run suspend '{suspend_type}' test under stress", test.log.info
        )
        ...
        if bg.is_alive():
            try:
                ...
                err_msg = f"Run guest suspend: '{suspend_type}' error!\n"
                err_msg += f"Error info: '{e}'"
                test.fail(err_msg)
