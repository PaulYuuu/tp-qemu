...
def run_server(host, port, daemon, path, queue_size, threshold, drift):
    ...
    while 1:
        ...
        if verbose:
            if check_drift:
                print(f"{local_timestamp:.2f}: {heartbeat} ({drift})")
            else:
                print(f"{local_timestamp:.2f}: {heartbeat}")
...
def run_client(host, port, daemon, path, interval):
    ...
    while 1:
        try:
            ...
            print(f"{float(time.time()):.2f}: ERROR - {message}")
...
def check_heartbeat(heartbeat, local_timestamp, threshold, check_drift):
    ...
    if check_drift:
        ...
        return f"drift {drift:+4.2f} ({drift_delta:+4.2f})"
...
for param, value in opts:
    if param in ["-p", "--port"]:
        ...
        print(f"error: unrecognized option: {value}")
        usage()
        sys.exit(1)
